...
/**
 * This class is geared to replace the ListCellRenderer in the long run to
 * provide a more generic rendering API. A "rubber stamp" tool that allows us to
 * extract a component (often the same component instance for all invocations)
 * that is initialized to the value of the current item extracted from the
 * model, this component is drawn on the list and discarded. No state of the
 * component is kept and the component is essentially discarded.
 * <p>
 ...
 * support features such as repaint(). This is accomplished by overriding
 * repaint in the subclass with an empty implementation. This is advised for
 * performance reasons, otherwise every change made to the component might
 * trigger a repaint that wouldn't do anything but still cost in terms of
 * processing.
 * 
...
public interface CellRenderer {
	/**
	 * Returns a component instance that is already set to render "value". While
	 * it is not a requirement many renderes often derive from a component (such
	 * as a label) and return "this". Notice that a null value for the value
	 * argument might be sent when refreshing the theme of the list.
	 * 
	 * @param list
	 *            the list component
	 * @param model
	 ...
	 * renderer and is animated to provide smooth scrolling. When the selection
	 * moves, this component is drawn above/bellow the list items - it is
	 * recommended to give this component some level of transparency (see above
	 * code example). This method is optional an implementation can choose to
	 * return null.
	 * 
	 * @param list
	 *            the parent list

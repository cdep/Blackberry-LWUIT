 ...
 * list while pulling out some of the drawbacks of both. This container uses the
 * model/renderer approach for populating itself, adding/removing entries will
 * probably break it. It still provides most of the large size advantages a list
 * offers since the components within it are very simple and don't contain any
 * actual state other than layout information. The big advantage with this class
 * is the ability to leverage elaborate LWUIT layouts such as Grid, Table &amp;
 * flow layout to provide other ways of rendering the content of a list model.
 * 
 * @author Shai Almog
 */
public class ContainerList extends Container {
	 ...
	 * 
	 * @param model
	 *            a model class that is mapped into the internal components
	 */

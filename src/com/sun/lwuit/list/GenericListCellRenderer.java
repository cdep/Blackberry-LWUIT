...
/**
 * The generic list cell renderer can display containers or arbitrary LWUIT
 * components as items in a list. It generally relies on the source data being
 * either a hashtable or a list of Strings. It extracts values from the
 * hashtable using the component name as an indication to the hashtable key
 * lookup. This renderer supports label tickering, check boxes/radio buttons
 * etc. seamlessly. Please notice that you must use at least two distinguished
 * instances of the component to render, reusing the same instance WILL NOT
 * WORK. Also the renderer instance cannot be reused for multiple lists, each
 * list will need a new instance of this renderer!
 * 
 * @author Shai Almog
 */
public class GenericListCellRenderer implements ListCellRenderer, CellRenderer {
	...
	/**
	 * If this flag exists in a hashtable of data the renderer will
	 * enable/disable the entries, the flag assumes either Boolean.TRUE or
	 * Boolean.FALSE. Notice that just setting it to false when necessary will
	 * not work, when its used it must be applied to all entries otherwise the
	 * reuse of the renderer component will break this feature.
	 */
	public static final String ENABLED = "$$ENABLED$$";

	/**
	 * Put this flag as a hashtable key to indicate that a checkbox entry
	 * rendered by this renderer should act as a "select all" entry and toggle
	 * all other entries. The value for this entry is ignored
	 */
	...
	/**
	 * Constructs a generic renderer with the given selected/unselected
	 * components
	 * 
	 * @param selected
	 ...
	 * query the renderer to "see" whether a button within the component was
	 * "touched" by the user on a touch screen device. This method will reset
	 * the value to null after returning a none-null value!
	 * 
	...
	/**
	 * Constructs a generic renderer with the given selected/unselected
	 * components for odd/even values allowing a "pinstripe" effect
	 * 
	 * @param odd
	 *            indicates the selected value for the renderer
	 * @param oddUnselected
			...
			if (value instanceof Hashtable) {
				...
				for (int iter = 0; iter < entries.length; iter++) {
					...
					if (currentName.equals("$number")) {
						...
						// a selected entry might differ in its value to allow
						// for
	 ...
	 * 
	 * @param cmp
	 *            one of the components that is or is a part of the renderer
	 * @param value
	 *            the value to install into the component
	 */
	protected void setComponentValue(Component cmp, Object value) {
		// fixed components shouldn't be modified by the renderer, this allows
		// for
		// hardcoded properties in the renderer. We still want them to go
		// through the
	...
	/**
	 * In fisheye rendering mode the renderer maintains selected component
	 * drawing
	 * 
	 * @return the fisheye
	 */

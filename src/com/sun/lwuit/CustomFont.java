...
/**
 * Implements a bitmap font that uses an image and sets of offsets to draw a
 * font with a given character set.
 * 
 * @author Shai Almog
 */
class CustomFont extends Font {
	...
	/**
	 * The width of the character when drawing... this should not be confused
	 * with the number of cutOffset[o + 1] - cutOffset[o]. They are completely
	 * different since a character can be "wider" and "seep" into the next
	 * region. This is especially true with italic characters all of which
	 * "lean" outside of their bounds.
	 */
	int[] charWidth;
	...
	void drawChar(Graphics g, char character, int x, int y) {
		...
		if (i > -1) {
			...
			// g.drawRegion(cache, cutOffsets[i], 0, charWidth[i], imageHeight,
			// x, y);
		}
	 ...
	 *            the chars to draw
	 * @param offset
	 *            the offset to draw the chars
	 * @param length
	 *            the length of chars
	 * @param x
	 *            the x coordinate to draw the chars
	 * @param y
	 *            the y coordinate to draw the chars
	 */

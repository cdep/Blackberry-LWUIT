...
public class ConnectionRequest implements IOProgressListener {
			...
			if (responseCode != 200) {
				...
				if (followRedirects && (responseCode == 301 || responseCode == 302
					...
					if (!(uri.startsWith("http://") || uri.startsWith("https://"))) {
						// relative URI's in the location header are illegal but
	...
	private Cookie parseCookieHeader(String h) {
		...
		if (index != -1) {
			...
			if (url.indexOf(domain) < 0) { // if
											// (!hc.getHost().endsWith(domain))
											// {
	 ...
	 * invoked current network operations should be stoped as soon as possible
	 * for this class.
	 * 
	 ...
	 * overriden by subclasses to add additional tests. It is usefull to do

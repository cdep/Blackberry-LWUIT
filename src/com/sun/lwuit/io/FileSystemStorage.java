...
/**
 * Unlike networking, the file system storage mostly tries to emulate
 * java.io.File with some simplifications for mobile devices
 * 
 * @author Shai Almog
 */
public class FileSystemStorage {
	...
	/**
	 * Represents the type for the get root type method, this type generally
	 * represents an SD card although due to variability in phone standards an
	 * SD card might be detected incorrectly. E.g. newer Nokia devices such as
	 * N97 have a large storage area that is marked as "E:" but is really
	 * internal storage. If an SD card isn't physically in the phone the "F:"
	 * won't be returned and it will be impossible to detect that "E:" is not
	 * the actual SD card.
	 */
	public static final int ROOT_TYPE_SDCARD = 2;

	/**
	 * Returned for different types of root for which there is no specific
	 ...
	 * method will activate it. Regardless it will retry deleting (with delay)
	 * several times to allow streams time to close.
	 * 
	 * @param file
	 *            file to delete
	 * @param retryCount

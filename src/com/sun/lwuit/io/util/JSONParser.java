 ...
 * Hashtable object mapping the object fields to their values. If the value is a
 * nester object a nested hashtable is returned.
 * 
 * @author Shai Almog
 */
public class JSONParser implements JSONParseCallback {
	...
	/**
	 * Static method! Parses the given input stream and fires the data into the
	 * given callback.
	 * 
	 * @param i
	 *            the reader
	...
	public static void parse(Reader i, JSONParseCallback callback) throws IOException {
		...
		try {
			while (callback.isAlive()) {
				...
				if (quoteMode) {
					...
					case 't':
						...
						continue;
					case 'f':
						// this can either be the start of "false" or the end of
						// a
						// fraction number...
			...
			err.printStackTrace();
			/*
			 * System.out.println(); int current = i.read(); while(current >= 0)
			 * { System.out.print((char)current); current = i.read(); }
			 */
			i.close();
		}
	}

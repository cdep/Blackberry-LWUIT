...
/**
 * Based on the buffered input stream from the JDK with some minor tweaks to
 * allow external classes to monitor stream status and progress.
 */
public class BufferedInputStream extends InputStream {
	...
	public void close() throws IOException {
		...
		while ((buffer = buf) != null) {
			if (buf == buffer) { // bufUpdater.compareAndSet(this, buffer,
									// null)) {
				buf = null;
				InputStream input = in;
				in = null;
	...
	/**
	 * Allows setting a yield duration for this stream which is useful for
	 * background operations to release CPU
	 * 
	 * @return the yield
	 */
	public int getYield() {

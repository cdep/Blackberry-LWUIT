 ...
 * encoded image is created and installs itself into the given component. For
 * the case of a Label this is seamless, in case of a List renderer the model or
 * the renderer must register itself as a listener and update the data when the
 * response arrives.
 * 
 * @author Shai Almog
 */
public class ImageDownloadService extends ConnectionRequest {
	...
	/**
	 * Accepts the url to bind to the list renderer, on completion the action
	 * listener will be invoked with the image so a list can be updated with the
	 * data
	 * 
	 * @param url
	 *            the image URL
	...
	/**
	 * Constructs an image request that will automatically populate the given
	 * list when the response arrives. This assumes the GenericListCellRenderer
	...
	/**
	 * Accepts the url to bind to the label, on completion the label will be
	 * updated and revalidated with the new image.
	 * 
	 * @param url
	 *            the image URL
	 * @param parentLabel
	...
	/**
	 * Constructs an image request that will automatically populate the given
	 * list when the response arrives, it will cache the file locally as a file
	 * in the file storage. This assumes the GenericListCellRenderer style of
	...
	/**
	 * Constructs an image request that will automatically populate the given
	 * Label when the response arrives, it will cache the file locally to the
	 * Storage
	 * 
	 * @param url
	 *            the image URL
	 * @param l
	 *            the Label that should be updated when the data arrives to just
	 *            use storage and the url as the key
	 * @param cacheId
	 *            a unique identifier to be used to store the image into storage
	 * @param toScale
	...
	/**
	 * Constructs an image request that will automatically populate the given
	 * Label when the response arrives, it will cache the file locally.
	 * 
	 * @param url
	 *            the image URL
	 * @param callback
	...
	protected void readResponse(InputStream input) throws IOException {
		...
		if (parentLabel != null) {
			...
			if (parentLabel.getComponentForm() != null) {
				...
				// this might not be necessary if the label already had an
				// identically

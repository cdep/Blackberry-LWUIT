 ...
 * content from the RSS chanel. When clicking an article it displays the HTML
 * content of said article in a new Form.
 * 
 * @author Shai Almog
 */
public class RSSReader extends List {
	...
	/**
	 * Sets the form/container to which the RSS will navigate when clicking a
	 * RSS entry
	 */
	...
	/**
	 * Send the request to the server, will only work once. This is called
	 * implicitly when the list is initialized
	 */
	...
	private void updateComponentValues(Container root, Hashtable h) {
		...
		for (int iter = 0; iter < c; iter++) {
			...
			// the comparison assumes that other container subclases are really
			// just
			// custom components e.g. tree, table and HTMLComponent as well as
			// third party
			// subclasses

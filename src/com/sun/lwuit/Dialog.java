 ...
 * rather than x, y, width and height values!
 * <p>
 * To style the dialog you would usually want to style the content pane rather
...
public class Dialog extends Form {
	 ...
	 * @deprecated this method will be removed and the functionality will be
	 *             removed in the next version of LWUIT
	 * @param aDialogTitleCompatibilityMode
	 *            the dialogTitleCompatibilityMode to set
	 */
	...
	/**
	 * Indicates whether LWUIT should try to automatically adjust a showing
	 ...
	 *            glued onto the content pane
	 * @param modal
	 *            indicates the dialog should be modal set to false for modeless
	 *            dialog which is useful for some use cases
	 * @return the last command pressed by the user if such a command exists
	 */
	 ...
	 * @param cancelText
	 *            optionally null for a text to appear in the cancel command for
	...
	protected void autoAdjust(int w, int h) {
		if (autoAdjustDialogSize) {
			...
			// if the dialog is packed we can scale it far more accurately based
			// on intention
			if (position != null) {
				int menuHeight = 0;
				...
				int oldH = getHeight();
				if (oldW != w || oldH != h) {
					// try to preserve the old size of the dialog if we still
					// have room for it...
	 ...
	 *            canceling the dialog
	 * @param timeout
	 *            a timeout after which null would be returned if timeout is 0
	 *            inifinite time is used
	 * @return true if the ok command was pressed or if cancelText is null.
	 *         False otherwise.
	 */
	...
	private void showImpl(boolean reverse) {
		// this behavior allows a use case where dialogs of various sizes are
		// layered
	...
	void showModal(int top, int bottom, int left, int right, boolean includeTitle, boolean modal, boolean reverse) {
		...
		// hide the title if no text is there to allow the styles of the dialog
		// title to disappear
	 ...
	 * 
	 * @param c
	 *            the context component which is used to position the dialog and
	...
	public Command showPopupDialog(Component c) {
		...
		// hide the title if no text is there to allow the styles of the dialog
		// title to disappear, we need this code here since otherwise the
		// preferred size logic of the dialog won't work with large title
		// borders
		if ((!dialogTitleCompatibilityMode) && dialogTitle != null && UIManager.getInstance().isThemeConstant("hideEmptyTitleBool", false)) {
			getTitleComponent().setVisible(getTitle().length() > 0);
		}

		// allows a text area to recalculate its preferred size if embedded
		// within a dialog
		revalidate();
		...
		if (showPortrait) {
			if (width < availableWidth) {
				...
				if (idealX > 0) {
					// if the idealX is too far to the right just align to the
					// right
	 ...
	 * 
	 * @param position
	 *            one of the values from the BorderLayout class e.g.
	...
	public void dispose() {
		...
		// the dispose parent method might send us back to the form while the
		// command
		// within the dialog might be directing us to another form causing a
		// "blip"
		if (!menu) {
			super.dispose();
		}
	...
	protected void actionCommand(Command cmd) {
		// this is important... In a case of nested dialogs based on
		// commands/events a command might be
		// blocked by a different dialog, so when that dialog is disposed (as a
		// result of a command) going
		// back to this dialog will block that command from proceeding and it
		// can be fired again later
		// E.g.:
		...
		// Cancel command event firing is blocked since the dialog B dispose
		// method is now blocking on dialog A show()...
		// When dialog A is disposed using the OK command the OK command is sent
		// correctly and causes dispose
		// EDT is released which also releases the Cancel for dialog B to keep
		// processing...
		// Cancel for dialog B proceeds in the event chain reaching this
		// method....
	 ...
	 * 
	 * @param a
	 *            true to indicate that LWUIT should make a "best effort" to
	 *            resize the dialog
	 */

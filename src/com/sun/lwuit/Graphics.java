 ...
 * achieve portability between MIDP devices and CDC devices. This abstaction
 * simplifies and unifies the Graphics implementations of various platforms.
 * 
 * <p>
 * A graphics instance should never be created by the developer and is always
 * accessed using either a paint callback or a mutable image. There is no
 * supported way to create this object directly.
 */
public final class Graphics {
	 ...
	 * 
	 * @param g
	 *            a given javax.microedition.lcdui.Graphics
	 */
	...
	/**
	 * Clips the given rectangle by intersecting with the current clipping
	 * region, this method can thus only shrink the clipping region and never
	 * increase it.
	 * 
	 * @param x
	...
	/**
	 * Makes the current color slightly lighter, this is useful for many visual
	 * effects
	 * 
	 * @param factor
	 *            the degree of lightening a color per channel a number from 1
	 *            to 255
	 */
	...
	/**
	 * Makes the current color slightly darker, this is useful for many visual
	 * effects
	 * 
	 * @param factor
	 *            the degree of lightening a color per channel a number from 1
	 ...
	 *            Text decoration bitmask (See Style's TEXT_DECORATION_*
	 *            constants)
	 */
	public void drawString(String str, int x, int y, int textDecoration) {
		// this if has only the minor effect of providing a slighly faster
		// execution path
		if (textDecoration != 0) {
	...
	/**
	 * Draws the image so its top left coordinate corresponds to x/y and scales
	 * it to width/height
	 * 
	 * @param img
	 *            the specified image to be drawn. This method does nothing if
	 ...
	 * Draws a radial gradient in the given coordinates with the given colors,
	 * doesn't take alpha into consideration when drawing the gradient. Notice
	 * that this method differs from fillRadialGradient since it draws a sqare
	 * gradient at all times and can thus be cached Notice that a radial
	 * gradient will result in a circular shape, to create a square use fillRect
	 * or draw a larger shape and clip to the appropriate size.
	 * 
	 * @param startColor
	 *            the starting RGB color
	 * @param endColor
	...
	/**
	 * Indicates whether invoking set/getAlpha would have an effect on all
	 * further rendering from this graphics object.
	 * 
	 * @return false if setAlpha has no effect true if it applies to everything
	 *         some effect
	 */
	...
	/**
	 * Sets alpha as a value between 0-255 (0 - 0xff) where 255 is completely
	...
	/**
	 * Returns true if anti-aliasing for standard rendering operations is
	 * supported, notice that text anti-aliasing is a separate attribute.
	 * 
	...
	/**
	 * Returns true if anti-aliasing for text is supported, notice that text
	 * anti-aliasing is a separate attribute from standard anti-alisaing.
	 * 
	...
	/**
	 * Returns true if anti-aliasing for standard rendering operations is turned
	 * on.
	 * 
	...
	/**
	 * Set whether anti-aliasing for text is active, notice that text
	 * anti-aliasing is a separate attribute from standard anti-alisaing.
	 * 
	 * @param a
	...
	/**
	 * Indicates whether anti-aliasing for text is active, notice that text
	 * anti-aliasing is a separate attribute from standard anti-alisaing.
	 * 
	 * @return true if text anti aliasing is supported
	 */
	 ...
	 * the native graphics LWUIT shouldn't be used! The native graphics is
	 * unclipped and untranslated by default and its the responsibility of the
	 * caller to clip/translate appropriately.
	 * <p>
	 * When finished with the native graphics it is essential to <b>invoke

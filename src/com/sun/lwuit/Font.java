 ...
 * library to use more elaborate fonts unsupported by a specific device. This
 * abstraction also supports bitmap fonts using an Ant task (more details about
 * the unifier are explained in the javadoc overview document).
 * <p>
 * A bitmap font can be created manually but that is tedious, normally you would
 * use the Ant task as illustrated bellow to produce a resource file containing
 ...
 * <ul>
 * <li>name - name for the font to load from the resource file (optional:
 * defaults to logical name or file name).
 * <li>charset - defaults to the English alphabet, numbers and common signs.
 * Should contain a list of all characters that should be supported by a font.
 * E.g. if a font would always be used for uppercase letters then it would save
 * space to define the charset as: {@code "ABCDEFGHIJKLMNOPQRSTUVWXYZ" }
 ...
 * <li>trueType - defaults to true, relevant only when src is used. If set to
 * false type 1 fonts are assumed.
 * <li>antiAliasing - defaults to true otherwise fonts will be aliased
 * <li>logicalName - logical name of the font as specified by java.awt.Font in
 * Java SE: {@code Dialog, DialogInput, Monospaced, Serif, or SansSerif }
 * </ul>
 */
public class Font {
	/**
	...
	/**
	 * Bitmap fonts are cached this method allows us to flush the cache thus
	 * allows us to reload a font
	 */
	...
	/**
	 * Returns true if the underlying platform supports loading truetype fonts
	 * from a file stream.
	 * 
	 * @return true if the underlying platform supports loading truetype fonts
	 *         from a file stream
	 */
	...
	/**
	 * Creates a true type font from the given stream if the underlying platform
	 * supports truetype font loading.
	 * 
	 * @param stream
	...
	/**
	 * Creates a new font instance based on the platform specific string name of
	 * the font. This method isn't supported on some platforms.
	 * 
	 * @param lookup
	 *            a set of platform specific names delimited by commas, the
	 *            first succefully loaded font will be used
	 ...
	 * <p>
	 * Notice that this method only works in one way, contrast cannot be
	 * decreased properly in a font and it should be cleared and reloaed with a
	 ...
	 * @param bitmap
	 *            a transparency map in red and black that indicates the
	 *            characters
	 * @param cutOffsets
	 *            character offsets matching the bitmap pixels and characters in
	 *            the font
	 * @param charWidth
	 *            The width of the character when drawing... this should not be
	 *            confused with the number of cutOffset[o + 1] - cutOffset[o].
	 *            They are completely different since a character can be "wider"
	 *            and "seep" into the next region. This is especially true with
	 *            italic characters all of which "lean" outside of their bounds.
	 * @param charsets
	...
	public int stringWidth(String str) {
		// this happens often for icons without text and can cost a bit more in
		// the native platform
	 ...
	 * @param y
	 *            the y coordinate to draw the char
	 */
	 ...
	 * @param offset
	 *            the offset in the given char array
	 * @param length
	 *            the number of chars to draw
	 * @param x
	 *            the x coordinate to draw the char
	 * @param y
	 *            the y coordinate to draw the char
	...
	/**
	 * Indicates whether bitmap fonts should be enabled by default when loading
	 * or the fallback system font should be used instead. This allows easy
	 * toggling of font loading.
	 * 
	 * @param enabled
	 *            true to enable bitmap font loading (if they exist in the
	 *            resource)
	 */
	...
	/**
	 * Indicates whether bitmap fonts should be enabled when loading or the
	 * fallback system font should be used instead. This allows easy toggling of
	 * font loading.
	 * 
	 ...
	 * 
	 * @return platform specific font object for use by implementation classes
	 *         or native code
	 */

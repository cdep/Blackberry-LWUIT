 ...
 * properly on the EDT regardless of their source. This class handles listener
...
public class EventDispatcher {
	...
	/**
	 * When set to true, style events will be dispatched even from non-EDT
	 * threads. When set to false, when in non-EDT threads, style events will
	 * not be dispatched at all (And developer has to make sure changes will be
	 * reflected by calling revalidate after all the changes)
	 * 
	 * Default is false. Setting this to true results in a performance penalty,
	 * and it is better instead to simply aggregate events performed on non-EDT
	 * threads and when all are over - call revalidate on the relevant
	 * container.
	 * 
	 * @param fire
	...
	class CallbackClass implements Runnable {
		...
		/**
		 * Do not invoke this method it handles the dispatching internally and
		 * serves as an implementation detail
		 */
	...
	/**
	 * Indicates whether this dispatcher blocks when firing events or not,
	 * normally a dispatcher uses callSeriallyAndWait() to be 100% synchronos
	 * with event delivery however this method is very slow. By setting blocking
	 * to false the callSerially method is used which allows much faster
	 * execution for IO heavy operations.
	 * 

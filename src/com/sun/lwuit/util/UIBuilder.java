...
public class UIBuilder {
	...
	/**
	 * This method allows the UIBuilder to package a smaller portion of LWUIT
	 * into the JAR and add support for additional 3rd party components to the
	 * GUI builder. Components must be registered using their UIID name, by
	 * default all the content of com.sun.lwuit is registered however
	 * subpackages and 3rd party components are not. Registeration is essential
	 * for obfuscation to work properly!
	 * 
	 * @param name
	...
	private void readCommand(DataInputStream in, Component c, Container parent, Resources res, boolean legacy) throws IOException {
		...
		// prevent duplicate action handling only in the case of a component
		// form
		// the embeded component doesn't have a global command listener since it
		// has
		// no menu
	...
	/**
	 * Invoked after the components were created to allow properties that
	 * require the entire tree to exist to update the component. This is useful
	 * for properties that point at other components.
	 */
	...
	/**
	 * This method can be overriden to create custom components in a custom way,
	 * the component type is a shorthand for the component name and not the full
	 * name of the class. By default this method returns null which indicates
	 * LWUIT should try to reolve the component on its own.
	 * 
	 * @param componentType
	 ...
	 * hierarchy will be available at this stage). This is the perfect place to
	 * bind models/renderers etc. to components.
	 * 
	 * @param cmp
	 *            the component
	 */
	 ...
	 * for common LWUIT events but might be an issue with custom components and
	 * custom listener types so this method can be overloaded to add support for
	 * such cases.
	 * 
	 * @param cmp
	...
	private Component createComponent(DataInputStream in, Container parent, Container root, Resources res, Hashtable componentListeners, EmbeddedContainer embedded) throws Exception {
		...
		if (property == PROPERTY_BASE_FORM) {
			...
			if (!ignorBaseForm) {
				...
				// try finding an appropriate empty container if no "fixed"
		...
		while (property != -1) {
			...
			case PROPERTY_EMBED:
				...
				if (embed != null) {
					...
					// this isn't exactly the "right thing" but its the best we
					// can do to make all
					// use cases work
	...
	/**
	 * Creates a command instance. This method is invoked by the loading code
	 * and can be overriden to create a subclass of the Command class.
	 * 
	 * @param commandName
	 ...
	 * @param action
	 *            the action assigned to the command if such an action is
	 *            defined
	 ...
	 * dynamically load a resource file. Normally the navigation feature of the
	 * UIBuilder requires the resource file present in RAM. However, that might
	 * be expensive to maintain. By implementing this method and replacing the
	 * storeResourceFile() with an empty implementation the resource file
	 * storage can be done strictly in RAM.
	 * 
	...
	/**
	 * Allows the navigation code to avoid storing the resource file and lets
	 * the GC remove it from memory when its not in use
	 * 
	 ...
	 * processing occurs. The event can be consumed to prevent further
	 * processing.
	 * 
	 * @param ev
	 ...
	 * created. Notice that this method is only effective before the form was
	 * created and would do nothing for an existing form
	 * 
	 * @param formName
	 ...
	 * listener can be bound and delivers the event data for the given component
	 * seamlessly.
	 * 
	 * @param c
	 ...
	 * with asynchronous command processing. This method is invoked when the UI
	 * indicates that an operation should occur in the background. To finish the
	 * processing of the operation within the EDT one should overide the
	 * postAsyncCommand() method.
	 * 
	 * @param cmd
	 ...
	 * code can be execute to update the UI with the results from the separate
	 * thread.
	 * 
	 * @param cmd
	 *            the command
	...
	/**
	 * <b>Warning:</b> this method is invoked on a separate thread. This method
	 * is invoked when a next form property is defined, this property indicates
	 * a background process for a form of a transitional nature should take
	 * place (e.g. splash screen, IO etc.) after which the next form should be
	 * shown. After this method completes the next form is shown.
	 * 
	 * @param f
	 *            the form for which the background thread was constructed,
	 *            notice that most methods are not threadsafe and one should use
	 *            callSerially* in this method when mutating the form.
	 * @return if false is returned from this method navigation should not
	 *         proceed to that given form
	 */
	 ...
	 * state would be restored using setFormState. The default implementation of
	 * this method restores focus and list selection. You can add arbitrary keys
	 * to the form state, keys starting with a $ sign are reserved for the
	 * UIBuilder base class use.
	 * 
	 * @param f
	...
	/**
	 * Sets the state of the current form to which we are returing as part of
	 * the navigation logic. When a back command is pressed this form state
	 * should be restored, it was obtained via getFormState. The default
	 * implementation of this method restores focus and list selection.
	 * 
	 * @param f
	 *            the form whose state should be preserved
	 * @param state
	 ...
	 *            the component that triggered the back command which
	 *            effectively allows us to find the EmbeddedContainer for a case
	 *            of container navigation. Null can be used if not applicable.
	 */
	...
	private void initBackForm(Form f) {
		...
		if (formNavigationStack != null && formNavigationStack.size() > 0) {
			...
			if (formNavigationStack.size() > 0) {
				...
				// trigger listener creation if this is the only command in the
				// form
	 ...
	 * will run. Notice that container navigation (none-form) doesn't support
	 * the back() method or the form stack. However a command marked as back
	 ...
	 * @return the container thats being shown, notice that you can still
	 *         manipulate some states of the container before it actually
	 *         appears
	 */
		...
		if (sourceComponent != null) {
			...
			if (currentForm.getClientProperty(EMBEDDED_FORM_FLAG) != null) {
				...
				while (!(destContainer instanceof EmbeddedContainer || destContainer instanceof Form)) {
					// race condition, container was already removed by someone
					// else
				...
				if (destContainer instanceof EmbeddedContainer) {
					...
					// This seems to be no longer necessary now that we have the
					// replaceAndWait version that drops events
					// block the user from the ability to press the button twice
					// by mistake
					...
					if (formNavigationStack != null && !isBack && allowBackTo(commandAction) && !isSameBackDestination((Container) fromCmp, cnt)) {
						// trigger listener creation if this is the only command
						// in the form
	...
	private void showForm(Form f, Command sourceCommand, Component sourceComponent) {
		...
		if (sourceCommand != null && Display.getInstance().getCurrent().getBackCommand() == sourceCommand) {
				...
				if (currentForm != null) {
					...
					if (nextForm == null) {
						...
						if (allowBackTo(commandAction) && f.getBackCommand() == null) {
							...
							// trigger listener creation if this is the only
							// command in the form
							getFormListenerInstance(f, null);
			...
			if (f instanceof Dialog) {
				beforeShow(f);
				if (sourceComponent != null) {
					// we are cheating with the post show here since we are
					// using a modal
					// dialog to prevent the "double clicking button" problem by
					// using
					// a modal dialog
	 ...
	 * @param formName
	 *            the name of the form
	 * @return true to autogenerate and add a back command to the destination
	 *         form
	 */
	...
	/**
	 * When navigating from one form/container to another we sometimes might not
	 * want the back command to return to the previous container/form but rather
	 * to the one before source. A good example would be a "refresh" command or
	 * a toggle button that changes the form state.
	 * 
	 * @param source
	 *            the form or container we are leaving
	 * @param destination
	 *            the container or form we are navigating to
	 * @return false if we want a standard back button to source, true if we
	 *         want to use the same back button as the one in source
	 */
	...
	/**
	 * This method allows binding an action that should occur before leaving the
	 * given form, e.g. memory cleanup
	 * 
	 * @param f
	 *            the form being left
	 */
	...
	/**
	 * This method allows binding an action that should occur before showing the
	 * given form
	 * 
	 * @param f
	 ...
	 * root object e.g. a case where a created form needs data fetched for it.
	 * 
	 * @param rootName
	...
	/**
	 * Indicates that the UIBuilder should cache resources in memory and never
	 * release them. This is useful with small resource files or high RAM
	 * devices since it saves the cost of constantly fetching the res file from
	 * the jar whenever moving between forms. This can be toggled in the
	 * properties (e.g. jad) using the flag: cacheResFile (true/false) which
	 * defaults to false.
	 * 
		...
		public void run() {
			if (currentAction != null) {
				if (Display.getInstance().isEdt()) {
					...
					// wait for the destination form to appear before moving
					// back into the LWUIT thread
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (cmd == null) {
				...
				if (evt.getSource() instanceof Form) {
					// prevent a dialog from triggerring the background
					// processing again

...
/**
 * Loads resources from the binary resource file generated during the build
 * process or via the LWUIT Designer. A resource is loaded entirely into memory
 * since random file access is not supported in Java ME, any other approach
 * would be inefficient. This means that memory must be made available to
 * accommodate the resource file.
 * 
 * @author Shai Almog
 */
public class Resources {
	...
	/**
	 * Temporary member for compatibility with older versions, in future
	 * versions this will superceed the MAGIC_THEME property
	 */
	...
	/**
	 * These variables are useful for caching the last loaded resource which
	 * might happen frequently in the UI builder when moving between
	...
	/**
	 * Hashtable containing the mapping between element types and their names in
	 * the resource hashtable
	 */
	private Hashtable resourceTypes = new Hashtable();

	/**
	 * A cache within the resource allowing us to preserve some resources in
	 * memory so they can be utilized by a theme when it is loaded
	 */
	...
	/**
	 * Returns the version number for this resource file. This value relates to
	 * the value from the header defined by the resource file specification. 0
	 * is returned for legacy versions of the resource file format.
	 * 
	...
	/**
	 * Returns the minor version number for this resource file This value
	 * relates to the value from the header defined by the resource file
	 * specification.
	 * 
	 ...
	 * @throws NullPointerException
	 *             if the resource doesn't exist
	 * @deprecated animations are no longer distinguished from images in the
	 ...
	 * 
	 * @param resource
	 *            a local reference to a resource using the syntax of
	...
	public static Resources open(String resource) throws IOException {
		try {
			...
			// intercept exceptions since user code might not deal well with
			// runtime exceptions
			err.printStackTrace();
	 ...
	 * @return newly created input stream that allows reading the data of the
	 *         resource
	 */
	 ...
	 * 
	 * @param id
	 *            name of the ui resource
	 * @return newly created input stream that allows reading the ui of the
	 *         resource
	 */
	...
	/**
	 * Returns a hashmap containing localized String key/value pairs for the
	 * given locale name
	 * 
	 * @param id
		...
		if (h != null && h.containsKey("uninitialized")) {
			...
			while (e.hasMoreElements()) {
				...
				if (key.endsWith("font") || (key.endsWith("Image") && !key.endsWith("scaledImage"))) {
					...
					// the resource was not already loaded when we loaded the
					// theme
					// it must be loaded now so we can resolve the temporary
					// name
					if (value instanceof String) {
						Object o;

						// we need to trigger multi image logic in the resource
						// editor
				...
				// if this is a border we might need to do additional work for
				// older versions
				// of LWUIT and for the case of an image border where the images
				// might not have
		...
		if (majorVersion == 0 && minorVersion == 0) {
			...
			case 0xF5: {
				...
				if (Image.isSVGSupported()) {
					...
					// read the base url, the animated property and screen
					// ratios to skip them as well...
					input.readUTF();
					input.readBoolean();
					input.readFloat();
					input.readFloat();
	...
	Font loadBitmapFont(DataInputStream input, String id, com.sun.lwuit.Font font) throws IOException {
		...
		if (font == null) {
			if (Font.isBitmapFontEnabled()) {
				Font old = Font.getBitmapFont(id);
				if (old != null) {
					// Returning bitmap font from cache, to prevent collision
					// with an
	...
	Hashtable loadTheme(String id, boolean newerVersion) throws IOException {
		...
		// marks the theme as uninitialized so we can finish "wiring" cached
		// resources
		...
		for (int iter = 0; iter < size; iter++) {
			...
			// if this is a padding or margin then we will have the 4 values as
			// bytes
			...
			if (key.endsWith("ackground")) {
				...
				case 0xF3:
					// the image name coupled with the type and with alignment
					// information
	...
	private Image createPackedImage8() throws IOException {
		...
		// 0 means the last bit overflowed, there is no sense for 0 sized
		// palette
		if (size == 0) {
			size = 256;
		}

...
public class Image {
	...
	/**
	 * Subclasses may use this and point to an underlying native image which
	 * might be null for a case of an image that doesn't use native drawing
	 * 
	 * @param image
	 *            native image object passed to the LWUIT implementation
	 */
	 ...
	 * considerations e.g. an encoded image might choose to cache itself in RAM.
	 ...
	 * this method will throw an exception if SVG is unsupported.
	 * 
	 * @param baseURL
	...
	/**
	 * Creates a mask from the given image, a mask can be used to apply an
	 * arbitrary alpha channel to any image. A mask is derived from the blue
	 * channel (LSB) of the given image. The generated mask can be used with the
	 * apply mask method.
	 * 
	...
	/**
	 * Applies the given alpha mask onto this image and returns the resulting
	 * image see the createMask method for indication on how to convert an image
	 * into an alpha mask.
	 * 
	 * @param mask
	 ...
	 * @param processAlpha
	 *            whether alpha should be processed as well as part of the
	 *            cutting
	...
	/**
	 * Creates an indexed image with byte data this method may return a native
	 * indexed image rather than an instance of the IndexedImage class
	 * 
	 * @param width
	 *            image width
	 * @param height
	...
	/**
	 * Creates a new image instance with the alpha channel of opaque pixels
	 * within the image using the new alpha value. Transparent (alpha == 0)
	 * pixels remain transparent. Semi translucent pixels will be multiplied by
	 * the ratio difference and their translucency reduced appropriately.
	 * 
	 * @param alpha
	 *            New value for the entire alpha channel
	 * @return Translucent/Opaque image based on the alpha value and the pixels
	...
	/**
	 * Callback invoked internally by LWUIT to draw the image/frame onto the
	 * display. Image subclasses can override this method to perform drawing of
	 * custom image types.
	 * 
	 * @param g
	 *            the graphics object
	...
	/**
	 * Callback invoked internally by LWUIT to draw a portion of the image onto
	 * the display. Image subclasses can override this method to perform drawing
	 * of custom image types.
	 * 
	 * @param g
	 *            the graphics object
	 * @param nativeGraphics
	 ...
	 * cached instance if possible. Note that cached instances may be garbage
	 * collected.
	 * 
	...
	/**
	 * Returns the platform specific image implementation,
	 * <strong>warning</strong> the implementation class can change between
	 * revisions of LWUIT and platforms.
	 * 

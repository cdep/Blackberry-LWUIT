...
/**
 * Date widget for selecting a date/time value. To localize strings for month
 * names use the values "Calendar.Month" using the 3 first characters of the
 * month name in the resource localization e.g. "Calendar.Jan", "Calendar.Feb"
 * etc... To localize strings for day names use the values "Calendar.Day" in the
 * resource localization e.g. "Calendar.Sunday", "Calendar.Monday" etc...
 * 
...
public class Calendar extends Container {
	 ...
	 * alternative date
	 * 
	 * @param changesSelectedDateEnabled
	 *            if true pressing on a date will cause the selected date to be
	 *            changed to the pressed one
	 */
	...
	class MonthView extends Container implements ActionListener {
		...
		private void setMonth(int year, int month) {
			...
			// if this is past the last day of the month (e.g. going from
			// January 31st
			// to Febuary) we need to decrement the day until the month is
			// correct

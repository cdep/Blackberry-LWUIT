...
/**
 * Represents a vendor extension mechanizm for LWUIT, <b>WARNING: this class is
 * for internal use only and is subject to change in future API revisions</b>.
 * To replace the way in which LWUIT performs its task this class can be
 * extended and its functionality replaced or enhanced.
 * <p>
 * It is the responsibility of the implementation class to grab and fire all
 * events to the Display specifically for key, pointer events and screen
 * resolution.
 * 
 * @author Shai Almog
 */
public abstract class LWUITImplementation {
	...
	/**
	 * Useful since the content of a single element touch event is often
	 * recycled and always arrives on 1 thread. Even on multi-tocuh devices a
	 * single coordinate touch event should be very efficient
	 */
	...
	/**
	 * Some implementations might need to perform initializations of the EDT
	 * thread
	 */
	public void initEDT() {
	}
	...
	/**
	 * Invoked when a dialog is shown, this method allows a dialog to play a
	 * sound
	 * 
	 * @param type
	 *            the type of the dialog matching the dialog classes defined
	 *            types
	 */
	...
	/**
	 * Returns the width dimention of the display controlled by this
	 * implementation
	 * 
	 * @return the width
	 */
	...
	/**
	 * Returns the height dimention of the display controlled by this
	 * implementation
	 * 
	 * @return the height
	 */
	 ...
	 * @return false by default, true if the exception shouldn't be handled
	 *         further by the EDT
	 */
	...
	/**
	 * Invoked if LWUIT needs to dispose the native text editing but would like
	 * the editor to store its state.
	 */
	...
	/**
	 * This method allows customizing/creating a graphics context per component
	 * which is useful for some elaborate implementations of LWUIT. This method
	 * is only relevant for elborate components such as container which render
	 * their own components rather than invoke repaint()
	 * 
	 * @param cmp
	...
	/**
	 * Allows for painting an overlay on top of the implementation for notices
	 * during testing etc.
	 * 
	 * @param g
	 *            graphics context on which to draw the overlay
	 */
	 ...
	 * 
	 * @param enter
	 *            true before the edt sleeps and false when exits from the idle
	 *            state
	 */
	...
	/**
	 * Flush the currently painted drawing onto the screen if using a double
	 * buffer
	 * 
	 * @param x
	...
	/**
	 * Installs the display lock allowing implementors to synchronize against
	 * the Display mutex, this method is invoked internally and should not be
	 * used.
	 * 
	 * @param lock
	...
	/**
	 * Returns a lock object which can be synchrnoized against, this lock is
	 * used by the EDT.
	 * 
	...
	/**
	 * Removes an entry from the paint queue if it exists, this is important for
	 * cases in which a component was repainted and immediately removed from its
	 * parent container afterwards. This happens sometimes in cases where a
	 * replace() operation changes a component to a new component that has an
	 * animation() the animation might have triggered a repaint before the
	 * removeComponent method was invoked
	 * 
	 * @param cmp
	 *            the component to
	 */
	 ...
	 * 
	 * @param cmp
	 *            component or animation to push into the paint queue
	 */
	...
	/**
	 * Extracts RGB data from the given native image and places it in the given
	 * array
	 * 
	 * @param nativeImage
	 *            native platform image object
	 * @param arr
	 *            int array to store RGB data
	 * @param offset
	 *            position within the array to start
	 * @param x
	 *            x position within the image
	 * @param y
	 *            y position within the image
	 * @param width
	 *            width to extract
	 * @param height
	 *            height to extract
	 ...
	 * 
	 * @param rgb
	 *            ARGB data from which to create a platform image
	 * @param width
	 *            width for the resulting image
	 * @param height
	 ...
	 * 
	 * @param width
	 *            the width of the mutable image
	 * @param height
	 *            the height of the mutable image
	 * @param fillColor
	 *            the ARGB fill color, alpha may be ignored based on the value
	 *            of isAlphaMutableImageSupported
	 ...
	 * 
	 * @return true if mutable images can have an alpha value when initially
	 *         created
	 */
	 ...
	 * @return the set of keycodes which can indicate the softkey, multiple
	 *         keycodes might apply to the same functionality
	 */
	...
	/**
	 * Returns the display game action for the given keyCode if applicable to
	 * match the contrct of LWUIT for the game action behavior
	 * 
	 * @param keyCode
	 *            the device keycode
	...
	/**
	 * This method is used internally to determine the actual current form it
	 * doesn't perform the logic of transitions etc. and shouldn't be invoked by
	 * developers
	 * 
	 * @param f
	 *            the current form
	...
	/**
	 * Callback method allowing the implementation to confirm that it controls
	 * the view just before a new form is installed.
	 */
	public void confirmControlView() {
	}

	/**
	 * Returns the current form, this method is for internal use only and does
	 * not take transitions/menus into consideration
	 * 
	...
	/**
	 * LWUIT can translate all coordinates and never requires a call to
	 * translate this works well for some devices which have hairy issues with
	 * translate. However for some platforms where translate can be leveraged
	 * with affine transforms this can be a problem. These platforms can choose
	 * to translate on their own
	 * 
	 * @return true if the implementation is interested in receiving translate
	...
	/**
	 * Sets the current rgb color while ignoring any potential alpha component
	 * within said color value.
	 * 
	 * @param graphics
	 *            the graphics context
	 * @param RGB
	...
	/**
	 * Returns true if alpha can be applied for all elements globally and
	 * efficiently otherwise alpha should be ignored. Notice that fillRect MUST
	 * always support alpha regardless of the value of this variable!
	 * 
	...
	/**
	 * Indicates whether the underlying implementation allows for anti-aliasing
	 * in regular drawing operations
	 * 
	 * @return false by default
	 */
	...
	/**
	 * Indicates whether the underlying implementation allows for anti-aliased
	 * fonts
	 * 
	 * @return false by default
	 */
	 ...
	 *            the graphics context
	 * @param rect
	 *            rectangle representing the new clipping area
	 */
	...
	/**
	 * Changes the current clipping rectangle to subset the current clipping
	 * with the given clipping.
	 * 
	 * @param graphics
	 *            the graphics context
	 * @param rect
	 ...
	 *            the graphics context
	 * @param img
	 *            the specified native image to be drawn
	 * @param x
	 *            the x coordinate.
	 * @param y
	 *            the y coordinate.
	...
	/**
	 * Indicates if image scaling on the fly is supported by the platform, if
	 * not LWUIT will just scale the images on its own before drawing
	 */
	 ...
	 *            the y coordinate.
	 * @param imageX
	 *            location within the image to draw
	 * @param imageY
	 *            location within the image to draw
	 * @param imageWidth
	 *            size of the location within the image to draw
	 * @param imageHeight
	 *            size of the location within the image to draw
	 */
	 ...
	 *            the y coordinate.
	 * @param degrees
	 *            either 0, 90, 180 or 270 degree rotation for the image drawing
	 */
	public void drawImageRotated(Object graphics, Object img, int x, int y, int degrees) {
	}

	/**
	 * Indicates whether drawImageRotated is supported by the platform for FAST
	 * drawing, if not then its not worth calling the method which will be
	 * unimplemented!
	 * 
	 ...
	 * region. The values are stored in rgbData in a format with 24 bits of RGB
	 * and an eight-bit alpha value (0xAARRGGBB), with the first value stored at
	 * the specified offset. The scanlength specifies the relative offset within
	 * the array between the corresponding pixels of consecutive rows. Any value
	 * for scanlength is acceptable (even negative values) provided that all
	 * resulting references are within the bounds of the rgbData array. The ARGB
	 * data is rasterized horizontally from left to right within each row. The
	 * ARGB values are rendered in the region specified by x, y, width and
	 * height, and the operation is subject to the current clip region and
	 ...
	 * @param processAlpha
	 *            true if rgbData has an alpha channel, false if all pixels are
	 *            fully opaque
	 */
	public abstract void drawRGB(Object graphics, int[] rgbData, int offset, int x, int y, int w, int h, boolean processAlpha);

	/**
	 * Returns the native graphics object on which all rendering operations
	 * occur
	 * 
	...
	/**
	 * Return the width of the given characters in the given native font
	 * instance
	 * 
	 * @param nativeFont
	...
	/**
	 * Return the global default font instance, if font is passed as null this
	 * font should be used
	 * 
	 ...
	 * @param style
	 *            the style of the font. The value is an OR'ed combination of
	 *            STYLE_BOLD, STYLE_ITALIC, and STYLE_UNDERLINED; or the value
	 *            is zero (STYLE_PLAIN).
	 * @param size
	 *            the size of the font, can be one of SIZE_SMALL, SIZE_MEDIUM,
	 *            SIZE_LARGE
	...
	/**
	 * Subclasses should invoke this method, it delegates the event to the
	 * display and into LWUIT.
	 * 
	 * @param keyCode
	...
	protected boolean hasDragStarted(final int x, final int y) {
		...
		if (((float) getDisplayWidth()) / 100.0f * ((float) getDragStartPercentage()) <= Math.abs(dragActivationX - x)) {
			dragActivationCounter = getDragAutoActivationThreshold() + 1;
			return true;
		}

		// have we passed the motion threshold on the Y axis?
		if (((float) getDisplayHeight()) / 100.0f * ((float) getDragStartPercentage()) <= Math.abs(dragActivationY - y)) {
			dragActivationCounter = getDragAutoActivationThreshold() + 1;
			return true;
		}
	...
	/**
	 * This method allows us to manipulate the drag started detection logic. If
	 * the pointer was dragged for more than this percentage of the display size
	 * it is safe to assume that a drag is in progress.
	 * 
	...
	/**
	 * This method allows subclasses to manipulate the drag started detection
	 * logic. If more than this number of drag events were delivered it is safe
	 * to assume a drag has started This number must be bigger than 0!
	 * 
	 * @return number representing a minimum number of motion events to start a
	 *         drag operation
	 */
	...
	protected void pointerReleased(final int[] x, final int[] y) {
		...
		// Samsung devices
		// that send a pointerPressed/Released with widely differing X/Y values
		// but don't send
	 ...
	 * @param relativeX
	 *            indicates the relative position of the gradient within the
	 *            drawing region
	 * @param relativeY
	 *            indicates the relative position of the gradient within the
	 *            drawing region
	 * @param relativeSize
	 *            indicates the relative size of the gradient within the drawing
	 *            region
	 */
	...
	/**
	 * Fills a closed polygon defined by arrays of x and y coordinates. Each
	 ...
	 * KEYBOARD_TYPE_NUMERIC, KEYBOARD_TYPE_QWERTY, KEYBOARD_TYPE_VIRTUAL,
	...
	/**
	 * Indicates whether the device supports native in place editing in which
	 * case lightweight input logic shouldn't be used for input.
	 * 
	 * @return false by default
	 */
	 ...
	 * stages to touch events: click and hover. This is true for devices such as
	 * the storm but can also be true for a PC with a mouse pointer floating on
	 * top.
	 * <p>
	 * A click touch screen will also send pointer hover events to the
	 * underlying software and will only send the standard pointer events on
	 * click.
	 * 
	 * @return false by default
	 */
	 ...
	 *            the media component
	 * @param fullScreen
	 *            indicates whether this is fullscreen or not
	 * @param nativeGraphics
	 *            the native graphics object
	 * @param video
	 *            the native videoo control
	 * @param player
	 *            the native player object
	 * @deprecated replaced by the version that accepts the video component
	 *             instance
	 */
	...
	/**
	 * Rotates the coordinate system around a radian angle using the affine
	 * transform
	 * 
	 * @param angle
	 ...
	 * 
	 * @return true if the method create SVG image would return a valid image
	 *         object from an SVG Input stream
	 */
	 ...
	 * 
	 * @param baseURL
	 *            URL which is used to resolve relative references within the
	 *            SVG file
	 * @param data
	...
	/**
	 * Returns a platform specific DOM object that can be manipulated by the
	 * user to change the SVG Image
	 * 
	 * @param svgImage
	 *            the underlying image object
	 * @return Platform dependent object, when JSR 226 is supported an
	 *         SVGSVGElement might be returned.
	 */
	...
	/**
	 * Callback to allow images animated by the underlying system to change
	 * their state e.g. for SVG or animated gif support. This method returns
	 * true if an animation state has changed requiring a repaint.
	 * 
	 * @param nativeImage
	 *            a native image used within the image object
	 * @param lastFrame
	 *            the time the last frame of animation was shown
	 * @return true if a repaint is required since the image state changed,
	 *         false otherwise
	 */
	...
	/**
	 * Returns a list of the platform names ordered by priority, platform names
	 * are used to choose a font based on platform. Since a platform might
	 * support several layers for choice in narrowing platform font selection
	 * 
	 ...
	 * @throws IOException
	 *             will be thrown in case of an io error
	 */
	...
	/**
	 * Returns true if the system supports dynamically loading truetype fonts
	 * from a stream.
	 * 
	 * @return true if the system supports dynamically loading truetype fonts
	 *         from a stream.
	 */
	...
	/**
	 * Minimizes the current application if minimization is supported by the
	 * platform (may fail). Returns false if minimization failed.
	 * 
	 * @return false if minimization failed true if it succeeded or seems to be
	 *         successful
	 */
	...
	/**
	 * Restore the minimized application if minimization is supported by the
	 * platform
	 */
	...
	/**
	 * Indicates whether the implementation is interested in caching radial
	 * gradients for drawing.
	 * 
	...
	/**
	 * Indicates whether the implementation is interested in caching linear
	 * gradients for drawing.
	 * 
	 ...
	 *            array of locations
	 * @param dragPathTime
	 *            the time difference between each point
	 * @param dragPathOffset
	 *            the offset in the arrays
	...
	/**
	 * Indicates whether LWUIT should consider the bidi RTL algorithm when
	 ...
	 * 
	 * @param activate
	 *            set to true to activate the bidi algorithm, false to disable
	 *            it
	 */
	...
	/**
	 * Converts the given string from logical bidi layout to visual bidi layout
	 * so it can be rendered properly on the screen. This method is only
	 * necessary for devices/platforms that don't have "built in" bidi support
	 * such as Sony Ericsson devices. See <a href=
	 * "http://www.w3.org/International/articles/inline-bidi-markup/#visual"
	 * >this</a> for more on visual vs. logical ordering.
	 * 
	 * 
	 * @param s
	 ...
	 * index isn't necessarily the same when bidi is involved See <a href=
	 * "http://www.w3.org/International/articles/inline-bidi-markup/#visual"
	 * >this</a> for more on visual vs. logical ordering.
	 * 
	 * @param source
	 ...
	 * however some platforms might define unique ways in which to load
	 * resources within the implementation.
	 * 
	 * @param cls
	...
	/**
	 * Animations should return true to allow the native image animation to
	 * update
	 * 
	 * @param nativeImage
	 ...
	 * @return a LWUIT peer component that can be manipulated just like any
	 *         other LWUIT component but would internally encapsulate the given
	 *         native peer
	 */
	...
	/**
	 * Returns the property from the underlying platform deployment or the
	 * default value if no deployment values are supported. This is equivalent
	 * to the getAppProperty from the jad file.
	 * <p>
	...
	/**
	 * Returns one of the density variables appropriate for this device, notice
	 * that density doesn't alwyas correspond to resolution and an
	...
	public int getDeviceDensity() {
		...
		if (isTablet()) {
			// tablets have lower density and allow fitting more details in the
			// screen despite a high resolution
			if (d >= 1440 * 720) {
	...
	/**
	 * Plays a builtin device sound matching the given identifier,
	 * implementations and themes can offer additional identifiers to the ones
	 * that are already built in.
	 * 
	 * @param soundIdentifier
	 *            the sound identifier which can match one of the common
	 *            constants in this class or be a user/implementation defined
	 *            sound
	 */
	...
	/**
	 * Converts a sound object to a form which will be easy for the
	 * implementation to play later on. E.g. a byte array or a file/file name
	 * and return an object that will allow playNativeBuiltinSound() to use
	 * 
	 * @param i
	...
	/**
	 * Installs a replacement sound as the builtin sound responsible for the
	 * given sound identifier (this will override the system sound if such a
	 * sound exists).
	 * 
	 * @param soundIdentifier
	 *            the sound string passed to playBuiltinSound
	 * @param data
	 *            an input stream containing platform specific audio file, its
	 *            usually safe to assume that wav/mp3 would be supported.
	 * @throws IOException
	...
	/**
	 * Performs a clipboard copy operation, if the native clipboard is supported
	 * by the implementation it would be used
	 * 
	 * @param obj
	 *            object to copy, while this can be any arbitrary object it is
	 *            recommended that only Strings or LWUIT image objects be used
	 *            to copy
	 */
	...
	/**
	 * Returns true if the device allows forcing the orientation via code,
	 * feature phones do not allow this although some include a jad property
	 * allowing for this feature
	 * 
	...
	/**
	 * On devices that return true for canForceOrientation() this method can
	 * lock the device orientation either to portrait or landscape mode
	 * 
	 * @param portrait
	...
	/**
	 * An implementation can return true if it supports embedding a native
	 * browser widget
	 * 
	 * @return true if the implementation supports embedding a native browser
	 *         widget
	 */
	...
	/**
	 * If the implementation supports the creation of a browser component it
	 * should be returned in this method
	 * 
	 * @param browserComponent
	 *            instance of the browser component thru which events should be
	 *            fired
	 ...
	 * ways including platform specific settings. When a property isn't
	 * supported by a specific platform it is just ignored.
	 * 
	 * @param browserPeer
	 *            browser instance
	 * @param key
	 *            see the documentation with the LWUIT Implementation for
	 *            further details
	 * @param value
	 *            see the documentation with the LWUIT Implementation for
	 *            further details
	...
	/**
	 * Allows exposing the given object to JavaScript code so the JavaScript
	 * code can invoke methods and access fields on the given object. Notice
	 * that on RIM devices which don't support reflection this object must
	 * implement the propriatery Scriptable interface
	 * http://www.blackberry.com/developers
	 * /docs/5.0.0api/net/rim/device/api/script/Scriptable.html
	 * 
	 * @param browserPeer
	 *            browser instance
	 * @param o
	 *            the object to invoke, notice all public fields and methods
	 *            would be exposed to JavaScript
	 * @param name
	...
	/**
	 * Converts the dips count to pixels, dips are roughly 1mm in length. This
	 * is a very rough estimate and not to be relied upon
	 * 
	 * @param dipCount
	...
	/**
	 * Indicates whether the device is a tablet, notice that this is often a
	 * guess
	 * 

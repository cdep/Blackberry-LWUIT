...
/**
 * The implementation of the blackberry platform delegates the work to the
 * underlying UI application to allow for deep BB integration
 * 
...
public class BlackBerryImplementation extends LWUITImplementation {
	...
	// blackberry sometimes "breaks" the drawing color... No idea why this might
	// happen...
	private int color;
	...
	void finishEdit(final boolean canceled) {
		// assigning the field here prevents a null pointer exception in the
		// layout
	...
	public void setClip(Object graphics, int x, int y, int width, int height) {
		...
		/**
		 * applying a clip will automatically reset some information that we
		 * need to keep track of manually (it seems).
		 */
	...
	class RIMVideoComponent extends VideoComponent {
		...
		public void paint(com.sun.lwuit.Graphics g) {
			/*
			 * if(isVisible()){ try { if (isFullScreen()) {
			 * c.setDisplayLocation(0, 0);
			 * c.setDisplaySize(Display.getInstance().getDisplayWidth(),
			 * Display.getInstance().getDisplayHeight()); } else {
			 * c.setDisplayLocation(getAbsoluteX(), getAbsoluteY()); int w =
			 * getWidth(); int h = getHeight(); if (c.getDisplayWidth() != w ||
			 * c.getDisplayHeight() != h) { c.setDisplaySize(w, h); } } } catch
			 * (MediaException ex) { ex.printStackTrace(); } }
			 */
		}

		/*
		 * public boolean animate() { Field fld = (Field)getNativePeer();
		 * if(fld.isDirty()) { setShouldCalcPreferredSize(true); } return
		 * super.animate(); }
		 */
		/**
		 * @inheritDoc
	...
	public void drawRGB(Object graphics, int[] rgbData, int offset, int x, int y, int w, int h, boolean processAlpha) {
		...
		// we will paint only the relevant rows row by row to avoid some devices
		// bugs
	...
	public void setAntiAliasedText(Object graphics, boolean a) {
		// this method uses undocumented behavior so it won't compile on
		// different JDE versions
		/*
		 * Graphics g = (Graphics)graphics; Font f = g.getFont(); if(a) {
		 * if(f.getAntialiasMode() != Font.ANTIALIAS_SUBPIXEL) { f =
		 * f.derive(f.getStyle(), f.getHeight(), Ui.UNITS_px,
		 * Font.ANTIALIAS_SUBPIXEL, f.getEffects()); g.setFont(f); } } else {
		 * if(f.getAntialiasMode() == Font.ANTIALIAS_SUBPIXEL) { f =
		 * f.derive(f.getStyle(), f.getHeight(), Ui.UNITS_px,
		 * Font.ANTIALIAS_NONE, f.getEffects()); g.setFont(f); } }
		 */
	}
	...
	public boolean isAntiAliasedText(Object graphics) {
		...
		// different JDE versions
		/*
		 * Graphics g = (Graphics)graphics; Font f = g.getFont(); return
		 * f.getAntialiasMode() == Font.ANTIALIAS_SUBPIXEL;
		 */
		return false;
	}
	...
	/**
	 * Indicates whether the application should minimize or exit when the end
	 * key is pressed
	 * 
	 ...
	 * key is pressed
	 * 
	 * @param aMinimizeOnEnd
	 *            the minimizeOnEnd to set
	 */
	...
	/**
	 * Volume listener is invoked when the volume up/down buttons on the
	 * blackberry device are pressed. The key event would be either
	 * Characters.CONTROL_VOLUME_UP or Characters.CONTROL_VOLUME_DOWN
	 * 
	 * @param al
	...
	private class EditPopup extends PopupScreen implements FocusChangeListener, Runnable {
		...
		protected EditPopup(TextArea lightweightEdit, boolean password, int maxSize) {
			...
			// using Field.EDITABLE flag now because of bug with DevTrack ID
			// 354265 at
		...
		protected void onMenuDismissed(Menu arg0) {
			...
			/**
			 * at this point the EDT is still blocked, so we repaint the screen
			 * with our old buffer and request some fresh paint for the
			 * textfield.
			 */
			this.invalidate();
		}
		...
		protected void onExposed() {
			super.onExposed();
			/**
			 * we reach this if the context menu opened another dialog on top.
			 * 
			 * at this point the EDT is still blocked, so we repaint the screen
			 * with our old buffer and request some fresh paint for the
			 * textfield.
	...
	class NativeEditCallback implements Runnable {
		...
		public void run() {
			...
			if (t != null) {
				...
				// allows the lightweight text area to grow to fill up the
				// screen
	...
	class InvokeLaterWrapper implements Runnable {
		...
		public void run() {
			...
			case INVOKE_AND_WAIT_setFocus:
				try {
					// Not sure why an exception is thrown here during a touch
					// event...
					if (val) {
						fld.setFocus();
					} else {
	...
	public void execute(String url) {
		if (midlet) {
			...
			// if you need this functionality and own a signature add a define
			// to
			// signed
			// #ifdef signed
			// #
	...
	protected void playNativeBuiltinSound(Object data) {
		try {
			try {
				...
				// some simulators take issue with the audio/mpeg string but the
				// mp3 string
				// works fine
	...
	// # }
	// #
	// # public void documentLoaded(BrowserField browserField, Document
	...
	// # }
	// #
	// # public void setBrowserProperty(PeerComponent browserPeer, String key,
	// Object value) {
	// # }
	// #
	...
	// # return
	// ((BrowserField)browserPeer.getNativePeer()).getHistory().canGoBack();
	// # }
	// #
	// # public boolean browserHasForward(PeerComponent browserPeer) {
	// # synchronized(UiApplication.getEventLock()) {
	// # return
	// ((BrowserField)browserPeer.getNativePeer()).getHistory().canGoForward();
	// # }
	// # }
	// #
	...
	// # }
	// #
	// # public void setBrowserPage(PeerComponent browserPeer, String html,
	// String baseUrl) {
	// # synchronized(UiApplication.getEventLock()) {
	// # ((BrowserField)browserPeer.getNativePeer()).displayContent(html,
	// baseUrl);
	// # }
	// # }
	// #
	// # public void browserExecute(PeerComponent browserPeer, String
	// javaScript) {
	// # synchronized(UiApplication.getEventLock()) {
	...
	// # }
	// #
	// # public void browserExposeInJavaScript(PeerComponent browserPeer, Object
	// o, String name) {
	// # synchronized(UiApplication.getEventLock()) {
	// # try {
	// # ((BrowserField) browserPeer.getNativePeer()).extendScriptEngine(name,
	// (Scriptable) o);
	// # } catch (Exception ex) {
	// # ex.printStackTrace();
	// # }

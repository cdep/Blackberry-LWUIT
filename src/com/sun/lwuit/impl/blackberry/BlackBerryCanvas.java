...
class BlackBerryCanvas extends FullScreen {
	...
	/**
	 * If a peer component is the current focused component it should receive
	 * all events directly from the device
	 */
	Field eventTarget;
					...
					if (currentForm != null) {
						...
						if (p != null) {
							...
							if (p.getPreferredH() < f.getHeight() || p.getPreferredW() < f.getWidth()) {
								// blackberry reports a false size until the
								// actual layout occurs we need to revalidate
								p.invalidate();
							}
						} else {
	...
	/*
	 * public void subpaint(Graphics g) { paint(g); }
	 */

	/**
	 * Clips the RIM native graphics based on the component hierarchy within
	 * LWUIT so the native RIM component doesn't paint itself above other
	 * components such as the forms title.
	 */
	...
	protected boolean keyChar(char c, int status, int time) {
		...
		if (Keypad.getHardwareLayout() != 1230263636) {
			...
			// BlackBerryCanvas.debug = "K " + code + " c " + ((char)code) +
			// " s " + status;

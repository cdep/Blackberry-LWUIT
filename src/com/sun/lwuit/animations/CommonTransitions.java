 ...
 * <ol>
 * <li>Slide - the exiting form slides out of the screen while the new form
 * slides in.
 * <li>Fade - components fade into/out of the screen
 * <li>Timeline - uses an animation image as an alpha mask between the
 * source/destination
 * </ol>
 * <p>
...
public final class CommonTransitions extends Transition {
	...
	/**
	 * Creates a slide transition for the body of the form that fades the title
	 * in while sliding
	 * 
	 * @param forward
	 ...
	 * than on minimizing heap usage. This method works by creating two images
	 * and sliding them which works much faster for all devices however takes up
	 * more ram. Notice that this method of painting doesn't support various
	 * basic LWUIT abilities such as translucent menus/dialogs etc.
	 * 
	 * @param type
	...
	/**
	 * Creates a transition using an animated image object (e.g. timeline
	 * object) as an alpha mask between the source/target
	 * 
	 * @param animation
	...
	public void initTransition() {
		...
		// a transition might occur with illegal source or destination values
		// (common with
		...
		if (buffer == null) {
			...
			// this might happen when screen orientation changes or a MIDlet
			// moves
			...
			if (buffer.getWidth() != w || buffer.getHeight() != h) {
				...
				// slide motion might need resetting since screen size is
				// different
				motion = null;
			}
		}
	...
	/**
	 * This method can be overriden by subclasses to create their own motion
	 * object on the fly
	 * 
	 * @param startOffset
	 *            the start offset for the menu
	 * @param dest
	...
	private Motion getComponentShiftMotion(Component c, boolean incoming) {
		...
		if (m == null) {
			...
			if (getDestination().getWidth() - c.getWidth() < 10) {
				// big component that takes up all the space such as a title
				// that occupies the entire title area
	...
	private void paintAlpha(Graphics graphics) {
		// this will always be invoked on the EDT so there is no race condition
		// risk
	...
	private void paintSlideAtPosition(Graphics g, int slideX, int slideY) {
		...
		// if this is the first form we can't do a slide transition since we
		// have no source form
		if (source == null) {
			return;
		}
	...
	private void paint(Graphics g, Component cmp, int x, int y, boolean background) {
		...
		// g.clipRect(cmp.getAbsoluteX(), cmp.getAbsoluteY(), cmp.getWidth(),
		// cmp.getHeight());
		g.translate(x, y);
		// g.clipRect(cmp.getAbsoluteX(), cmp.getAbsoluteY(), cmp.getWidth(),
		// cmp.getHeight());
	...
	public Transition copy(boolean reverse) {
		...
		case TYPE_SLIDE: {
			...
			// prevent double reversing of forward due to bidi when copying a
			// transition
	...
	/**
	 * Indicates whether the motion associated with these transitions by default
	 * is linear or spline motion
	 * 

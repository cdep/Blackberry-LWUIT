...
/**
 * An animation object is an element within the timeline that has a visibility
 * state for rendering at a given point in time. E.g. the object can be queried
 * of its position and render itself for any time.
 * 
 * @author Shai Almog
 */
public final class AnimationObject {
	 ...
	 * 
	 * @param img
	 *            the image to animate within the timeline
	 * @param x
	...
	/**
	 * Defines the frames of the animation if this is a frame changing animation
	 * (e.g. a sprite of a walking person). Notice that this method must not be
	 * invoked more than once or after the image was initilized
	 * 
	 * @param frameWidth
	 *            the width of the frame within the image object
	 * @param frameHeight
	 *            the height of the frame within the image object
	 * @param frameDelay
	...
	/**
	 * Creates an animation object instance that can define the animation
	 * properties for an image. This version of the method is useful while a
	 * resource file is in the process of being loaded and not all images are in
	 * place. Loading will finish implicitly when the image is first used.
	 * 
	 * @param imageName
	 *            the image to animate within the timeline
	 * @param res
	 *            the resources file from which the image should be fetched.
	 * @param x
	 *            position of the animation
	 * @param y
	 *            position of the animation
	...
	Image getImage() {
		...
		if (frameDelay > -1) {
			if (!framesInitialized) {
				...
				// if we are on a device we no longer need the img from now on
				// and can use
				// only the frames. However, in the resource editor we still
				// need the image instance
	 ...
	 * 
	 * @param motionType
	 *            the type of the motion (spline/linear)
	 * @param startTime
	 *            the start time for the motion within the timeline timeframe
	 * @param duration
	 *            the duration of the motion
	 * @param start
	 *            the starting position (the value before startTime)
	 * @param end
	 *            the ending position for the property (the value after endTime)
	 */
	...
	/**
	 * Defines a rotation animation starting at the given time/value and ending
	 ...
	 * ending at the given position. Values should rance from 0 (transparent) to
	 * 255 (opaque).
	 * 
	 * @param motionType
	 *            the type of the motion (spline/linear)
	...
	void draw(Graphics g, float scaleX, float scaleY) {
		...
		// this can happen due to a race condition, mostly in the resource
		// editor which

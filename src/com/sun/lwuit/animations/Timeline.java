...
public final class Timeline extends Image implements Animation, Painter {
	...
	/**
	 * Inidicates the minimal delay between animation frames allowing the CPU to
	 * rest. Increase this number to increase general device performance,
	 * decrease it to speed the animation.
	 */
	 ...
	 * @param size
	 *            size of the animation in virtual pixels, if the size differs
	 *            the animation would be scaled on the fly
	 ...
	 * for the current frame. This allows functionality such as responding to
	 * pointer events on the resource editor. Notice that this method is not
	 * efficient since it tests the pixel opacity which is a pretty expensive
	 * operation...
	 * 
	 * @param x
	 ...
	 * @return an animation object or null if no animation object is at that
	 *         position.
	 */
	public AnimationObject getAnimationAt(int x, int y) {
		for (int iter = 0; iter < animations.length; iter++) {
			...
			if (Rectangle.intersects(ax, ay, w, h, x, y, 1, 1)) {
				// we now need to check if the pixel at that position is not
				// transparent

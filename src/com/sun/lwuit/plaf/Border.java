 ...
 * component. It is the responsibility of the component not to draw outside of
 * the border line.
 * <p>
 * This class can be extended to provide additional border types and custom made
 * border types.
 * <p>
 * A border can optionally paint the background of the component, this depends
 * on the border type and is generally required for rounded borders that "know"
 * the area that should be filled.
 * 
 * @author Shai Almog
 */
public class Border {
	...
	/**
	 * Indicates whether theme colors should be used or whether colors are
	 * specified in the border
	 */
	boolean themeColors;
	...
	Border outerBorder; // A border added outside of this border (Used for CSS
						// outline property, but can also be used for other
						// purposes)
	String borderTitle; // border title, currently supported only for line
						// borders
	...
	/**
	 * This method is designed mainly for the purpose of creating an arrow that
	 * will track a specific component using the image border the tile given
	 * would be an arrow like image just like the ones used for the
	 * top/bottom/left/right images. This image would be positioned so it points
	 * at the center of the track component
	 * 
	 * @param tileTop
	 *            an image that will replace one of the tiles on the top if the
	 *            track component is there
	 * @param tileBottom
	 *            an image that will replace one of the tiles on the bottom if
	 *            the track component is there
	 * @param tileLeft
	 *            an image that will replace one of the tiles on the left if the
	 *            track component is there
	 * @param tileRight
	 *            an image that will replace one of the tiles on the right if
	 *            the track component is there
	 * @param trackComponent
	 *            the component that will be tracked for the positioning of the
	 *            tile
	 */
	...
	/**
	 * Cleans the tile tracking state allowing the garbage collector to pick up
	 * the component and the image data
	 */
	...
	/**
	 * Returns the minimum size required to properly display this border,
	 * normally this is 0 but a border might deem itself undisplayable with too
	 * small a size e.g. for the case of an image border the minimum height
	 * would be top + bottom and the minimum width would be left+right
	 * 
	...
	/**
	 * Returns an empty border, this is mostly useful for overriding components
	 * that have a border by default
	 * 
	...
	/**
	 * Creates an empty border, this is useful where we don't want a border for
	 * a component but want a focus border etc...
	 * 
	 * @return a border than draws nothing
	 */
	...
	/**
	 * The given top/bottom/left/right images are tiled appropriately across the
	 * matching sides of the border and the corners are placed as expected in
	 * the four corners. The background image is optional and it will be tiled
	 ...
	 * @param left
	 *            the image of the left line
	 * @param right
	 *            the image of the right line
	 * @param topLeft
	 *            the image of the top left corner
	 * @param topRight
	 *            the image of the top right corner
	 * @param bottomLeft
	 *            the image of the bottom left corner
	 * @param bottomRight
	 *            the image of the bottom right corner
	 * @param background
	...
	/**
	 * The given top/bottom/left/right images are scaled appropriately across
	 * the matching sides of the border and the corners are placed as expected
	 * in the four corners. The background image is optional and it will be
	 * tiled in the background if necessary.
	 * 
	 * <p>
	 * By default this border does not override background unless a background
	 * image is specified
	 ...
	 * border, rotated and placed as expected in the four corners. The
	 * background image is optional and it will be tiled in the background if
	 * necessary.
	 * <p>
	 * By default this border does not override background unless a background
	 * image is specified.
	 * <p>
	 * Notice that this version of the method is potentially much more efficient
	 * since images are rotated internally and this might save quite a bit of
	 * memory!
	 * <p>
	 * <b>The top and topLeft images must be square!</b> The width and height of
	 * these images must be equal otherwise rotation won't work as you expect.
	 * 
	 * @param top
	 *            the image of the top line
	 * @param topLeft
	 ...
	 * <p>
	 * This border overrides any painter used on the component and would ignor
	 * such a painter.
	 * 
	 * @param arcWidth
	...
	/**
	 * Creates a rounded border that uses the given color for the component. Due
	 * to technical issues (lack of shaped clipping) performance and memory
	...
	/**
	 * Compares two object including the scenario one of them is null (thus
	 * avoiding equals pitfalls)
	 * 
	 * @param obj1
	...
	public static Border createCompoundBorder(Border top, Border bottom, Border left, Border right) {
		...
		if ((isSame(top, bottom)) && (isSame(top, left)) && (isSame(top, right))) {
			return top; // Borders are all the same, returning one of them
						// instead of creating a compound border which is more
						// resource consuming
		}
		...
		// Calculates the thickness of the entire border as the maximum of all 4
		// sides
		b.thickness = 0;
		for (int i = Component.TOP; i <= Component.RIGHT; i++) {
	 ...
	 *            RGB of the outer edge of the highlight area
	 * @param highlightInner
	 *            RGB of the inner edge of the highlight area
	 * @param shadowOuter
	 *            RGB of the outer edge of the shadow area
	 * @param shadowInner
	...
	/**
	 * Allows us to define a border that will act as the pressed version of this
	 * border
	 * 
	 * @param pressed
	 *            a border that will be returned by the pressed version method
	 */
	...
	/**
	 * Allows us to define a border that will act as the focused version of this
	 * border
	 * 
	 * @param focused
	 *            a border that will be returned by the focused version method
	...
	/**
	 * When applied to buttons borders produce a version that reverses the
	 * effects of the border providing a pressed feel
	 * 
	 ...
	 * normally the painter will perform this work but in this case the border
	 * might do it instead.
	 * 
	 * @param g
			...
			if ((s.getBgImage() != null && s.getBackgroundType() == Style.BACKGROUND_IMAGE_SCALED) ||
				...
				if (i != null && i.getWidth() == width && i.getHeight() == height) {
					...
					if (s.getBackgroundType() == Style.BACKGROUND_IMAGE_SCALED) {
						...
						// might occur during temporary conditions in the theme
						// switching
						if (p == null) {
							return;
						}
				...
				if (s.getBgTransparency() == ((byte) 0xff)) {
					...
					if (g.isAlphaSupported()) {
						...
						// if its transparent we don't need to do anything, if
						// its
						// translucent... well....
						if (s.getBgTransparency() != 0) {
							...
							if (g.getColor() != 0xffffff) {
								...
								// background color is white we need to remove a
								// different color
								// black is the only other "reliable" color on
								// the device
								Graphics imageG = i.getGraphics();
								imageG.setColor(0);
	...
	/**
	 * Draws the border for the given component, this method is called before a
	 * call to background painting is made.
	 * 
	 * @param g
	 *            graphics context to draw onto
	 * @param c
	...
	void paint(Graphics g, int x, int y, int width, int height, Component c) {
		...
		case TYPE_LINE:
			if (borderTitle == null) {
				...
				if (c.isRTL()) {
					g.fillRect(x + width - TITLE_MARGIN, topY, TITLE_MARGIN, thickness); // top
																							// (segment
																							// before
																							// the
																							// title)
					g.fillRect(x, topY, width - (TITLE_MARGIN + titleW + TITLE_SPACE * 2), thickness); // top
																										// (segment
																										// after
																										// the
																										// title)
					...
					g.fillRect(x, topY, TITLE_MARGIN, thickness); // top
																	// (segment
																	// before
																	// the
																	// title)
					g.fillRect(x + TITLE_MARGIN + titleW + TITLE_SPACE * 2, topY, width - (TITLE_MARGIN + titleW + TITLE_SPACE * 2), thickness); // top
																																					// (segment
																																					// after
																																					// the
																																					// title)
	 ...
	 * 
	 * @param g
	 *            The graphics to apply the clip area on
	 * @param rect
	 *            A Rectangle representing the saved clip area
	 */
	...
	/**
	 * This method returns how thick is the border in pixels, notice this
	 * doesn't apply to most border types
	 * 
	...
	/**
	 * This method returns sets the border thickness in pixels, notice this
	 * doesn't apply to most border types
	 * 
	 * @param thickness
	...
	/**
	 * Allows toggling the order in which the outer and inner borders are
	 * painted for the Outer border type
	 * 
	 * @param paintOuterBorderFirst
	...
	/**
	 * This method returns the Compound Borders array. The array size is 4 and
	 * the borders arranged as follows : Border[] borders =
	 * getCompoundBorders(); Border top = borders[Component.TOP]; Border bottom
	 * = borders[Component.BOTTOM]; Border left = borders[Component.LEFT];
	...
	/**
	 * This callback indicates that a component pointing at this border is
	 * initialized, this method is useful for image borders whose lock methods
	 * are implicitly invoked. This method may be invoked multiple times.
	 */
	public void lock() {
		if (images != null) {
			for (int iter = 0; iter < images.length; iter++) {

...
public class DefaultLookAndFeel extends LookAndFeel implements FocusListener {
	 ...
	 * @param unchecked
	 *            the image to draw in order to represent an uncheck checkbox
	 * @param disabledChecked
	 *            same as checked for the disabled state
	 * @param disabledUnchecked
	 ...
	 * 
	 * @param selected
	 *            the image to draw in order to represent a selected radio
	 *            button
	 * @param unselected
	 *            the image to draw in order to represent an unselected radio
	 *            button
	 */
	 ...
	 *            the image to draw in order to represent an unselected radio
	 *            button
	 * @param disabledSelected
	 *            same as selected for the disabled state
	 * @param disabledUnselected
	...
	/**
	 * Returns the images used to represent the radio button (selected followed
	 * by unselected).
	 * 
	 * @return images representing the radio button or null for using the
	 *         default drawing
	 */
	...
	/**
	 * Returns the images used to represent the radio button when in focused
	 * mode
	 * 
	 * @return images representing the radio button or null for using the
	 *         default drawing
	 ...
	 * unselected).
	 * 
	 * @return images representing the check box or null for using the default
	 *         drawing
	 */
	...
	public void drawCheckBox(Graphics g, Button cb) {
		if (chkBoxImages != null) {
			...
			// checkbox square needs to be the height and width of the font
			// height even
			// when no text is in the check box this is a good indication of
			// phone DPI
	 ...
	 * 
	 * @param coordinate
	 *            a position in a theoretical plain
	 * @param plain
	 *            the amount of space in the theoretical plain
	 * @param pixelSize
	 *            the amount of pixels available on the screen
	...
	public void drawRadioButton(Graphics g, Button rb) {
		if (rButtonImages != null) {
			...
			// radio button radius needs to be of the size of the font height
			// even
			// when no text is in the radio button this is a good indication of
			// phone DPI
	...
	private int findDestColor(int color) {
		...
		if (sourceR > 128 && sourceG > 128 && sourceB > 128) {
			...
			// special case for black, since all channels are 0 it can't be
			// brightened properly...
			if (color == 0) {
				return 0x222222;
			}
	...
	public Dimension getCheckBoxPreferredSize(Button cb) {
		...
		// checkbox square needs to be the height and width of the font height
		// even
		// when no text is in the check box this is a good indication of phone
		// DPI
	...
	private void drawComponent(Graphics g, Label l, Image icon, Image stateIcon, int preserveSpaceForState) {
		...
		if (stateIcon != null) {
			stateIconSize = stateIcon.getWidth(); // square image width ==
													// height
	...
	protected int drawLabelText(Graphics g, Label l, String text, int x, int y, int textSpaceW) {
		...
		if ((!isTickerRunning) || rtl) {
			...
			if (txtW > textSpaceW && textSpaceW > 0) {
				// Handling of adding 3 points and in fact all text positioning
				// when the text is bigger than
				// the allowed space is handled differently in RTL, this is due
				// to the reverse algorithm
				// effects - i.e. when the text includes both Hebrew/Arabic and
				// English/numbers then simply
				// trimming characters from the end of the text (as done with
				// LTR) won't do.
				// Instead we simple reposition the text, and draw the 3 points,
				// this is quite simple, but
				// the downside is that a part of a letter may be shown here as
				// well.
			...
			public void paint(Graphics g) {
				...
				if (tabP == TabbedPane.TOP || tabP == TabbedPane.BOTTOM) {
					if (tabP == TabbedPane.TOP) {
						...
						g.fillRect(getX(), getY(), getWidth(), tPBorder);// north
																			// line

						g.fillRect(getX(), getY(), tPBorder, getHeight());// west
																			// line
						...
						g.setColor(tabbedPaneStyle.getFgColor());
						g.fillRect(getX(), getY() + getHeight() - tPBorder, getWidth(), tPBorder);// south
																									// line
					...
					if (x == contentPaneSize.getWidth()) {
						g.fillRect(x + cellOffsetX - tPBorder, getY(), tPBorder, getHeight());// east
																								// line
					...
					if (!tp.isRTL() && tabP == TabbedPane.LEFT ||
						...
						g.fillRect(getX() + getWidth() - tPBorder, getY(), tPBorder, getHeight());// east
																									// border
					}
				}
			}
	...
	public void drawTextField(Graphics g, TextArea ta) {
		...
		// QWERTY devices don't quite have an input mode hide it also when we
		// have a VK
		...
		if (ta.isSingleLineTextArea()) {
			...
			if (align == Component.LEFT) {
				if (cursorCharPosition > 0) {
					...
					// no point in showing the input mode when there is only one
					// input mode...
		...
		// no point in showing the input mode when there is only one input
		// mode...
		if (inputModeWidth > 0 && ta.getInputModeOrder() != null && ta.getInputModeOrder().length > 1) {

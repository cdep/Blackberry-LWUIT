...
/**
 * Layout manager similar in spirit to HTML tables allowing rows and columns of
...
public class TableLayout extends Layout {
	...
	public void layoutContainer(Container parent) {
		...
		// try to recalculate the columns for none horizontally scrollable
		// tables
		...
		if (!parent.isScrollableX()) {
			...
			// check how many columns we can modify (the user hasn't requested a
			// specific size for those)
			...
			if (pWidth < totalWidth) {
				...
				// Go over the modifyable columns and remove the right pixels
		...
		for (int r = 0; r < rowSizes.length; r++) {
			for (int c = 0; c < columnSizes.length; c++) {
				...
				if (con != null && con != H_SPAN_CONSTRAINT && con != V_SPAN_CONSTRAINT && con != VH_SPAN_CONSTRAINT) {
					...
					// conX = left + leftMargin + columnPositions[c]; // bugfix
					// table with padding not drawn correctly
					// conY = top + topMargin + rowPositions[r]; // bugfix table
					// with padding not drawn correctly
					...
					if (con.spanHorizontal > 1) {
						...
						// for RTL we need to move the component to the side so
						// spanning will work
						if (rtl) {
	...
	/**
	 * Returns the position of the given table row. A valid value is only
	 * returned after the layout occurred.
	 * 
	 * @param row
	 *            the row in the table
	 * @return the Y position in pixels or -1 if layout hasn't occured/row is
	 *         too large etc.
	 */
	...
	/**
	 * Returns the position of the given table column. A valid value is only
	 * returned after the layout occurred.
	 * 
	 * @param col
	 *            the column in the table
	 * @return the X position in pixels or -1 if layout hasn't occured/column is
	 *         too large etc.
	 */
	...
	/**
	 * Places the component/constraint in the proper alignment within the cell
	...
	/**
	 * Sets the minimum size for a column in the table, this is applicable for
	 * tables that are not scrollable on the X axis. This will force the earlier
	 * columns to leave room for the latter columns.
	 * 
	 * @param minimumSize
	...
	/**
	 * Indicates the minimum size for a column in the table, this is applicable
	 * for tables that are not scrollable on the X axis. This will force the
	 * earlier columns to leave room for the latter columns.
	 * 
	...
	public static class Constraint {
		...
		/**
		 * Sets the cells to span vertically, this number must never be smaller
		 * than 1
		 * 
		 * @param span

 ...
 * grid of components/labels. The table reflects and updates the underlying
 * model data.
 * 
 * @author Shai Almog
 */
public class Table extends Container {
	...
	/**
	 * This flag allows us to workaround issue 275 without incuring too many
	 * updateModel calls
	 */
	 ...
	 * 
	 * @param model
	 *            the model underlying this table
	 */
	 ...
	 *            the model underlying this table
	 * @param includeHeader
	 *            Indicates whether the table should render a table header as
	 *            the first row
	 */
	 ...
	 * 
	 * @return the offset of the selected column in the table if a selection
	 *         exists
	 */
	...
	protected void paintGlass(Graphics g) {
		if ((drawBorder) && (innerBorder != INNER_BORDERS_NONE)) {
			...
			if ((collapseBorder) || (innerBorder != INNER_BORDERS_ALL) || // inner
																			// borders
																			// cols/rows
																			// are
																			// supported
																			...
																			// collapsed
																			// mode
			(t.hasHorizontalSpanning()) || (t.hasVerticalSpanning())) { // TODO
																		// - We
																		// currently
																		// don't
																		// support
				...
				if ((innerBorder == INNER_BORDERS_ALL) || (innerBorder == INNER_BORDERS_ROWS)) {
					if (t.hasVerticalSpanning()) {
						// iterate over the components and draw a line on the
						// side of all
						// the components other than the ones that are at the
						// last column.
						for (int cellRow = 0; cellRow < rows - 1; cellRow++) {
							for (int cellColumn = 0; cellColumn < cols; cellColumn++) {
								...
								if (cellRow + t.getCellVerticalSpan(cellRow, cellColumn) - 1 != rows - 1) {
									// if this is a spanned through cell we
									// don't want to draw a line here
						...
						// this is much faster since we don't need to check
						// spanning
				...
				for (int row = 0; row < rows; row++) {
					...
					for (int col = 0; col < cols; col++) {
						...
						if ((comp.isVisible()) &&
							...
							if (col == 0) {
								rightMargin *= 2; // Since the first cell
													// includes margins from
													// both sides (left/right)
													// so the next cell location
													// is farther away - but we
													// don't want to paint the
													// border up to it
							}
							if (row == 0) {
								bottomMargin *= 2;
	...
	private Component createCellImpl(Object value, final int row, final int column, boolean editable) {
		...
		// we do this here to allow subclasses to return a text area or its
		// subclass
	 ...
	 * @param row
	 *            row number, -1 for the header rows
	 * @param column
	 *            column number
	 * @param editable
	...
	protected Component createCell(Object value, int row, int column, boolean editable) {
		if (row == -1) {
			...
			header.getStyle().setAlignment(titleAlignment);
		...
		cell.setUIID("TableCell");
		cell.getStyle().setAlignment(cellAlignment);
	...
	public void initComponent() {
		// this can happen if deinitialize is invoked due to a menu command
		// which modifies
	...
	public void deinitialize() {
		// we unbind the listener to prevent a memory leak for the use case of
		// keeping
	...
	/**
	 * Sets how to draw the inner border (All of it, only rows/columns, none,
	 * groups) Note that setting to any mode other than NONE/ALL will result in
	 * the border drawing as collapsed whether this is a collpased border or not
	 * 
	 * @param innerBorder
	 *            one of the INNER_BORDER_* constants
	 */
	 ...
	 * 
	 * @return the current inner border mode (one of the INNER_BORDER_*
	 *         constants)
	 */
	 ...
	 * This allows customization in subclasses to create for example the effects
	 * of segments in atable, i.e. instead of a line after each row - lines
	 * after "chunks" of rows. Note that this is queried only when the inner
	 * border mode is set to INNER_BORDER_ROWS
	 * 
	 * @param row
	 *            The row in question
	...
	/**
	 * Indicates whether the borders of the cells should collapse to form a one
	 * line border
	 * 
	 * @param collapseBorder
	 *            true to collapse (default), false for separate borders
	 */
	public void setCollapseBorder(boolean collapseBorder) {
		if (this.collapseBorder != collapseBorder) {
			...
			if ((horizontalBorderSpacing != 0) || (verticalBorderSpacing != 0)) { // Only
																					// if
																					// one
																					// of
																					// the
	...
	/**
	 * Indicates whether empty cells should have borders (relevant only for
	 * separate borders and not for collapsed)
	 * 
	 * @param drawEmptyCellsBorder
	 *            - true to draw (default), false otherwise
	 */
	...
	/**
	 * Sets the spacing of cells border (relevant only for separate borders and
	 * not for collapsed)
	 * 
	 * @param horizontal
	...
	/**
	 * Creates the table cell constraint for the given cell, this method can be
	 * overriden for the purposes of modifying the table constraints.
	 * 
	 * @param value
	...
	class Listener implements DataChangedListener, ActionListener {
		...
		public final void dataChanged(int row, int column) {
			...
			// a repaint sent right before this might result in an artifact for
			// some use cases so

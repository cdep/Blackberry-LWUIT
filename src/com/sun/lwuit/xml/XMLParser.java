...
/**
 * The parser class is used to parse an XML document into a DOM object
 * (Element).
 * 
 * @author Ofir Leitner
 */
public class XMLParser {
	...
	boolean includeWhitespacesBetweenTags; // For HTML white spaces between tags
											// are significant to seperate
											// words, in XML less so and it
											// mostly creates garbage elements
											// (Text with one space)

	/**
	 * The char entities strings supported in XML. When a char entity is found
	 * these will be compared against first.
	 * 
	 * private static final String[] XML_CHAR_ENTITIES = { "lt", // lesser-than
	 * "gt", // greater-than "amp", // ampersand "quot", //quotation mark
	 * "apos", // apostrophe //"bull", //bullet //"euro" //euro };
	 * 
	 * /** The numericals value of char entities strings above.
	 * 
	 * private static final int[] XML_CHAR_ENTITIES_VALS = { 60, // "lt", //
	 * lesser-than 62, // "gt", // greater-than 38, // "amp", // ampersand 34,
	 * // "quot", //quotation mark 39, // "apos", // apostrophe //8226, //
	 * "bull", //bullet //8364 // "euro"}; //euro };
	 */
	...
	/**
	 * A constant containing the CDATA tag identifier (minus the C which is read
	 * anyway first) CDATA nodes will be converted to text nodes
	 */
	...
	/**
	 * Returns a string identifying the document type this parser supports. This
	 * should be overriden by subclassing parsers.
	 * 
	...
	/**
	 * This method translates between a XML char entity string to the according
	 * char code. The string is first compared to the 5 XML supported strings:
	 * quot,apos,amp,lt and gt. If still not found it goes to look in the user
	 * defined char entities hashtable
	 * 
	 * @param symbol
	 *            The symbol to lookup
	 * @return The char code of the symbol, or -1 if none found
	 * 
	 *         protected int getCharEntityCode(String symbol,Hashtable
	 *         userDefined) { // First tries the XML basic char entities int
	 *         val=-1; for (int i=0;i<XML_CHAR_ENTITIES.length;i++) { if
	 *         (symbol.equalsIgnoreCase(XML_CHAR_ENTITIES[i])) { return
	 *         XML_CHAR_ENTITIES_VALS[i]; } } if (val!=-1) { return val; } else
	 *         {
	 * 
	 *         // Not found in the standard symbol table, see if it is in the
	 *         user defined symbols table if (userDefined!=null) { Object
	 *         charObj=userDefined.get(symbol); if (charObj!=null) { return
	 *         ((Integer)charObj).intValue(); } }
	 * 
	 *         // Not found anywhere return -1; } }
	 */

	/**
	 * Converts a char entity to the matching character. This handles both
	 * numbered and symbol char entities (The latter is done via
	 * getCharEntityCode)
	 * 
	 * @param charEntity
	 *            The char entity to convert
	 * @return A string containing a single char, or the original char entity
	 *         string (with & and ;) if the char entity couldn't be resolved
	 */
	protected String convertCharEntity(String charEntity) {
		try {
			...
			return "&" + charEntity + ";"; // Another option is to return an
											// empty string, but returning the
											// entity will unravel bugs and will
											// also allow ignoring common
											// mistakes such as using the & char
											// (instead of &apos;)
		}

		/*
		 * int charCode=-1; if (charEntity.startsWith("#")) { //numbered char
		 * entity if (charEntity.startsWith("#x")) { //hex try {
		 * charCode=Integer.parseInt(charEntity.substring(2),16); } catch
		 * (NumberFormatException nfe) { //if not a number - simply ignore char
		 * entity } } else { try {
		 * charCode=Integer.parseInt(charEntity.substring(1)); } catch
		 * (NumberFormatException nfe) { //if not a number - simply ignore char
		 * entity } } } else { //not numbered, rather a symbol
		 * charCode=getCharEntityCode(charEntity,userDefinedCharEntities); }
		 * 
		 * if (charCode!=-1) { return ""+(char)charCode; } else {
		 * notifyError(ParserCallback
		 * .ERROR_UNRECOGNIZED_CHAR_ENTITY,null,null,null,
		 * "Unrecognized char entity: "+charEntity); return "&"+charEntity+";";
		 * // Another option is to return an empty string, but returning the
		 * entity will unravel bugs and will also allow ignoring common mistakes
		 * such as using the & char (instead of &apos;) }
		 */

	}

	/**
	 * This is the entry point for parsing a document and the only non-private
	 * member method in this class
	 * 
	 * @param is
	 *            The InputStream containing the XML
	 * @return an Element object describing the parsed document (Basically its
	 *         DOM)
	 */
	public Element parse(Reader is) {
		eventParser = false;
		Element rootElement = createNewElement("ROOT"); // ROOT is a "dummy"
														// element that all
														// other document
	...
	/**
	 * Creates a new element. This should be overriden by parsers that use a
	 * subclass of Element.
	 * 
	 * @param name
	...
	/**
	 * Creates a new text element. This should be overriden by parsers that use
	 ...
	 *             if an exception is thrown by the reader
	 */
	...
	/**
	 * Invoked when the event parser encounters a text element. This callback
	 * method is invoked only on the eventParser.
	 * 
	 * @param text
	 ...
	 * the tag or return false to skip the tag. This callback method is invoked
	...
	/**
	 * Invoked for every attribute value of the givne tag This callback method
	 * is invoked only on the eventParser.
	 * 
	 * @param tag
	 *            the tag name
	 */
	...
	/**
	 * Parses tags content, accumulating text and child elements . Upon bumping
	 * a start tag character it calls the parseTag method. This method is called
	 * at first from the parse method, and later on from parseTag (which creates
	 * the recursion).
	 * 
	 * @param element
	 ...
	 *            The InputStream containing the XML
	 * @throws IOException
	 *             if an I/O error in the stream is encountered
	 */
	protected void parseTagContent(Element element, Reader is) throws IOException {
		...
		while ((byte) c != -1) {
			if (c == '<') {
				...
				if (text != null) {
					// Mistakenly "collected" something that is not a char
					// entity, perhaps
				...
				if (childElement == END_TAG) { // was actually an ending tag
					...
					if (element != null) {
						if (closingTag.equalsIgnoreCase(element.getTagName())) {
							...
							// do nothing, someone chose to close an empty tag
							// i.e. <img ....></img> or <br></br>
						} else {
				...
				if (charEntity != null) {
					...
					charEntity = ""; // The & is not included in the string we
										// accumulate
				} else {
					text += c;
				}
			...
			} else { // leading space is relevant also for newline and other
						// whitespaces //if (c==' ') {
				leadingSpace = true;
			}
			c = (char) is.read();
	...
	/**
	 * Checks if the specified character is a white space or not. Exposed to
	 * packaage since used by HTMLComponent as well
	 * 
	 * @param ch
	 *            The character to check
	...
	/**
	 * This method collects the tag name and all of its attributes. For comments
	 * and XML declarations this will call the parseCommentOrXMLDeclaration
	 * method. Note that this method returns an Element with a name and
	 * attrbutes, but not its content/children which will be done by
	 * parseTagContent
	 * 
	 * @param is
	 *            The InputStream containing the XML
	...
	protected Element parseTag(Reader is) throws IOException {
		...
		String curAttribute = "";
		String curValue = "";
		// boolean procInst=false; // Support for the styleshhet processing
		// instruction was removed, as it is not supported in most browsers, and
		// it causes problems by adding tags before the HTML element (Makign the
		...
		if (c == '/') {
			...
			if ((c == '-') && (c2 == '-')) { // comment
				...
				if (idx == CDATA_STR.length()) { // found CDATA
					return parseCommentOrXMLDeclaration(is, "]]>"); // parse
																	// doctypes
																	// i.e.
																	// <!DOCTYPE
																	// .... > as
				...
				return parseCommentOrXMLDeclaration(is, ">"); // parse doctypes
																// i.e.
																// <!DOCTYPE
																// .... > as
																// comments as
			...
			// procInst=true;
			// c=(char)is.read();
			return parseCommentOrXMLDeclaration(is, ">"); // parse XML
															// declaration i.e.
															// <?xml
															// version="1.0"
															// encoding="ISO-8859-1"?>
		...
		/*
		 * if (procInst) { if (tagName.equals("xml-stylesheet")) { // The XML
		 * processing instruction <?xml-stylesheet ... ?> has the same
		 * parameters as <link .. > and behaves the same way tagName="link"; }
		 * else { // Processing instruction not supported - read till its end
		 * c=(char)is.read(); while (c!='>') { c=(char)is.read(); } Element
		 * procInstElem=createNewElement("unsupported");
		 * procInstElem.isComment=true; return procInstElem; } }
		 */
		...
		if (!processTag || !isSupported(element)) {
			...
			if (!processTag || !shouldEvaluate(element)) {
				...
				// This is especially important for the script tag which may
				// contain '<' and '>' which might confuse the parser
				char lastChar = c;
				while (c != '>') { // Read till the end of the tag
					lastChar = c;
					c = (char) is.read();
				}
				if (lastChar != '/') { // If this is an empty tag, no need to
										// search for its closing tag as there's
										// none...
		...
		if (c == '>') { // tag declartion ended, process content
			...
			return element;
		} else if (c == '/') { // || ((procInst) && (c=='?'))) { //closed tag -
								// no content
			c = (char) is.read();
			if (c == '>') {
				if (eventParser) {
		...
		while (true) {
			...
			if (c == '>') { // tag close char shouldn't be found here, but if
							// the XML is slightly malformed we return the
							// element
			...
			if (c != '=') {
				...
				if (c == '>') { // tag close char shouldn't be found here, but
								// if the XML is slightly malformed we return
								// the element
					if (!isEmptyTag(tagName)) {
						parseTagContent(element, is);
					}
				...
				continue; // if attribute is not followed by = then process the
							// next attribute
			}
			...
			while (!ended) {
				...
				if (c == quote) {
					...
					if (charEntity != null) {
						curValue += "&" + charEntity; // Wasn't a char entit,
														// probably a url as a
														// parameter : i.e.
														// param="/test?p=val&pw=val2&p3=val3
					}
					charEntity = "";
				} else {
					if (charEntity != null) {
			...
			if (charEntity != null) { // Mistaken something else for a char
										// entity - for example an action which
										// is
										// action="http://domain/test.html?param1=val1&param2=val2"
				curValue += "&" + charEntity;
				charEntity = null;
			}
			...
			if (eventParser) {
				...
				if (error == ParserCallback.ERROR_ATTRIBUTE_NOT_SUPPORTED) {
					...
					// notifyError(error, tagName, curAttribute, curValue,
					// "Attribute '"+curAttribute+"' is not supported for tag '"+tagName+"'. Supported attributes: "+element.getSupportedAttributesList());
	...
	/**
	 * This utility method is used to parse comments and XML declarations in the
	 * XML. The comment/declaration is returned as an Element, but is flagged as
	 * a comment since both comments and XML declarations are not part of the
	 * XML DOM. This method can be overridden to process specific XML
	 * declarations
	 * 
	 * @param is
	 *            The inputstream
	...
	protected Element parseCommentOrXMLDeclaration(Reader is, String endTag) throws IOException {
		...
		while (!ended) {
			...
			if (c == endTag.charAt(endTagPos)) {
				...
				if (endTagPos != 0) { // add - or -- if it wasn't an end tag
										// eventually
	...
	/**
	 * A utility method used to notify an error to the ParserCallback and throw
	 * an IllegalArgumentException if parsingError returned false
	 * 
	 * @param errorId
	 *            The error ID, one of the ERROR_* constants in ParserCallback
	 * @param tag
	 *            The tag in which the error occured (Can be null for non-tag
	 *            related errors)
	 * @param attribute
	 *            The attribute in which the error occured (Can be null for
	 *            non-attribute related errors)
	 * @param value
	 *            The value in which the error occured (Can be null for
	 *            non-value related errors)
	 * @param description
	 *            A verbal description of the error
	 * @throws IllegalArgumentException
	 *             If the parser callback returned false on this error
	 */
	...
	/**
	 * Returns true if this element is supported, false otherwise In XMLParser
	 * this always returns true, but subclasses can determine if an element is
	 * supported in their context according to its name etc. Unsupported
	 * elements will be skipped by the parser and excluded from the resulting
	 * DOM object
	 * 
	 * @param element
	...
	/**
	 * Checks if this element should be evaluated by the parser This can be
	 * overriden by subclasses to skip certain elements
	 * 
	 * @param element
	 *            The element to check
	 * 
	 * @return true if this element should be evaluated by the parser, false to
	 *         skip it completely
	 */

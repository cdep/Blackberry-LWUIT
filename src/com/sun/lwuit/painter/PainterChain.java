...
public class PainterChain implements Painter {
	...
	/**
	 * Create a new painter chain which will paint all of the elements in the
	 * chain in sequence from 0 to the last element
	 * 
	 * @param chain
	 *            the chain of components in the painter
	 */
	 ...
	 * 
	 * @param f
	 *            form on which to install the chain
	 * @param p
	 *            painter to install
	 */
	 ...
	 * 
	 * @param f
	 *            form from which to remove the chain
	 * @param p
	 *            painter to remove
	 */
	public static void removeGlassPane(Form f, Painter p) {
		...
		if (existing instanceof PainterChain) {
			...
			if (pc.chain.length == 1) {
				...
				pc.chain = newChain;
				f.repaint(); // Since setGlassPane was not called and still the
								// painter changed, we need to call repaint
			}
		}
	}
}

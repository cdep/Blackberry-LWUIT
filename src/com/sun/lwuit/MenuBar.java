...
/**
 * This class represents the Form MenuBar. This class is responsible to show the
 * Form Commands and to handle device soft keys, back key, clear key, etc...
...
public class MenuBar extends Container implements ActionListener {
	...
	static {
		// RIM and potentially other devices reinitialize the static initializer
		// thus overriding
		// the new static values set by the initialized display
		// https://lwuit.dev.java.net/issues/show_bug.cgi?id=232
	...
	protected void initMenuBar(Form parent) {
		...
		if (softkeyCount > 1 && commandBehavior < Display.COMMAND_BEHAVIOR_BUTTON_BAR) {
			...
			// special case for touch screens we still want the 3 softbutton
			// areas...
			if (thirdSoftButton) {
				setLayout(new GridLayout(1, 3));
				soft = new Button[] { createSoftButton("SoftButtonCenter"), createSoftButton("SoftButtonLeft"), createSoftButton("SoftButtonRight") };
	...
	private void updateTitleCommandPlacement() {
		...
		if (!(parent instanceof Dialog)) {
			if (commandBehavior == Display.COMMAND_BEHAVIOR_BUTTON_BAR_TITLE_BACK && parent.getTitle() != null && parent.getTitle().length() > 0) {
				...
				/*
				 * adaptTitleLayoutBackCommandStructure(); Container
				 * leftContainer = findLeftTitleContainer(); Container
				 * rightContainer = findRightTitleContainer();
				 * 
				 * if(parent.getTitle() != null && parent.getTitle().length() >
				 * 0) { if(backCommand != null) { Button b = new
				 * Button(backCommand); b.setUIID("BackCommand");
				 * leftContainer.addComponent(b); removeCommand(backCommand); }
				 * 
				 * if(Display.getInstance().isTablet()) { if(getCommandCount() >
				 * 0) { moveCommandToTitle(rightContainer, getCommand(0)); }
				 * if(getCommandCount() > 1) { moveCommandToTitle(leftContainer,
				 * getCommand(1)); } if(getCommandCount() > 2) {
				 * moveCommandToTitle(rightContainer, getCommand(2)); }
				 * if(getCommandCount() > 3 && backCommand == null) {
				 * moveCommandToTitle(leftContainer, getCommand(3)); } } else {
	...
	/**
	 * This method shows the menu on the Form. The method creates a Dialog with
	 * the commands and calls showMenuDialog. The method blocks until the user
	 * dispose the dialog.
	 */
	public void showMenu() {
		...
		if (result != cancelMenuItem) {
			...
			// menu item was handled internally in a touch interface that is not
			// a touch menu
			if (c != null) {
	...
	/**
	 * Factory method that returns the Form select Command. This Command is used
	 * when Display.getInstance().isThirdSoftButton() returns true. This method
	 * can be overridden to customize the Command on the Form.
	 * 
	 * @return Command
	 */
	...
	/**
	 * Factory method that returns the Form Menu select Command. This method can
	 * be overridden to customize the Command on the Form.
	 * 
	 * @return Command
	 */
	...
	/**
	 * Factory method that returns the Form Menu cancel Command. This method can
	 * be overridden to customize the Command on the Form.
	 * 
	 * @return Command
	 */
	 ...
	 * contained in a Dialog. This method replaces the menu ListCellRenderer of
	 * the Menu List.
	 * 
	...
	/*
	 * private void fixCommandAlignment() { if (left != null) { if
	 * (parent.isRTL()) { left.setAlignment(Label.RIGHT);
	 * right.setAlignment(Label.LEFT); } else { left.setAlignment(Label.LEFT);
	 * right.setAlignment(Label.RIGHT); } if(main != null && main != left &&
	 * main != right) { main.setAlignment(CENTER); } } }
	 */
	 ...
	 * softbuttons, this method is RTL sensitive and might reverse the result
	 * based on RTL state
	 * 
	 * @return The value of
	 ...
	 * @return Component that will result in the parent menu dialog recieving a
	 *         command event
	 */
	protected Component createCommandComponent(Vector commands) {
		...
		if (UIManager.getInstance().getLookAndFeel().isTouchMenus()) {
			...
			if (!UIManager.getInstance().isThemeConstant("touchCommandFlowBool", false)) {
				...
				if (rows > 1) {
					// try to prevent too many columns concentraiting within a
					// single row

 ...
 * very platform specific and this component tries to abstract that logic as
 * much as possible while maintaining portability.
 * <p>
 * Notice that unlike the previous implementation these implementations
 * completely hide the underlying MMAPI (or other) interface. To access it
 * directly you can extract the Player/VideoControl instance (if applicable in
 * this implementation) by invoking
 * getClientProperty("Player")/getClientProperty("VideoController"). This
 * behavior is implementation specific and we encorage implementations to expose
 * such features via the client property facility.
 * 
 * @author Shai Almog
 */
public abstract class VideoComponent extends PeerComponent {
	...
	/**
	 * Some devices might not allow extracting the native peer before the first
	 * initialization
	 * 
	 * @param nativePeer
	 *            the native peer
	...
	/**
	 * Some platforms might allow invoking the native video player for this file
	 * type
	 * 

...
/**
 * A composite pattern with {@link Component}, allows nesting and arranging
 * multiple components using a pluggable layout manager architecture. Containers
 * can be nested one within the other to form elaborate UI's.
 * 
...
public class Container extends Component {
	...
	/**
	 * Sets the lead component for this container, a lead component takes over
	 * the entire component hierarchy and receives all the events for the
	 * container hierarchy.
	 * 
	 * @param lead
	...
	/**
	 * Same as setShouldCalcPreferredSize(true) but made accessible for layout
	 * managers
	 */
	...
	public void setShouldCalcPreferredSize(boolean shouldCalcPreferredSize) {
		// minor optimization preventing repeated invokations to
	...
	/**
	 * Returns the width for layout manager purposes, this takes scrolling into
	...
	/**
	 * Invokes apply/setRTL recursively on all the children components of this
	 * container
	 * 
	 * @param rtl
	 ...
	 *             if Component is already contained or the cmp is a Form
	 *             Component
	 */
	 ...
	 * Current Component must be contained in this Container. This method
	 * returns when transition has finished.
	 * 
	 * @param current
	 ...
	 * @param growSpeed
	 *            after replace is completed the component can gradually
	 *            grow/shrink to fill up available room, set this to 0 for
	 *            immediate growth or any larger number for gradual animation.
	 *            -1 indicates a special case where no validation occurs
	 */
	...
	/**
	 * This method replaces the current Component with the next Component.
	 * Current Component must be contained in this Container. This method return
	 * immediately.
	 * 
	 * @param current
	 *            a Component to remove from the Container
	 ...
	 * Calling form.repaint() will workaround such an issue.
	 * 
	 * @param cmp
	...
	/**
	 * Flushes ongoing replace operations to prevent two concurrent replace
	 * operations from colliding. If there is no ongoing replace nothing will
	 * occur
	 */
	 ...
	 * still have a pending repaint in the queue that won't be removed. Calling
	 * form.repaint() will workaround such an issue.
	 */
	...
	public void layoutContainer() {
		// will compute the container + components and will layout the
		// components.
		if (shouldLayout) {
			shouldLayout = false;
			doLayout();
			...
			if (c != null) {
				...
				if (c.getParent() != null) {
					...
					if (f != null && f.findFirstFocusable() == c) {
						// support this use case only if the component doesn't
						// explicitly declare visible bounds
						if (r == c.getBounds()) {
	 ...
	 * 
	 * @param direction
	 *            is the direction of the navigation (Display.GAME_UP,
	 *            Display.GAME_DOWN, ...)
	 * @param next
	 *            the Component to move the scroll towards.
	 * 
	...
	boolean moveScrollTowards(int direction, Component next) {
		if (isScrollable()) {
			...
			if ((nextIntersects && !currentLarge && !edge) || (Rectangle.contains(
				...
				if (!scrollOutOfBounds) {
					scrollRectToVisible(x, y, w, h, this);
					// if after moving the scroll the current focus is out of
					// the
	...
	/**
	 * Determines the scroll increment size of this Container. This value is in
	 * use when the current foucs element within this Container is larger than
	 * this Container size.
	 * 
	 ...
	 * enabled/disabled. If the Container is disabled and a child Component
	 * changed it's state to be enabled, the child Component will be treated as
	...
	/**
	 * Returns false for the special case where a container has an
	 * opaque/flattened child that occupies its entire face
	 */
	 ...
	 * revalidate with a more visual form of animation. This method waits until
	 * the operation is completed before returning
	 * 
	 * @param duration
		...
		public boolean animate() {
			...
			case 2:
				...
				for (int iter = 0; iter < componentCount; iter++) {
					...
					// this might happen if a container was replaced during
					// animation
					if (currentCmp == null) {
						continue;
					}

...
/**
 * A parser for CSS segments or files. Note that this class is not derived from
 * XMLParser or HTMLParser as CSS format is significantly different than
 * XML/HTML
 * 
 * @author Ofir Leitner
 */
class CSSParser {
	...
	/**
	 * The supported CSS media types, this is relevant for CSS at-rules (i.e. @import
	 * and @media) The default values according to the WCSS specs the default
	 * one is "handheld" and "all" (Which is always accepted)
	 */
	 ...
	 *            The stream reader
	 * @param readNewline
	 *            true to read new lines and not break when they're found, false
	 *            otherwise
	 * @param ignoreCommas
	 *            true to ignore commas and not break when they're found, false
	 *            otherwise
	 * @param ignoreColons
	 *            true to ignore colons and not break when they're found, false
	 *            otherwise
	 * @param ignoreWhiteSpaces
	 *            true to ignore white spaces and not break when they're found,
	 *            false otherwise
	...
	private String nextToken(ExtInputStreamReader r, boolean readNewline, boolean ignoreCommas, boolean ignoreColons, boolean ignoreWhiteSpaces) throws IOException {
		...
		if (c == ';' && readNewline) { // leftover from compound operation
			c = r.readCharFromReader();
			while (((byte) c) != -1 && isWhiteSpace(c)) { // This was added
															// since after
															// reading ; there
															// might be some
															// more white
		...
		while (((byte) c) != -1 && ((!isWhiteSpace(c)) || (segment != '\0') || (ignoreWhiteSpaces)) && c != ';' && ((c != ':') ||
			...
			if ((segment == '\0') && (c == '/')) { // comment start perhaps, if
													// inside brackets - ignore
				c = handleCSSComment(r);
			}

			if ((c == '}' || c == '{' || c == '*') && (segment == '\0')) { // enter
																			// only
																			// if
																			// not
																			// in
	 ...
	 * @param selectors
	 *            A vector containing grouped selectors to copy the attributes
	 *            to
	 * @param addTo
	...
	private void copyAttributes(CSSElement element, Vector selectors, HTMLElement addTo) {
		...
		for (Enumeration e = selectors.elements(); e.hasMoreElements();) {
			...
			while (selector.getNumChildren() > 0) { // This makes sure we get
													// the last nested selector
	...
	/**
	 * Returns true if the specified CSS media type is unsupported, false
	 * otherwise
	 * 
	 * @param media
	 *            A string identifying the media type (i.e. "handheld")
	 * @return true if the specified CSS media type is uspported, false
	 *         otherwise
	 */
	 ...
	 * @return true if one of the supported media types is denoted, false
	 *         otherwise
	 */
	 ...
	 * 
	 * @param token
	 *            The string including the url and media of the import at-rule
	 *            (example: url("mycss.css") handheld,tv;
	...
	/**
	 * Handles a media at-rule segment. This method checks if the media type
	 * specified in the media at-rule is supported, if it does it returns only
	 * the media segment as a separate stream, otherwsie it returns null
	 * 
	 * @param isr
	 *            The stream representing the CSS
	 * @param encoding
	 *            The encoding string
	 * @param htmlC
	 *            The HTMLComponent
	 * @return An input stream with the relevant media segment or null if the
	 *         media is not supported
	 * @throws IOException
	 *             on input stream failure
	 */
	private ExtInputStreamReader getMediaSegment(ExtInputStreamReader r, String encoding, HTMLComponent htmlC) throws IOException {
		...
		while ((((byte) c) != -1) && (c != '{')) { // Find the first { that
		...
		if (match) {
			...
			if (segmentReader == null) { // either no encoding, or encoding
											// failed
	...
	/**
	 * Reads a CSS file/stream and returns the tokenized CSS as a single level
	 * element tree with the root appearing as a "style". This method is called
	 * upon finding linked/external CSS and embedded CSS segments. It handles
	 * at-rules such as import/charset/media and forwards relevant segments to
	 * the parseCSS method
	 * 
	 * @param isr
	 *            The Reader representing the stream
	 * @param is
	 *            The InputStream representing the stream (We need it too, in
	 *            case encoding changes and we need to create another
	 *            InputStreamReader)
	 * @param htmlC
	 *            The HTMLComponent
	 * @param pageURL
	 *            For external CSS the URL of the CSS, for embedded - null
	 * @return A CSSElement containing all selectors found in the stream as its
	 *         children
	 * @throws IOException
	 *             on input stream failure
	 */
		...
		while (token.startsWith("@")) {
			if (token.equals("@import")) {
				...
				if (url != null) {
					...
					if (docInfo != null) {
						htmlC.getThreadQueue().addCSS(docInfo.convertURL(url), encoding); // Referred
																							// CSS
																							// "inherit"
																							// charset
																							// from
						...
						if (DocumentInfo.isAbsoluteURL(url)) {
							htmlC.getThreadQueue().addCSS(url, encoding); // Referred
																			// CSS
																			// "inherit"
																			// charset
																			// from
				...
				if (is != null) { // @charset applies only to external style
									// sheet, and the inputstream is null for
									// embedded CSS segments
					try {
	 ...
	 * Reads a CSS file/stream and returns the tokenized CSS as a single level
	 * element tree with the root appearing as a "style". This method is called
	 * either directly on style attributes.
	 * 
	 * @param r
	 *            The stream reader containing the CSS segment
	 * @param htmlC
	 *            The HTMLComponent
	 * @return A CSSElement containing all selectors found in the stream as its
	 *         children
	...
	/**
	 * Reads a CSS file/stream and returns the tokenized CSS as a single level
	 * element tree with the root appearing as a "style".
	 * 
	 * @param r
	 *            The stream reader containing the CSS segment
	 * @param htmlC
	 *            The HTMLComponent
	 * @param addTo
	 *            the master CSSElement to add the selectors to (or null to open
	 *            a new one_
	 * @param firstToken
	 *            A first toekn to process, or null if none
	 * @return A CSSElement containing all selectors found in the stream as its
	 *         children
	 * @throws IOException
	 *             on input stream failure
	...
	CSSElement parseCSS(ExtInputStreamReader r, HTMLComponent htmlC, CSSElement addTo, String firstToken) throws IOException {
		...
		boolean grouping = false; // Grouping is when selector are grouped, i.e.
									// h1,h2,h3 { ... }
		boolean childSelector = false; // when 'a > b' appears it is a child
										// seelctor, meaning only 'b' that is a
										// direct child of 'a' (unlike 'a b'
										// which is a descendant selector)
		boolean siblingSelector = false; // when 'a + b' appears it is a sibling
											// seelctor, meaning only 'b' that
											// is adjacent to 'a'
		...
		while (true) {
			...
			if (selectorMode) {
				// Checks for grouped selectors, note that due to spacing the
				// comma can either appear as a separate token, or at the start
				// of a token or at its end
				// All these scenarios are checked in the following lines of
				// code.
				if (",".equals(token)) {
					grouping = true;
					continue;
				...
				if (grouping) {
					if (token.endsWith(",")) {
						...
						grouping = false; // there was no comma at the end, so
											// next time it is not a grouped
											// element (unless a comma will be
											// detected as the next token or the
				...
				for (int iter = 0; iter < CSSElement.CSS_SHORTHAND_ATTRIBUTE_LIST.length; iter++) {
					if (CSSElement.CSS_SHORTHAND_ATTRIBUTE_LIST[iter].equals(token)) {
						...
						// This array is used for collatable attributes - the
						// values can't be set as they are read, first we need
						// to see how many values appear and set accordingly
						...
						// The following assigns the collatable attributes
						// according to CSSElement.CSS_COLLATABLE_ORDER
				...
				// if this is a "regular" css attribute is it one of the
				// supported attributes
				if (!compoundToken) {
					// We ignore commas when collecting a value, since it can be
					// for example: font-family:arial,tahoma,sans-serif etc.
					// We also ignore spaces in font-family / access key since
					// the value can be: arial, tahoma / send * , #
					...
					if (result != -1) {
						...
						// while(nextToken(r, false, false,false) != null &&
						// !newline) {}
						while (nextToken(r, false, false, false, false) != null) {
						} // TODO - is newline truly unnecessary ? + what if
							// that happens in the end of the file - do we get
	...
	/**
	 * Adds the specified value to the specified selector as a value to the
	 * shorthand attribute whose index is specified This methods deals with the
	 * complexity of adding values for shorthand attributes, since they can be
	 * specified in any order It also handles multiple-shorthand levels such as
	 ...
	 * @param selector
	 *            The selector to add the attribuet to
	 * @return true if succeeded to add, false otherwise (for example invalid
	 *         value)
	 */
	private boolean addShorthandAttribute(String value, int shorthandAttr, CSSElement selector) {
		...
		for (int i = 0; i < CSSElement.CSS_SHORTHAND_ATTRIBUTE_INDEX[shorthandAttr].length; i++) {
			...
			if (attrIndex >= CSSElement.CSS_STYLE_ID_OFFSET) {
				...
				if (!selector.isAttributeAssigned(attrIndex)) { // Only check if
																// the attribute
																// wasn't set
																// yet
	...
	/**
	 * Adds the specified value to the specified selector as a value to the
	 * shorthand and collatable attribute whose index is specified This is
	 * called from addShorthandAttribute when a shorthand attribute maps to a
	 * collatable attribute Note that while usually collatable attributes can
	 * have 1-4 values, and are mapped according to
	 * CSSElement.CSS_COLLATABLE_ORDER When they are specified as part of a top
	 * shorthand attribute, only one value can be specified and it is copied to
	 * all base attributes. For example, While the definition 'border-width: 5px
	 * 10px' will set the vertical border width to 5 and the horizontal to 10,
	 * One cannot specify: 'border: 5px 10px solid red' - but rather has to
	 * specify only one value that will be set as the width for all sides.
	 * 
	 * @param value
	 *            The attribute's value
	 * @param shorthandAttr
	 ...
	 *            The attribute value
	 * @param htmlC
	 *            The HTMLComponent (To obtain the ParserCallback)
	...
	/**
	 * A helper method that handles reporting of CSS errors to the
	 * ParserCallback (if available)
	 * 
	 * @param errorCode
	 *            The error code as returned by the CSSElement.addAttribute
	 *            methods
	 * @param selector
	 *            The selector we're working on
	 * @param attributeName
	...
	/**
	 * A decorator for Reader that adds the ability to "unread" a character This
	 * makes parsing easier, and is used for CSS parsing.
	 * 
	 * @author Ofir Leitner
	 */
	class ExtInputStreamReader {
		...
		/**
		 * "Unreads" a character from the stream by placing it in a member
		 * variable to be later retreived by readCharFromReader, used by the CSS
		 * Parser
		 * 
		 * @param c
		...
		/**
		 * Reads the next character from the input stream, used by the CSS
		 * Parser If there's an "unread" character in the buffer it is returned
		 * (and no reading is done to the actual stream)
		 * 
		 * @param r
		 *            The stream reader

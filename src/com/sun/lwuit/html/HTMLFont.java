...
/**
 * This class is a wrapper class for a LWUIT font object, that keeps additional
 * info on the font as required for HTML rendering by HTMLComponent.<br>
 * The info kept on the font is its family, size and style. These attributes are
 * available in LWUIT only for system fonts, but here they are made available
 * for bitmap fonts using a key to describe the font (See HTMLComponent.addFont)<br>
 * <br>
 * In addition this class keeps track of "counterpart" fonts for this font in a
 * space of 4 attributes: BOLD, ITALIC, BIG and SMALL.<br>
 * For example a bold font is the BOLD counterpart of a plain one. A font with
 * size 16 is the BIG counterpart of a font the size of 12 - provided that all
 * other attributes are the same (i.e. an arial.16 font is not the BIG
 * counterpart of a courier.12 etc.)<br>
 * 
 * @author Ofir Leitner
 */
class HTMLFont {
	...
	/**
	 * The following tags are tags that mainly define the style of their
	 * content, and one of the things they can change is the font
	 * 
	 */
	...
	/**
	 * The static segment sets up the SPECIAL_FONT_TAGS vector with values from
	 * the SPECIAL_FONT_TAGS_ID array. This vector is used for lookup later on.
	 */
	static {
	 ...
	 * 
	 * @param fontKey
	 *            The key for this font (See class definition)
	 * @param font
	 *            The actual LWUIT font object (Can be either system or bitmap
	 *            font)
	 */
	HTMLFont(String fontKey, Font font) {
		this.font = font;
		systemFont = (font.getCharset() == null); // A systemfont has no
													// "supported" charset
		...
		if (isSystemFont()) {
			...
			if (fontKey != null) { // fontKey can be null in the case this is a
									// default font (Though even then it is
									// recommended to provide a font key so the
									// engine can use this font in other cases)
	...
	/**
	 * Checks if the given font is the counterpart of the current form in the
	 * sense of the given attribute
	 * 
	 * @param attribute
	 *            The attribute, can be one of BOLD, ITALIC, BIG or SMALL
	 * @param font
	 *            The font to check
	...
	/**
	 * Utility method that first checks that neither font family is null and
	 * then compares them
	 * 
	 * @param font
	 *            The other font to check
	 * @return true if the family of the compared font and this font are
	 *         identical (and both are not null), false otherwise
	 */
	...
	/**
	 * Returns the counterpart font for this font in the given attribute. This
	 * method either creates it or fetches it if it was already set. It handles
	 * both system and bitmap fonts. Note that a counterpart font can be the
	 * font itself if no other suitable font was found.
	 * 
	 * @param attribute
	 *            The requested counterpart attribute
	 * @return the counterpart font for this font in the given attribute
	 */
	HTMLFont getCounterpartFont(int attribute) {
		if ((systemFont) && (counterpartFonts[attribute] == null)) { // Note
																		// that
																		// bold
																		// counterpart
																		// for a
			...
			case ITALIC:
				...
				break;
			case BIG:
				// If current form's size is medium then the big counterpart is
				// large, if it's small then medium
				...
				break;
			case SMALL:
				// If current form's size is medium then the small counterpart
				// is small, if it's large then medium
	...
	/**
	 * Sets the specified font as the counterpart of this font in the given
	 * attribute
	 * 
	 * @param attribute
	 *            The attribute in which the specified font is the counterpart
	 *            of this font.
	 * @param counterpartFont
	 *            The counter part font
	 */
	 ...
	 * 
	 * @return STYLE_PLAIN, STYLE_BOLD or STYLE_ITALIC or STYLE_BOLD |
	 *         STYLE_ITALIC
	 */
	int getStyle() {
		return style;
	...
	/**
	 * Returns the font size. For system fonts this would be either SIZE_SMALL,
	 * SIZE_MEDIUM or SIZE_LARGE. For bitmap font this would be the exact size
	 * as defined in the font key.
	 * 
	...
	int getSizeInPixels() {
		if (systemFont) {
			return font.getHeight() - 2; // estimation - usually a font's hright
											// is bigger in a few pixels than
											// the size
		} else {
			return size;
		}

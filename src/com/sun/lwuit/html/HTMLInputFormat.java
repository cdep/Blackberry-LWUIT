...
/**
 * This class implements HTML's input format restrictions. These restrictions
 * can be provided in the FORMAT attribute of the INPUT tag and are relevant for
 * textfields. However as the FORMAT tag was deprectaed it is more standard to
 * supply them in the '-wap-input-format' of the input field CSS or Style.
 * 
 * @author Ofir Leitner
 */
class HTMLInputFormat {
	 ...
	 * 
	 * @param formatString
	 *            The string representing the format defintion (As taken from
	 *            the HTML/CSS)
	 * @return An HTMLInputFormat object containing all the constraints or null
	 *         if there are none or if the string is invalid
	 */
	...
	/**
	 * A private constructor, to obtain an HTMLInputFormat object use
	 * HTMLInputFormat.getInputFormat
	 * 
	 * @param formatString
	 *            The string representing the format defintion (As taken from
	 *            the HTML/CSS)
	 ...
	 * After invoking this method the returned TextArea should be used as
	 * restrictions are made sometimes on a new object. In case this is a
	 * TextField, this method will also set the input modes as needed.
	 * 
	 * @param ta
	 *            The TextArea to apply the constraints on.
	 * @return An instance of TextArea (Either the given one or a new one) with
	 *         the constraints.
	 */
	TextArea applyConstraints(TextArea ta) {
		...
		if (ta instanceof TextField) {
			...
			if (((widestConstraint & FormatConstraint.TYPE_SYMBOL) == 0) && ((widestConstraint & FormatConstraint.TYPE_ANY) == 0)) { // No
																																		// symbols
																																		// allowed
	...
	/**
	 * Verifies that the given String conforms to the constraints represented by
	 * this object.
	 * 
	 * @param str
	...
	boolean verifyString(String str) {
		...
		if (!str.equals("")) {
			...
			for (; e.hasMoreElements();) {
				...
				if (constraint.count == FormatConstraint.COUNT_EXACTLY_ONE) {
					...
					while ((i < str.length()) && (charNum < constraint.count)) {
						if (!verifyChar(c, constraint.type)) {
							// Note that if a char doesn't apply to the current
							// constraint it might conform to the next one
							// (since we are in an
							// "up to X chars" count, so perhaps from the
							// current constraint segment there are less chars,
							// this is why we don't
							// fail, but break out, and let the char be compared
							// against the next constraint
							break;
						}
						i++;
						charNum++;
			...
			if (i < str.length()) { // Chars left that are not covered by any
									// constraint
				return false;
			}
		}

		// All chars covered, but more perhaps more constraints are still
		// available
		// They will be checked to see if anyone forces to have one more char
		// and if so fail the verification
	...
	/**
	 * Verifies the given character. THis method is used by verifyString on each
	 * char
	 * 
	 * @param c
	 *            The char to verify
	 ...
	 * @return true if the char conforms to the given constraint, false
	 *         otherwise
	 */
	...
	/**
	 * This class reprensents a single constraint on an input field The
	 * HTMLInputFormat breaks down the format string into 1 or more
	 * FormatConstraint objects
	 * 
	 * Each constraint is represented by a type and a count. The type indicated
	 * which typs of characters are allows and can be one or more of the TYPE_*
	 * constants ORed together. The count is either a number that indicates we
	 * allow up to this count or one of the COUNT_* constants (See below)
	 * 
	 * @author Ofir Leitner
	 */
	class FormatConstraint {
		...
		/**
		 * A constant representing that there must be one and one only of this
		 * type
		 */
		...
		/**
		 * The constructor which converts the count string into an integer or
		 * one of the COUNT_* constants above
		 * 
		 * @param type
		 *            The constraint type (one or more of the TYPE_* constants
		 *            ORed together)
		 * @param countStr
		 *            A string representing the constraint count
		 */
		 ...
		 * 
		 * @return a printout of a user-friendly string describing this
		 *         constraint
		 */
		public String toString() {
			String str = "";

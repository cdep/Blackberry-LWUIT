...
class HTMLForm {
	 ...
	 * 
	 * @param htmlC
	 *            The HTMLComponent containing this form
	 * @param action
	 *            The action of this form (the URL)
	 * @param method
	 *            The method of this form (get/post)
	 */
	 ...
	 * 
	 * @param value
	 *            The reset button display name, if null or empty
	 *            DEFAULT_RESET_TEXT will be assigned to it
	 ...
	 * 
	 * @param name
	 *            The submit command name (i.e. it's NAME attribute, or null if
	 *            none specified)
	 * @param value
	 *            The submit button display name, if null or empty
	 *            DEFAULT_SUBMIT_TEXT will be assigned to it
	 * @return The reset command
	 */
	...
	/**
	 * Adds an input field to the form, note that unlike adding to a LWUIT form
	 * here the components are added logically only to query them for their
	 * value on submit.
	 * 
	 * @param key
	 *            The input field's name/id
	 * @param input
	 *            The field itself (either the component or the value for a
	 *            hidden field)
	 * @param defaultValue
	 *            The default value of the field (or null if none)
	 */
	void addInput(String key, Object input, String defaultValue) {
		if (defaultValue != null) { // Default values are added even before the
									// key is checked, since even if the input
									// component is unnamed, the form still
									// needs to be able to reset it
	 ...
	 *            The textarea/field
	 * @param inputFormat
	 *            An input format verifier (or null if none)
	 */
	 ...
	 *            The TextArea
	 * @param ok
	 *            true if can be left empty, false otherwise
	 */
	 ...
	 * 
	 * @param input
	 *            The input field to set the default value to
	 * @param defaultValue
	 *            The default value
	 */
	...
	/**
	 * Adds the specified CheckBox to the form. Note that unlike adding to a
	 * LWUIT form here the components are added logically only to query them for
	 * their value on submit.
	 * 
	 * @param key
	 *            The CheckBox's name/id
	 * @param cb
	 *            The CheckBox
	 * @param value
	...
	/**
	 * Adds the specified RadioButton to the form. Note that unlike adding to a
	 * LWUIT form here the components are added logically only to query them for
	 * their value on submit.
	 * 
	 * @param key
	...
	/**
	 * Called when the a form submit is needed. This querys all form fields,
	 * creates a URL accordingly and sets it to the HTMLComponent
	 */
	void submit(String submitKey, String submitVal) {
		if (action == null) {
			return;
		}
		boolean error = false; // If this is turned to true anywhere, the form
		...
		if (comps.size() > 0) {
			...
			for (Enumeration e = comps.keys(); e.hasMoreElements();) {
				...
				if (input instanceof String) { // hidden
					...
					params += key + "=" + value + "&";
				} else if (input instanceof Hashtable) { // checkbox /
															// radiobutton
				...
				} else if (input instanceof TextArea) { // catches both
														// textareas and text
														// input fields
					...
					if (HTMLComponent.SUPPORT_INPUT_FORMAT) {
						boolean ok = false;
						if (text.equals("")) { // check empty - Note that
												// emptyok/-wap-input-required
												// overrides input format
						...
						if ((!error) && (!ok)) { // If there's already an error
													// or it has been cleared by
													// the emptyOK field, no
													// need to check
				...
				} else if (input instanceof ComboBox) { // drop down lists
														// (single selection)
					...
					} // if not - value may be an OPTGROUP label in an only
						// optgroup combobox
				} else if (input instanceof MultiComboBox) { // drop down lists
																// (multiple
																// selection)
		...
		// Add the submit button param, only if the key is non-null (unnamed
		// submit buttons are not passed as parameters)
		if (submitKey != null) {
			if (params == null) {
				params = "";
			}
	...
	/**
	 * Called when a form reset is needed and resets all the form fields to
	 * their default values.
	 */
	void reset() {
		for (Enumeration e = defaultValues.keys(); e.hasMoreElements();) {
			Object input = e.nextElement();
			if (input instanceof TextArea) { // catches both textareas and text
												// input fields
	 ...
	 * 
	 * @param button
	 *            The button to set (CheckBox/RadioButton)
	 * @param checked
	...
	class NamedCommand extends Command {
		...
		HTMLForm htmlForm;
		String key; // The key is taken from the NAME attribute of the INPUT
					// tag. It is called key to avoid mixup with the command's
					// name (i.e. display name which is in fact 'value')
					// For reset buttons the name will always be null, as it is
					// not relevant. Submit buttons can be named with the NAME
					// attribute, resulting in their name and value passed as a
					// parameter upon form submission

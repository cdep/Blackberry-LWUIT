...
/**
 * HTMLTable extends LWUIT's Table and overrides some it's rendering and
 * constaints methods do adapt to HTMLTableModel
 * 
 * @author Ofir Leitner
 */
class HTMLTable extends Table {
	...
	/**
	 * Constant denoting that only inner borders rows should be drawn, only when
	 * seperating between table segments The groups/segments feature is not
	 * available to Table, but is available internally to HTMLTable
	 */
	static final int INNER_BORDERS_GROUPS = 4;

	/**
	 * Constructs an HTMLTAble with the given model, which has to be
	 * HTMLTableModel
	 * 
	 * @param model
	 *            The HTMLTableModel for this Table
	 */
	...
	/**
	 * This method is overriden to return the component that is contained in the
	 * HTMLTableModel (Since our model contains the actual component and does
	 * not store data that can be rendered using toString we can't use the
	 * original createCell method)
	 * 
	 * @param value
	...
	protected Component createCell(Object value, int row, int column, boolean editable) {
		...
		if (value instanceof Component) {
			cmp = (Component) value;
			// TODO - table cells styling - needs to propogate downwards since
			// the cell is usually a container, on the other hand can't wipe out
			// the style of the container's components - TBD
	...
	/**
	 * This method is overriden to fetch the constraints from the associated
	 * HTMLTableModel and converts it to TableLayout.Constraint
	 * 
	 * @param value
	...
	protected Constraint createCellConstraint(Object value, int row, int column) {
		...
		if (cConstraint == null) { // Can be null for cells that were
									// "spanned over"
	...
	/**
	 * Overrides Table.setInnerBorderMode to accept also the INNER_BORDER_GROUPS
	 * constant that implement the 'groups' value for the 'rules' attribute on
	 * the 'table' tag.
	 * 
	 * @param innerBorder
	 *            The inner border mode, one of the INNER_BORDER_* constants
	 */

 ...
 * This class is constructed internally by HTMLComponent and HTMLForm and is
 * sent to the RequestHandler. It is intended for the RequestHandler to use and
 * update (For example update encoding according to the HTTP response, update
 * URL in case of a redirect etc.)
 * 
 * @author Ofir Leitner
 */
public class DocumentInfo {
	 ...
	 *            The parameters
	 * @param postRequest
	 *            true if this is a POST request, false otherwise (i.e. GET)
	 */
	...
	public void setUrl(String url) {
		...
		if (index == -1) {
			...
			if (index != -1) {
				protocol = pageURL.substring(0, index + 1); // The protocol will
															// be http: , ftp:
															// (without the //)
			}
			...
			if (index != -1) {
				...
				hostURL = pageURL;
				if (!protocol.startsWith("file")) { // for file the following is
													// not relevant as
													// "0file:///filename" means
													// the base url is file:///
					setBaseURL(pageURL + "/"); // The url was a domain without
												// folder and missing a trailing
												// / - i.e. http://www.sun.com
				}
	 ...
	 * TYPE_CSS
	 * 
	 * @return the expected content type, one of TYPE_HTML, TYPE_IMAGE or
	 *         TYPE_CSS
	 */
	 ...
	 * The differentiation is important to handle cases in which the
	 * HTMLComponent expects one type but the URL is has a resource of another
	 * type
	 * 
	 * @param requestType
	 *            the requestType to set, one of TYPE_HTML, TYPE_IMAGE or
	 *            TYPE_CSS
	 */
	 ...
	 * 
	 * @param params
	 *            The request paramters to set, should be as a percentage
	 *            encoded string
	 */
	...
	/**
	 * Sets the document encoding (This can be determined via the charset
	 * attribute in the response)
	 * 
	 * @param encoding
	 *            the encoding to set. It is recommended to use the ENCODING_*
	...
	static boolean isAbsoluteURL(String url) {
		return (url.substring(0, Math.min(10, url.length())).indexOf("://") != -1); // Absolute
																					// URL
																					// -
																					// check
																					// only
	...
	String convertURL(String url) {
		...
		if (isAbsoluteURL(url)) {
			...
			if (url.startsWith("//")) { // Take just the protocol from the
										// original url
				return protocol + url;
				...
				while ((url.length() > 0) && (url.charAt(0) == '.')) {
					if (url.startsWith("./")) { // Same folder
						...
						url = "";
						back++;
					} else { // either "." or something else not-understandable
								// (resting the URL gets us out of the loop)
						url = "";
					}
				}
				String folder = getBaseURL().substring(hostURL.length() + 1); // +1
																				// in
																				// order
																				// not
																				// to
				...
				while ((back > 0) && (folder.length() > 0)) {
					...
					if (index == -1) {
						...
						folder = folder.substring(0, index + 1); // +1 to
																	// include
																	// the '/'
					}
				}
	...
	/**
	 * Sets the base URL for this document. Usually this is deduced
	 * automatically from the page URL, but in some cases this is different, for
	 * example when an HREF attribute is provided in the BASE tag
	 * 
	 * @param baseURL
	 *            the baseURL to set
	 */

...
/**
 * The CSSElement class defines a single CSS element with its attributes and
 * children. It extends Element and adds to it certain CSS-specific methods.
...
class CSSElement extends HTMLElement {
	...
	/**
	 * Defines the Dots-per-Inch - used when CSS length values are denoted in
	 * units like inches (in), cm, mm, points (pt) etc. This can be customized
	 * per device in build time. However since these units are not mandatory in
	 * WCSS so even if this is not accurate no harm done. In any case, always
	 * prefer using the supported length units: px, em, ex
	 */
	 ...
	 */
	private final static int CSS_LENGTH_FACTORS[] = { 1, 2, 1, DPI, DPI / 72, DPI / 6, (int) (DPI / 2.54 / 10), (int) (DPI / 2.54) }; // unit
																																		// factors
																																		// for
																																		// {"px","em","ex","in","pt","pc","mm","cm"};
	...
	private final static String CENTER_STR = "center"; // Since this value is
														// used in multiple CSS
														// properties it is
														// defined as a constant
	private static final String CSS_NONE = "none"; // Since this value is used
													// in multiple CSS
													// properties it is defined
													// as a constant

	/**
	 * A constant representing both the attribute value of 'font-variant' and
	 * the font name that should be given to small-caps fonts Since LWUIT/J2ME
	 * doesn't support small-caps fonts, when a small-caps font varinat is
	 * requested the font-family is changed to "smallcaps" which should be
	 * loaded to HTMLComponent and the theme as a bitmap font If no smallcaps
	 * font is found at all, then the family stays the same, but if even only
	 * one is found - the best match will be used.
	 */
	...
	// Constants defining the approximate sizes of a small/medium/large font
	// (This can be tweaked per platform)
	...
	// Font style strings and their corresponding values, note that oblique is
	...
	// Note that since we have only two levels of boldness (plain, bold) -
	// bolder always means bold, and lighter always means plain
	...
	static final int CSS_PAGEURL = CSS_STYLE_ID_OFFSET + 50; // This attribute
																// is not a CSS
																// attribute,
																// but rather
																// for internal
	...
	/**
	 * An array containing the allowed strings for CSS attributes. Note that
	 * these strings are allowed in addition for the allowed values according to
	 * the attribute types. Also, unlike the allowed string in Element that are
	 * matched per type, here in CSSElement each line matches an attribute (and
	 * not its type). This is because of the great variance of allowed strings
	 * in CSS that are really per attribute and not per type.
	 */
	private static final String[][] CSS_ALLOWED_STRINGS = {
			...
			{ "left", CENTER_STR, "right" }, // TYPE_CSS_LENGTH_OR_PERCENTAGE,
												// //CSS_BACKGROUND_POSITION_X
			{ "top", CENTER_STR, "bottom" }, // TYPE_CSS_LENGTH_OR_PERCENTAGE,
			...
			{ "inline", "block", "list-item", CSS_NONE, "-wap-marquee" }, // TYPE_NMTOKENS,
																			// //CSS_DISPLAY
			...
			{ CSS_NONE, "disc", "circle", "square", "decimal", "upper-alpha", "lower-alpha", "upper-roman", "lower-roman" }, // TYPE_NMTOKENS,
																																// //CSS_LIST_STYLE_TYPE
			...
			{ "underline", "line-through", CSS_NONE, "overline" }, // TYPE_NMTOKENS,
																	// //CSS_TEXT_DECORATION
																	// //
																	// 'blink'
																	// is not
			...
			{ CSS_NONE, "uppercase", "lowercase", "capitalize" }, // TYPE_NMTOKENS,
																	// //CSS_TEXT_TRANSFORM
	...
	/**
	 * This very high integer value is added to a numeric value stored in
	 * attrVals to denote that it is a percentage value
	 */
	static final int VAL_PERCENTAGE = 1 << 20;

	/**
	 * This very high integer value is added to a numeric value stored in
	 * attrVals to denote that it is a value in the CSS 'ex' unit (EX means half
	 * of the font size)
	 */
	...
	/**
	 * The values of each of the allowed strings. When adding an attribute which
	 * its value is an allowed string, the attribute value is set according to
	 * this array. For attributes that have no allowed strings (i.e. accpeting
	 * numeric values solely) this is not reelvant. For attributes that do have
	 * allowed strings and 'null' as the values, it means that the first string
	 * in the allowed list will be translated to 0, the second to 1 etc. The
	 * purpose of converting strings in strings that have non-null values, is to
	 * convert them to a value representing the essence of the string (FOr
	 * example thin in border is 1) And for those with null values, is to avoid
	 * having to parse strings all the time (so they are conerted to 0,1...,n)
	 */
	private static final int[][] CSS_ALLOWED_STRINGS_VALS = {
			null, // TYPE_COLOR, // CSS_BACKGROUND_COLOR
			null, // TYPE_CSS_URL, //CSS_BACKGROUND_IMAGE
			BG_REPEAT_VALS, // TYPE_NMTOKENS, //CSS_BACKGROUND_REPEAT
			null, // {"fixed","scroll"}, //TYPE_NMTOKENS,
					// //CSS_BACKGROUND_ATTACHMENT
			BG_POS_PERCENTAGE, // TYPE_CSS_LENGTH_OR_PERCENTAGE,
								// //CSS_BACKGROUND_POSITION_X
			BG_POS_PERCENTAGE, // TYPE_CSS_LENGTH_OR_PERCENTAGE,
								// //CSS_BACKGROUND_POSITION_Y
			...
			null, // BORDER_STYLE_VALS, //TYPE_NMTOKENS,
			...
			null, // TYPE_CSS_URL, //CSS_LIST_STYLE_IMAGE
			null, // LIST_STYLE_POSITION_VALS, //TYPE_NMTOKENS,
					// //CSS_LIST_STYLE_POSITION
			null, // LIST_STYLE_TYPE_VALS, //TYPE_NMTOKENS,
					// //CSS_LIST_STYLE_TYPE
			null, // TYPE_CSS_LENGTH_OR_PERCENTAGE, //CSS_MARGIN_TOP
			null, // TYPE_CSS_LENGTH_OR_PERCENTAGE, //CSS_MARGIN_RIGHT
			null, // TYPE_CSS_LENGTH_OR_PERCENTAGE, //CSS_MARGIN_BOTTOM
			...
			null, // CSS_WORD_SPACING
			{ 1 }, // CSS_LINE_HEIGHT //normal means 100% of the font height (or
					// 1 * font height)
			null, // CSS_MIN_WIDTH
			null, // CSS_MAX_WIDTH
			null, // CSS_MIN_HEIGHT
	...
	/**
	 * The attrVals array holds the numeric values of most of the attributes of
	 * this element. This is used for quick access, instead of parsing strings
	 * in the attributes Hashtable Values are set during addAttribute.
	 */
	int[] attrVals = new int[LAST_CSS_PROPERTY_INDEX + 1];

	/**
	 * A string array with the attributes supported by the WCSS spec Note that
	 * shorthand attributes appear in a different array
	 */
	...
	/**
	 * A string array containing all supported shorthand attributes Shorthand
	 * attrributes are mapped to multiple CSS base attributes are used to define
	 * several CSS attributes at once
	 */
	...
	/**
	 * A boolean array defining which of the CSS shorthand attributes when
	 * defined with less than the expected number of values copy the values
	 * defined to the remaining undefined attributes For example, margin expects
	 * 4 values (top/right/bottom/left), but can also be defined with 1 value
	 * that will be used for all 4, or with 2 values which will be used one for
	 * the vertical margins and the other for the horizontal etc.
	 */
	...
	/**
	 * A constant defining the offset in which the base CSS attribute relating
	 * to the TOP appears in the shorthand attribute index
	 */
	private static final int CSS_TOP = 0;

	/**
	 * A constant defining the offset in which the base CSS attribute relating
	 * to the RIGHT appears in the shorthand attribute index
	 */
	private static final int CSS_RIGHT = 1;

	/**
	 * A constant defining the offset in which the base CSS attribute relating
	 * to the BOTTOM appears in the shorthand attribute index
	 */
	private static final int CSS_BOTTOM = 2;

	/**
	 * A constant defining the offset in which the base CSS attribute relating
	 * to the LEFT appears in the shorthand attribute index
	 */
	private static final int CSS_LEFT = 3;

	/**
	 * A map defining the rules by which values of collatable shorthand
	 * attributes are assigned to the base attributes This map is according to
	 * the CSS specs
	 */
	static final int[][][] CSS_COLLATABLE_ORDER = {
			...
			{ CSS_TOP, CSS_BOTTOM }, // The first value is set to the top and
										// bottom
			{ CSS_LEFT, CSS_RIGHT } // The second value is set to the left and
									// right
	},
			...
			{ // When four values are specified - each is set to the appropriate
				// side
			{ CSS_TOP },
			{ CSS_RIGHT },
			{ CSS_BOTTOM },
	...
	// The following constants are used in CSS_SHORTHAND_ATTRIBUTE_INDEX when a
	// shorthand attribute translates to a second level of shorthand attributes
	// For example 'border' which translated to width/style/color. The values of
	// the constants denote their location in the CSS_SHORTHAND_ATTRIBUTE_INDEX
	// map
	...
	/**
	 * A map of CSS shorthand attributes to their corresponding base attributes
	 * (or to other shorthand attributes)
	 */
	...
	private int selectorSpecificity = -1; // A value used to determine how
											// specific this selector is - the
											// more the selector is specific
											// (i.e. id > class > tag) the more
											// it overrides other less specific
											// selectors
	private String selectorId = null; // The selector's ID (if it's an ID
										// selector, i.e. '#someid')
	private String selectorClass = null; // The selector's class (if it's a
											// class selector i.e. '.someclass')
	private String selectorTag = null; // The selector's tag (if it's a tag
										// selector - i.e. 'div')

	private int selectorPseudoClass = 0;

	/**
	 * This is true if this selector is a descendant selector - and false if it
	 * is a child selector. For example is this selector is h2, and was derived
	 * from a 'h1 h2' combo it will be true - but if derived from 'h1 > h2' will
	 * be false. Note - for the first selector in the combo (h1 in the above
	 * example) this value is not significant and never read.
	 */
	boolean descendantSelector;

	/**
	 * This is true if this selector is a sibling (adjacent) selector - and
	 * false otherwise
	 */
	boolean siblingSelector;

	/**
	 * A vector holding any attribute selections (For example p[att=value]) This
	 * will be null in non-attribute selectors.
	 */
	...
	/**
	 * The values per each string in PSEUDO_CLASSES_STRINGS Note that 'hover' is
	 * mapped to PC_FOCUS since we don't support hover on mobile.
	 */
	final static int[] PSEUDO_CLASSES_VALS = { PC_FOCUS, PC_FOCUS, PC_ACTIVE, PC_LINK, PC_VISITED, PC_BEFORE, PC_AFTER, PC_FIRST_CHILD }; // Since
																																			// we
																																			// don't
																																			// have
																																			// hover
	 ...
	 * 
	 * @param units
	 *            The string with CSS units (can be either percentage or
	 *            px/em/ex and other sufixes from CSS_LENGTH_SUFFIX)
	 ...
	 *            px/em/ex and other sufixes from CSS_LENGTH_SUFFIX)
	 * @param defaultSuffix
	 *            The suffix to use as default if none is specified
	 * 
	 * @return The value in pixels/font-multipliers/percentage
	 */
	static int convertUnitsOrPercentage(String units, int defaultSuffix) {
		...
		if (units.charAt(units.length() - 1) == '%') {
			percentage = true;
			defaultSuffix = LENGTH_SUFFIX_PX; // for percentage, the number
												// without the % is sent and
												// needs to stay as is
												// (factor=1)
	 ...
	 * 
	 * @param units
	 *            The string with CSS units (can be px/em/ex and other sufixes
	 *            from CSS_LENGTH_SUFFIX)
	 * @param defaultSuffix
	 *            The suffix to use as default if none is specified
	 * 
	 ...
	 * 
	 * @param exp
	 *            The expression inside the square brackets (for example
	 *            p[title=test][class] will be parsed in the construvtor and be
	 *            sent here twice as 'title=test' and then 'class')
	 */
	...
	/**
	 * Returns the language code of the specified element (or one of its
	 * ancestors) This implementation is optimized for minimal storage but not
	 * for performance - each element checks its ancestors for any lang
	 * definition
	 * 
	 * @param element
	...
	/**
	 * Matches the attribute selections of this selector (if any) to the
	 * provided element
	 * 
	 * @param element
			...
			if (attStr.constraint == AttString.LANG) {
				...
				if (attStr.value != null) { // null means if defined it suffices
					...
					case AttString.CONTAINS_WORD:
						String str = " " + elementVal + " "; // adding leading
																// and trailing
																// space
						if (str.indexOf(" " + attStr.value + " ") == -1) { // adding
																			// spaces
																			// to
																			// check
																			// for
	...
	/**
	 * Constructs a CSSElement, This basically sets the name and ID and resets
	 * the attrVals array.
	 * 
	 * @param name
	 */
	CSSElement(String name) {
		...
		if (index != -1) {
			...
			if (HTMLComponent.PROCESS_HTML_MP1_ONLY) { // HTML-MP1 does not
														// supports attribute
														// selectors, so this
														// selector will be
														// ignored
				...
				while (index != -1) {
					...
					if ((endIndex != -1) && (endIndex > index + 1)) { // non
																		// empty
																		// (i.e.
																		// not
																		// [])
	 ...
	 * Unlike Element which discards the name string and uses the id to identify
	 * the name, the CSSElement retains the name and simply returns it here.
	 * This is because a CSSElement name is in fact the selector string which is
	 * any combination of tags, classes and IDs and as such cannot be converted
	 * to a simple int.
	 * 
	...
	/**
	 * Checks if the specified attribute is assigned (i.e. was set with a legal
	 * value)
	 * 
	 * @param attrId
	 *            The attribute ID (Should be one of the CSS attributes, i.e. >=
	 *            CSS_STYLE_ID_OFFSET)
	...
	/**
	 * Returns the CSSElement's child positioned at the specified index. This is
	 * a convenience method that is very similar to Element.getChildAt, but
	 * returns an object of the CSSElement class. Since all of the children of a
	 * CSSElement are CSSElements as well, this prevents redundant casting.
	 * 
	 * @param index
	 *            The requested child position
	 * @return The child at the requested position
	 * @throws ArrayIndexOutOfBoundsException
	 *             if the index is bigger than the children's count or smaller
	 *             than 0
	 */
	...
	/**
	 * Adds the specified attribute and value to this CSSElement if it is
	 * supported and has a valid value. This method overrides
	 * Element.addAttribute to provide with specific CSSElement functionality.
	 * Unlike Element which retains all the value strings in the attributes
	 * hashtable, In CSSElement the value is immediately converted to a numeric
	 * value if possible and placed in the attrVals array. The string is
	 * retained only for values that can't be converted to int (such as URLs).
	 * 
	 * @param attribute
	...
	/**
	 * Adds the specified attribute and value to this CSSElement if it is
	 * supported and has a valid value. This method is used by
	 * addAttribute(String,String) and also when we already know the attribute's
	 * id (Such as in shorthand attributes)
	 * 
	 * @param attrId
	 *            The attribute's id
	 * @param value
	...
	int addAttribute(int attrId, String value) {
		...
		if (val == -1) { // Some attributes can be either a number, or a string.
							// For example border-width may be a
							// number/percentage but can also be
							// "thin"/"medium"/"thick" which is translated by
							// the strings
	...
	/**
	 * Checks if the attribute is one of CSS_BACKGROUND_POSITION_X or
	 * CSS_BACKGROUND_POSITION_Y and if so fixes the default of the other
	 * attribute accordingly. Background position is a very special case, since
	 * when it is not specified the default is TOP, LEFT, but when one of the
	 * background positions is specified, the other's default turns to CENTER
	 * 
	 * @param attrId
	 *            The attributeID
	 */
	...
	/**
	 * Returns this selector's specificity. A specificity of a selector
	 * determines the order in which it should be applied. The bigger the
	 * specificty, the later the selector will be applied (Which means it will
	 * overdride previously applied selectors) This lazily invokes
	 * calcSelectorSpecificity if it wasn't invoked before.
	 * 
	...
	/**
	 * Returns the length value of the requested attribute. A CSS length value
	 * can be denoted in various units, or percentages. This method calculates
	 * the final value in pixels.
	 * 
	 * @param attrId
	 *            The attribute's id
	 * @param cmp
	 *            The component relevant to the length calculation (Used in case
	 *            of length values based on font size)
	 * @param origDimension
	 *            The original dimension to take into account if the value is
	...
	static int convertLengthVal(int val, Component cmp, int origDimension) {
		if (val >= 0) { // !=-1 is not enough, since some values as
						// FONT_SMALLER/LARGER are negative and then & VAL_* may
						// be true...
			if ((val & VAL_PERCENTAGE) != 0) {
				val -= VAL_PERCENTAGE;
				val = val * origDimension / 100;
			} else if ((val & VAL_EX) != 0) { // 'ex' means half of the font
												// size
				val -= VAL_EX;
	...
	int calcSelectorSpecificity() {
		int spec = 0;
		if (attributeSelections != null) {
			spec += attributeSelections.size(); // attribute selectors get extra
												// specificity points, same as
												// pseudo classes
		}
		...
		while (index != -1) {
			...
			} else if ((!HTMLComponent.PROCESS_HTML_MP1_ONLY) && (property.startsWith("lang("))) { // Though
																									// :lang
																									// is
																									// a
																									// pseudo-class,
		...
		setTagName(nameStr); // The tag name should not contain the pseudo
								// classes
		...
		for (int i = 0; i < getNumChildren(); i++) { // There should be usually
														// just one child
	...
	/**
	 * Returns this selector's id, or null if none This method assumes that
	 * calcSelectorSpecificity was invoked before.
	 * 
	...
	/**
	 * Returns this selector's class, or null if none This method assumes that
	 * calcSelectorSpecificity was invoked before.
	 * 
	...
	/**
	 * Returns this selector's tag, or null if none This method assumes that
	 * calcSelectorSpecificity was invoked before.
	 * 
	...
	/**
	 * Copies all properties, both string and numeric values to the destination
	 * element This is used for grouped selectors
	 * 
	 * @param dest
	...
	CSSElement(CSSElement selector) {
		...
		attributeSelections = selector.attributeSelections; // This assumes that
															// there are no
															// modifications to
															// this vector after
															// the copy time

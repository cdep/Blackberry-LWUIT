...
/**
 * A ComboBox control that allows multiple selection and supports OPTGROUP
 * labels. Note that this does not inherit from ComboBox but rather from List,
 * since basically like an HTML multiple ComboBox we show an open List The
 * List/Combo size is controlled from the outside in HTMLComponent using a
 * wrapping Container.
 * 
 * This class is also used by HTMLComboBox as the list popup when opening the
 * HTMLComboBox (To allow OPTGROUP support)
 * 
 * @author Ofir Leitner
 */
class MultiComboBox extends List {
	...
	MultiComboBox(ListModel underlyingModel, boolean multiple) {
		...
		setScrollToSelected(!multiple); // In multiple comboboxes we don't
										// scroll to selected, as there can be
										// several
		...
		if (underlyingModel != null) { // Need to scan the existing data since
										// there will be no "addItem" with an
										// optgroup item
	...
	// Overiding List's methods to provide for multiple selection and OPTGROUP
	// support:
	...
	public void keyPressed(int keyCode) {
		...
		if (gameAction == keyBck) {
			...
			if (selectedIndex < size() - 1) {
				if (model.getItemAt(selectedIndex + 1) instanceof String) {
					if (selectedIndex == size() - 2) { // Last item is an
														// optgroup
						return;
					}
					model.setDirection(1);
	...
	class MultiListModel extends DefaultListModel {
		...
		public void setSelectedIndex(int index) {
			if (getItemAt(index) instanceof String) { // don't select optgroup
				if (direction == 0) {
					toggleSelected(getItemAt(getSelectedIndex())); // Since the
																	// incorrect
																	// item is
																	// toggled
																	// later, we
	...
	/**
	 * A renderer that knows to handle both multiple selection and OPTGORUP
	 * labels
	 * 
	 * @author Ofir Leitner
	 */

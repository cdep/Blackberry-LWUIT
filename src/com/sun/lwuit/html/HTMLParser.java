...
/**
 * The HTMLParser class is used to parse an XHTML-MP 1.0 document into a DOM
 * object (Element). Unsupported tags and attributes as well as comments are
 * dropped in the parsing process. The parser is also makes use of CSSParser for
 * external CSS files, embedded CSS segments and CSS within the 'style'
 * attribute.
 * 
 * @author Ofir Leitner
 */
public class HTMLParser extends XMLParser {
	 ...
	 * This is used to enable empty tags to be closed also in a non-strict way
	 * (i.e. &lt;br&gt; instead of &lt;br&gt/;) some of these tags are not a
	 * part of the XHTML-MP 1.0 standard, but including them here allows a more
	 * smooth parsing if the document is not strictly XHTML-MP 1.0
	 */
	 ...
	 */
	public HTMLParser() {
		// Add common char entities that are above the HTML 2.0 char entities
		// range
	...
	/**
	 * Pair this HTMLParser with the HTMLComponent that uses it. This pairing is
	 * necessary to allow access to the htmlC in parseTagContent upon finding a
	 * CSS embedded segment
	 * 
	 * @param htmlC
	...
	/**
	 * Overrides XMLParser.parseTagContent to enable embedded CSS segments
	 * (Style tags)
	 * 
	 * @param element
	...
	protected void parseTagContent(Element element, Reader is) throws IOException {
		if ((HTMLComponent.SUPPORT_CSS) && (htmlC.loadCSS) && (((HTMLElement) element).getTagId() == HTMLElement.TAG_STYLE)) { // We
																																// aren't
																																// strict
																																// and
																																// don't
																																...
																																// //
																																// &&
																																// "text/css".equals(element.getAttributeById(Element.ATTR_TYPE))))
																																// {
	...
	/**
	 * This method translates between an HTML char entity string to the
	 * according char code. It first tries to find it using its super method. If
	 * not found, the search continues to a wider string array of char codes
	 * 160-255 which are supported in ISO-8859-1 / HTML 2.0
	 * 
	 * @param symbol
	 *            The symbol to lookup
	 * @return The char code of the symbol, or -1 if none found
	 * 
	 *         protected int getCharEntityCode(String symbol) { int
	 *         val=super.getCharEntityCode(symbol); if (val==-1) { // Not one of
	 *         the most popular char codes, proceed to check the ISO-8859-1
	 *         symbols array val=CSSElement.getStringVal(symbol,
	 *         CHAR_ENTITY_STRINGS); if (val!=-1) { return val+160; } } return
	 *         val; }
	 */
	 ...
	 * HTMLElement. Basically calling this method is simlar to calling parse and
	 ...
	 * 
	 * @return true if the tag is a supported XHTML Mobile Profile 1.0 tag,
	 *         false otherwise
	 */
	 ...
	 * tag. The script tag should be skipped entirely, since it may contain
	 * characters like greater-than and lesser-than which may break the HTML All
	 * other tags are evaluated (i.e. added including all their children to the
	 * tree), even if not supported (But of course their functionality is
	 * ignored by HTMLComponent)
	 * 

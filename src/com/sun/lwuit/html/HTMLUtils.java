...
public class HTMLUtils {
	...
	/**
	 * This is a list of ISO 8859-1 Symbols that can be used as HTML char
	 * entities
	 */
	...
	/**
	 * Converts an XML char entity to the matching character or string. This is
	 * a convenience method that uses convertCharEntity with false for
	 * lookupHTMLentities and a null userDefinedCharEntities
	 * 
	 * @param charEntity
	...
	/**
	 * Converts an HTML char entity to the matching character or string. This is
	 * a convenience method that uses convertCharEntity with true for
	 * lookupHTMLentities and a null userDefinedCharEntities
	 * 
	 * @param charEntity
	 *            The char entity to convert (Not including the & and ;)
	...
	/**
	 * Converts a char entity to the matching character or string. This handles
	 * both numbered and symbol char entities (The latter is done via
	 * getCharEntityCode)
	 * 
	 * @param charEntity
	 *            The char entity to convert (Not including the & and ;)
	 * @param lookupHTMLentities
	 *            true to include the basic HTML named char entities (unicode
	 *            160-255), false otherwise
	 * @param userDefinedCharEntities
	 *            A hashtable containing (String,int) dentoing the char entity
	 *            name and its unicode
	 * @return A string containing a single char, or the original char entity
	 *         string (with & and ;) if the char entity couldn't be resolved
	 */
	public static String convertCharEntity(String charEntity, boolean lookupHTMLentities, Hashtable userDefinedCharEntities) {
		...
		if (charCode != -1) {
			...
			// notifyError(ParserCallback.ERROR_UNRECOGNIZED_CHAR_ENTITY,null,null,null,
			// "Unrecognized char entity: "+charEntity);
			// return "&"+charEntity+";"; // Another option is to return an
			// empty string, but returning the entity will unravel bugs and will
			// also allow ignoring common mistakes such as using the & char
			// (instead of &apos;)
		}
	...
	private static int getCharEntityCode(String symbol, boolean html, Hashtable userDefined) {
		...
		if (val != -1) {
			...
			if (html) {
				// Not one of the most popular char codes, proceed to check the
				// ISO-8859-1 symbols array
			...
			// Not found in the standard symbol table, see if it is in the user
	...
	/**
	 * Encodes the specified string to "percent-encoding" or URL encoding. This
	 * encodes reserved, unsafe and unicode characters
	 * 
	 * @param str
	 *            The string to be encoded
	 * @return A percent-encoding of the string (safe characters remain the
	 *         same)
	 */
	public static String encodeString(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			char c = str.charAt(i);
			if (
			// Checks for unreserved characters that RFC 3986 defines that
			...
			{
				encodedStr += c;
			} else if ((c >= 0x80) && (c <= 0xffff)) { // UTF encoding - See
				...
				// The max value of a char is 0xffff, so though URL encoding
				// supports values bigger than that, we can't provide for it
				/*
				 * } else if (c>0xffff) { // 4 literals unicode (CJK upper
				 * ranges and others) int z=c/65536; int y=c%65536; int x=y%256;
				 * y=y/256; int[] literal=new int[4]; literal[0]=240+(z>>2);
				 * literal[1]=128+((z%4)<<4)+(y>>4);
				 * literal[2]=128+((y%16)<<2)+(x>>6); literal[3]=128+(x & 63);
				 * for(int l=0;l<literal.length;l++) {
				 * encodedStr+="%"+Integer.toHexString
				 * (literal[l]).toUpperCase(); }
				 */
			} else {
				String prefix = "%";
				if (c < 16) {
					prefix += "0"; // For a value lesser than 16, we'd like to
									// get %0F and not %F
				}
	...
	/**
	 * Matches the given string to the given options and returns the matching
	 * value, or -1 if none found.
	 * 
	 * @param str
	 *            The string to compare
	 * @param options
	 *            The options to match the string against
	 * @return The appropriate matching value: If the string equals (case
	 *         ignored) to the option in the X position of the options array,
	 *         the int X will be returned. If the string didn't match any of the
	 *         options -1 is returned.
	 */
	 ...
	 *            The options to match the string against
	 * @param vals
	 *            The values to match to each option (According to the position
	 *            in the array), this can be null.
	 * @return The appropriate matching value: If the string equals (case
	 *         ignored) to the option in the X position of the options array,
	 *         this returns the value in the X position of the vals array, or
	 *         simply X if vals is null. If the string didn't match any of the
	 *         options -1 is returned.
	 */
	...
	/**
	 * Matches the given string to the given options and returns the matching
	 * value, or the default one if none found.
	 * 
	 * @param str
	 *            The string to compare
	 * @param options
	 *            The options to match the string against
	 * @param defaultValue
	 *            The default value to return if the string was null or not
	 *            found among the options
	 * @return The appropriate matching value: If the string equals (case
	 *         ignored) to the option in the X position of the options array,
	 *         the int X will be returned. If the string didn't match any of the
	 *         options the defaultValue is returned.
	 */

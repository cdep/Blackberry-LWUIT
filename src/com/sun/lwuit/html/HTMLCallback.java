...
/**
 * HTMLCallback is used to dispatch document lifecycle events. Most methods are
 * called on the EDT thread, except parsingError, getAutoComplete and
 * getLinkProperties
 * 
 * @author Ofir Leitner
 */
public interface HTMLCallback extends ParserCallback, CSSParserCallback {
	...
	/**
	 * Error code denoting that a connection to the resource provider/server
	 * could not be made
	 */
	...
	/**
	 * Error code denoting that a relative URL was referenced from a document
	 * with no base URL (A document that was loaded via setBody/setHTML/setDOM
	 * and not via setPage) In this case the return value of parsingError is not
	 * considered - parsing continues and the resource at the URL (CSS
	 * file/image) is ignored
	 */
	public static int ERROR_NO_BASE_URL = 103;

	/**
	 * Error code denoting that a tag contains tags it shouldn't, or that a tag
	 * doesn't have the parent tag it should have
	 */
	...
	/**
	 * The page and all of its referenced images and CSS files were loaded
	 * completely
	 */
	...
	/**
	 * Called whenever a field is submitted to a form. This can be used to
	 * perform sanity checks and/or to store values for auto complete.
	 * 
	 * @param htmlC
	 ...
	 * @param errorMsg
	 *            The error message if any error occured (i.e. input validation
	 *            error) or null if no error occured
	 * @return The string to submit to the form (Should return value if nothing
	 *         changed)
	 */
	...
	/**
	 * Called on form creation and enabled implementations of this method to
	 * return a value to preset in a form field. This can be used to auto
	 * complete previously entered value Note that this method is always called
	...
	/**
	 * Returns properties about the given link to indicate to HTMLComponent how
	 * to render it Note that this method is always called NOT on the EDT
	 * thread.
	 * 
	 * @param htmlC
	 *            The HTMLComponent
	...
	/**
	 * Called when a link is clicked. This can be used to process links that
	 * needs additional/alternate handling than fetching an HTML.
	 * 
	 * @param htmlC
	 *            The HTMLComponent
	 * @param url
	 ...
	 * This method will be called only if HTMLComponent.isEventsEnabled() is
	 * true
	 * 
	 * @param evt
	 *            The event triggered, the component can be extracted using
	 *            getSource
	 * @param htmlC
	 *            The HTMLComponent
	 * @param element
	 *            The element associated with the component that triggered the
	 *            event
	 */
	...
	/**
	 * Called when one of the controls in the HTMLComponent lost focus This
	 * method will be called only if HTMLComponent.isEventsEnabled() is true
	 * 
	 * @param cmp
	 *            The component that triggered the event
	 ...
	 * Note: Selection is traversing through the items - once the user has
	 * actually decided, an ActionEvent will be sent as well This method will be
	 * called only if HTMLComponent.isEventsEnabled() is true
	 * 
	 * @param oldSelected
	 ...
	 *            The HTMLComponent
	 * @param list
	 *            The list that triggered the event (Usually a ComboBox but not
	 *            always)
	 * @param element
	 *            The element associated with the component that triggered the

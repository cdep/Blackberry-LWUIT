...
/**
 * This class serves as a aggregator of all the event listeners for all the
 * active components within the HTML page. It is in itself an ActionListener and
 * a FocusListener, and it creates SelectionListeners and DataChangedListeners
 * if needed. Upon events it both updates the DOM if needed, and dispatches
 * event methods on HTMLCallback
 * 
 * @author Ofir Leitner
 */
class HTMLEventsListener implements ActionListener, FocusListener {
	...
	/**
	 * Registeres the specified component/element duo to listen to all available
	 * events
	 * 
	 * @param cmp
	...
	void registerComponent(final Component cmp, final HTMLElement element) {
		...
		if (cmp instanceof Button) { // catches Button, CheckBox, RadioButton
			...
			SelectionListener sl = new SelectionListener() { // We create a
																// listener and
																// not listen
																// ourself since
																// the
			...
			if (cmp instanceof TextField) {
				...
				DataChangedListener dcl = new DataChangedListener() { // We
																		// create
																		// a
																		// listener
																		// and
	...
	void deregisterAll() {
		for (Enumeration e = comps.keys(); e.hasMoreElements();) {
			...
			if (cmp instanceof Button) { // catches Button, CheckBox,
											// RadioButton
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (src instanceof CheckBox) {
			...
			if ((curDomState == null) || (!curDomState.equals("checked"))) {
				...
				if (name != null) { // If this is named radiobutton, we need to
									// set the status of the others accordingly
					for (Enumeration e = comps.keys(); e.hasMoreElements();) {
						Component cmp = (Component) e.nextElement();
			...
			// element.setAttributeById(HTMLElement.ATTR_CHECKED,
			// ((RadioButton)src).isSelected()?"checked":null);
			...
			if (element.getNumChildren() == 0) {
				...
				if (textElem.isTextElement()) { // If the HTML is malformed we
												// may have a different element
												// - and we ignore
			...
			Vector v = element.getDescendantsByTagId(HTMLElement.TAG_OPTION); // This
																				// is
																				// activated
																				// on
																				// the
			...
			for (Enumeration e = v.elements(); e.hasMoreElements();) {
				...
				if (option.getNumChildren() == 1) { // we expect only a text
													// element, if not the HTML
													// is malformed and we
													// ignore
					HTMLElement textElem = (HTMLElement) option.getChildAt(0);
					if (textElem.isTextElement()) { // If the HTML is malformed
													// we may have a different
													// element - and we ignore

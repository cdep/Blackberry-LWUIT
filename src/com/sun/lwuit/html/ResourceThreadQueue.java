...
/**
 * ResourceThreadQueue is a thread queue used to create and manage threads that
 * download images and CSS files that were referred from HTML pages Was called
 * ImageThreadQueue but name was changed since it now handles CSS as well
 * 
 * @author Ofir Leitner
 */
class ResourceThreadQueue {
	...
	int threadCount;
	private int cssCount = -1; // As long as there are no CSS files this stays
								// -1 and thus CSS loading is ignored
	boolean started;
	 ...
	 * 
	 * @param htmlC
	 *            The HTMLComponent this queue belongs to
	 */
	...
	/**
	 * Sets the maximum number of threads to use for image download If
	 * startRunning was already called, this will takes effect in the next page
	 * loaded.
	 * 
	 * @param threadsNum
	 ...
	 * 
	 * @param imgLabel
	 *            The label in which the image should be contained after loaded
	 * @param imageUrl
	 *            The URL this image should be fetched from
	 */
	synchronized void add(Component imgLabel, String imageUrl) {
		/*
		 * if ((HTMLComponent.TABLES_LOCK_SIZE) && (htmlC.curTable!=null)) {
		 * downloadImageImmediately(imgLabel, imageUrl, 0); } else {
		 */
		if (started) {
			throw new IllegalStateException("ResourceThreadQueue already started! stop/cancel first");
		}
		images.put(imgLabel, imageUrl); // Using a hashtable to collect all
										// requests first enables overriding
										// urls for labels (For example in CSS
										// use cases)
		// }
	}

	/**
	 * Adds the image to the queue, to be used as a background image for a
	 * component
	 * 
	 * @param imgComp
	 *            The component for which the image should be used after loaded
	 * @param imageUrl
	 *            The URL this image should be fetched from
	 * @param styles
	 *            A mask of CSSEngine.STYLE_* values indicating in which styles
	 *            this background image should be displayed
	 */
	synchronized void addBgImage(Component imgComp, String imageUrl, int styles) {
		if (HTMLComponent.SUPPORT_CSS) {
			/*
			 * if ((HTMLComponent.TABLES_LOCK_SIZE) && (htmlC.curTable!=null)) {
			 * downloadImageImmediately(imgComp,imageUrl,styles); } else {
			 */
	...
	/**
	 * Downloads the image immediately // It seems that the issue that required
	 * this was already solved in the table package The image is not added to
	 * the queue, and not loaded on another thread but rather downloaded on the
	 * same thread that builds up the document This is useful for
	 * HTMLCoponent.TABLES_PATCH
	 * 
	 * @param imgComp
	 *            The component for which the image should be used after loaded
	 * @param imageUrl
	 ...
	 *            this background image should be displayed
	 * 
	 *            synchronized void downloadImageImmediately(Component
	 *            imgComp,String imageUrl,int styles) { try { InputStream is =
	 *            htmlC.getRequestHandler().resourceRequested(new
	 *            DocumentInfo(imageUrl,DocumentInfo.TYPE_IMAGE)); Image img =
	 *            Image.createImage(is); ResourceThread t = new
	 *            ResourceThread(imageUrl, imgComp, htmlC, null);
	 *            t.handleImage(img, imgComp,((styles &
	 *            CSSEngine.STYLE_UNSELECTED)!=0),((styles &
	 *            CSSEngine.STYLE_SELECTED)!=0),((styles &
	 *            CSSEngine.STYLE_PRESSED)!=0)); } catch (Exception ex) {
	 *            ex.printStackTrace(); }
	 * 
	 *            }
	 */
	 ...
	 * 
	 * @param cssUrl
	 *            The URL this style sheet should be fetched from
	 * @param dom
	 *            the dom object on which the CSS should be applied
	 */
	...
	/**
	 * Returns the queue size, this is relevant only when the queue hasn't
	 * started yet
	 * 
	 * @return the queue size
	 */
	int getQueueSize() {
		return (images.size() + queue.size()); // CSS files are added directly
												// to queue while images to the
												// images vector
		// return queue.size();
	}

	/**
	 * Notifies the queue that all images and CSS have been queues and it can
	 * start dequeuing and download the images. The queue isn't started before
	 * that to prevent multiple downloads of the same image
	 */
	...
	/**
	 * Starts downloading the images (This is called only after all CSS files
	 * have been downloaded, since they may contain image references)
	 */
	...
	/**
	 * Called by the ResourceThread when it finishes downloading and setting the
	 * image. This in turns starts another thread if the queue is not empty
	 * 
	...
	synchronized void threadFinished(ResourceThread finishedThread, boolean success) {
		...
		if (finishedThread.cssDocInfo != null) {
			cssCount--; // Reduce the number of waiting CSS, even if reading
						// failed
		}
	...
	/**
	 * Discards the entire queue and signals the running threads to cancel. THis
	 * will be triggered if the user cancelled the page or moved to another
	 * page.
	 */
	...
	class ResourceThread implements Runnable, AsyncDocumentRequestHandler.IOCallback {
		 ...
		 * 
		 * @param imgLabel
		 *            The label in which the image should be contained after
		 *            loaded
		 * @param imageUrl
		 *            The URL this image should be fetched from
		 * @param handler
		 ...
		 * @param threadQueue
		 *            The main queue, for callback purposes
		 */
		 ...
		 * 
		 * @param cssDocInfo
		 *            A DocumentInfo object with the URL this CSS file should be
		 *            fetched from
		 * @param handler
		 *            The RequestHandler through which to retrieve the image
		 * @param threadQueue
		...
		/**
		 * Adds a label which has the same URL, useful for duplicate images in
		 * the same page
		 * 
		 * @param label
		 *            A label which has the same image URL
		 */
		...
		/**
		 * This is the main entry point to this runnable, it checks whether the
		 * callback is synchronous or async. According to that it either runs
		 * this as a thread (sync) or simply calls the async method (async
		 * implements threading itself)
		 */
		void go() {
			...
			try {
				if (is == null) {
					...
					if (cssDocInfo != null) { // CSS
						if (HTMLComponent.SUPPORT_CSS) { // no need to also
															// check if loadCSS
															// is true, since if
															// we got so far -
															// it is...
		...
		/**
		 * After a successful download, this handles placing the image on the
		 * label and resizing if necessary
		 * 
		 * @param img
		 *            The image
		 * @param label
		 *            The label to apply the image on
		 */
		 ...
		 *            The image
		 * @param cmp
		 *            The component to apply the image on
		 * @param bgUnselected
		 *            true if the image should be used as a background for the
		 *            component when it is unselected, false otherwise
		 * @param bgSelected
		 *            true if the image should be used as a background for the
		 *            component when it is selected, false otherwise
		 * @param bgPressed
		 *            true if the image should be used as a background for the
		 *            component when it is pressed, false otherwise
		 */
		void handleImage(Image img, Component cmp, boolean bgUnselected, boolean bgSelected, boolean bgPressed) {
			...
			label.setText(""); // remove the alternate text (important to do
								// here before checking the width/height)

			// Was set in HTMLComponent.handleImage if the width/height
			// attributes were in the tag
			...
			if (width != 0) {
				if (height == 0) { // If only width was specified, height should
									// be calculated so the image keeps its
									// aspect ratio
					height = img.getHeight() * width / img.getWidth();
				}
			} else if (height != 0) {
				if (width == 0) { // If only height was specified, width should
									// be calculated so the image keeps its
									// aspect ratio
			...
			if (width != 0) { // if any of width or height were not 0, the other
								// one was set to non-zero above, so this check
								// suffices
			...
			htmlC.revalidate();
			if (label.getClientProperty(HTMLComponent.CLIENT_PROPERTY_IMG_BORDER) == null) { // if
																								// this
																								// property
																								// is
																								// defined,
																								...
																								// border
																								// already
				label.getUnselectedStyle().setBorder(null); // remove the border
															// which is a sign
															// the image is
															// loading
			} else {
				int borderSize = ((Integer) label.getClientProperty(HTMLComponent.CLIENT_PROPERTY_IMG_BORDER)).intValue();
				// Note that padding is set here and not in handleImage since we
				// rely on the image size (which includes padding) to know if a
				// width/height was specified

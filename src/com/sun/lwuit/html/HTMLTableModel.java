...
/**
 * HTMLTableModel is a TableModel that is adapted to HTMLTables and their
 * creation. The difference from other models is:<br>
 * - There are no column names, on the other hand each cell anywhere can be
 * defined as a header (which merely changes it style).<br>
 * - There is no prior declaration of the number of columns and rows in the
 * table.<br>
 * - Cells can be added only using the addCell method which adds them to the end
 * of the current row.<br>
 * - When a row ends, the commitRow method is called which results in creating a
 * new empty row.<br>
 * - Since a cell in an HTML table can be practically everthing (From a simple
 * string to a whole document), the objects added to the table are in fact LWUIT
 * components.
 * 
 * @author Ofir Leitner
 */
class HTMLTableModel implements TableModel {
	...
	/**
	 * Adds the given component as a cell to the end of the current row of the
	 * table
	 * 
	 * @param cell
	 *            The component to add
	 * @param isHeader
	 *            true if this is a header cell (Element.TAG_TH), false
	 *            otherwise
	 * @param constraint
	 *            Specific constraints for this cell (alignment, spanning)
	 */
	 ...
	 * 
	 * @param isHorizontal
	 *            true to set horizontal alignment, false for vertical
	 * @param align
	...
	void startSegment(int segmentType) {
		if ((segmentType == SEGMENT_THEAD) && (!hasTHead)) { // Can only have
																// one THEAD,
																// second one
																// will be
																// considered as
		...
		} else if ((segmentType == SEGMENT_TFOOT) && (!hasTFoot)) { // Can only
																	// have one
																	// TFOOT,
																	// second
																	// one will
	...
	/**
	 * Checks if the specified row ends a segment in the table (and as such a
	 * line should be drawn after it) This is used when a the 'rules' attribute
	 * in the 'table' tag was set to 'groups'
	 * 
	 * @param row
	 *            The row to check
	 * @return true if the specified row ends a segment in the table, false
	 *         otherwise
	 */

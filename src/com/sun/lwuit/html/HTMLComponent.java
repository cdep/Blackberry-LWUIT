...
/**
 * HTMLComponent is a LWUIT Component that renders HTML documents that conform
 * to the XHTML Mobile Profile 1.0
 * 
 * @author Ofir Leitner
 */
public class HTMLComponent extends Container implements ActionListener, AsyncDocumentRequestHandler.IOCallback {
	...
	/**
	 * If true a full screen width will be assumed, this helps to make less
	 * internal components as text is aggregated to long labels If false, then
	 * the width is flexible, but every word will be rendered as a separate
	 * label. Note that if false, RTL texts will display in
	 */
	static final boolean FIXED_WIDTH = false;

	/**
	 * A constant that can be used to obfuscate out HTMLInputFormat if
	 * unnecessary
	 */
	static final boolean SUPPORT_INPUT_FORMAT = true;

	/**
	 * A constant that can be used to obfuscate out all CSS related code if
	 * unnecessary
	 */
	static final boolean SUPPORT_CSS = true;

	/**
	 * If true, when a page is requested, the previous page is immediately
	 * removed, thus helping conserve memory (Since only 1 page is held in
	 * memory simultanously) In next releases a better and external control over
	 * this will be provided.
	 */
	private static final boolean CLEAN_ON_PAGE_REQUEST = true; // false;

	/**
	 * If true, checks will be made on each character in the text to see if it
	 * is CJK (Chinese, Japanese, Korean) If so each character will be
	 * considered as an entire word (for line wrapping puposes) as required in
	 * CJK. Since the tests require multiple unicode ranges check, this can be
	 * set to false avoiding these tests if the app doesn't use CJK
	 */
	private static final boolean CJK_SUPPORT = true;

	/**
	 * A constant denoting whether to process only XHTML-MP1 (and WCSS), or some
	 * other tags/properties from HTML4/CSS2
	 */
	static final boolean PROCESS_HTML_MP1_ONLY = false;

	/**
	 * If true, then table cells will be preset with a size according to their
	 * preferred size, before the table begins calculating needed sizes This
	 * also requires loading all images in table cells prior to page display
	 * (Unlike the normal way in which they can be completed afterwards) This is
	 * needed due to some complexity when calculating sizes of tables with
	 * complex containers as cells, and especially with nested tables
	 */
	// static final boolean TABLES_LOCK_SIZE = true; // The issue that required
	// this patch was solved in the table package

	// Indentation for various tags
	private static final int INDENT_BLOCKQUOTE = 20;
	private static final int INDENT_DD = 20;
	private static final int INDENT_OL = Font.getDefaultFont().stringWidth("8888. "); // Ordered
																						// list
	private static final int INDENT_UL = Font.getDefaultFont().charWidth('W'); // Unordered
																				// list

	// The minimum number of visible items of a multiple combobox (Unless it has
	// less)
	...
	/**
	 * Indicates a Justify alignment. Text justification is enabled only in
	 * FIXED_WIDTH mode
	 */
	...
	/**
	 * The time it takes a marquee to move across the screen (See -wap-marquee
	 * in CSS)
	 */
	private static final int MARQUEE_DELAY = 6000;

	/**
	 * When this is set to true, only exact fonts will be matched, i.e. if the
	 * font matching algorithm needs an arial.12.bold.italic - it will not
	 * "settle" for just bold or italic or a different size Default is false
	 */
	private static final boolean MATCH_EXACT_FONTS_ONLY = false;

	/**
	 * When this is set to true only bitmap fonts will be used for matching (and
	 * not system fonts) Default is true, and false has not been tested
	 * thoroughly
	 */
	private static final boolean MATCH_BITMAP_FONTS_ONLY = true;

	/**
	 * When this is set to true, only fonts of the same family will be matched.
	 * i.e. if we're looking for a bigger font than arial.10, and we have
	 * timesnewroman.12 it will not be taken as a matching font Default is true
	 */
	...
	// Constants for the various possible input types (i.e. allowed values of
	// the INPUT tag)
	...
	private static final int INPUT_IMAGE = 7; // image is not officially
												// supported in XHTML-MP 1.0 but
												// we support it
	// private static final int INPUT_BUTTON = 8; //button is not supported in
	// XHTML-MP 1.0
	// private static final int INPUT_FILE = 9; //file upload is not supported
	// in XHTML-MP 1.0

	/**
	 * Defines the possible values for the type attribute in the input tag,
	 * ordered according to the INPUT_* constants
	 */
	...
	static final String CLIENT_PROPERTY_QUOTE = "quote"; // Client property
															// added to quote
															// elements to
															// enable changing
															// them later on
	...
	/**
	 * Ordered list type legal identifiers as can be defined in the ATTR_TYPE of
	 * the OL_TAG
	 */
	private static final char[] ORDERED_LIST_TYPE_IDENTIFIERS = { '1', 'A', 'a', 'I', 'i' };

	/**
	 * Inidcates whether this component should load CSS files and consider STYLE
	 * tag and attributes when available.. Note that CSS requires to create
	 * significantlly more containers, as almost every tag needs to be in its
	 * own container in order to manipulate style attributes like bgcolor,
	 * border etc. This is why the developer can disable CSS. Currently this is
	 * a private variable, once CSS is actually available this will be exposed
	 * via public getter/setter. Also there's no point setting this to true in
	 * this release as it will not lead to actual CSS, but just prepare
	 * containers for it.
	 */
	boolean loadCSS = SUPPORT_CSS;

	/**
	 * If true than all active controls will be added event listeners and
	 * dispatch them via HTMLCallback Default is false in order not to add
	 * overhead when this feature is not needed
	 */
	...
	/**
	 * Style sheets embedded directly into the style tag (not the style
	 * attribute)
	 */
	...
	/**
	 * The default background color of an HTML document, can be changed with the
	 * BGCOLOR attribute in the BODY tag or via CSS
	 */
	static final int DEFAULT_BGCOLOR = 0xffffff;

	/**
	 * The default color of text in HTML documents, can be changed with the TEXT
	 * attribute in the BODY tag or via CSS
	 */
	static final int DEFAULT_TEXT_COLOR = 0;

	/**
	 * The default color of links in HTML documents, can be changed with the
	 * LINK attribute in the BODY tag or via CSS
	 */
	...
	private DocumentRequestHandler handler; // The HTMLComponent's request
											// handler
	private RedirectThread redirectThread; // A refrence to a redirection thread
											// if exists
	private ResourceThreadQueue threadQueue; // A reference to the
												// ResourceThreadQueue that
												// handles asynchronous image
												// download
	private HTMLCallback htmlCallback;
	private HTMLEventsListener eventsListener;

	// Page info and status
	private DocumentInfo docInfo; // The DocumentInfo object associated with
									// this document
	private HTMLElement document; // The page DOM representation
	private String pageURL; // The current page's URL
	private int pageStatus = HTMLCallback.STATUS_NONE; // The page's status (See
														// HTMLCallback)
	private boolean pageError; // true if there was an error during the page
								// building process
	private String title; // The page's title (as obtained from the TITLE tag in
							// the HEAD tag)
	private int displayWidth; // Used to store the display width to detect width
								// changes later on.
	private boolean cancelled; // true if the page was cancelled (Signals all
								// processing to stop)
	private boolean cancelledCaught; // true if the cancel signal was already
										// caught (Used to avoid multiple cancel
										// handling)
	...
	// Links related
	private Hashtable accessKeys = new Hashtable();// A hastable containing all
													// the access keys in this
													// document and their
													// respective links
	private Hashtable anchors;// A hashtable containing all the anchors of this
								// document
	private Hashtable inputFields; // A hashtable containing all the input
									// fields in the page. Used for FOR labels
	...
	// this variable is only useful with the properties mostly for the purpose
	// of the resource editor GUI builder
	private String htmlBody;

	// The following variables are used when building the component from the
	// document.
	// After the component construction they are not used (Basically they can
	// all be considered as "temporary" variables,
	// but due to the recursive nature of the document building operation, they
	// are defined as members)

	// Containers

	/**
	 * The main container is the top-level container to which other containers
	 * are added when building the page. After the page was built the main
	 * container is added to the HTMLComponent. We do not build directly on the
	 * HTMLComponent to allow the building process to be performed not in the
	 * EDT thread.
	 */
	private Container mainContainer;

	/**
	 * curContianer is the current container we are adding components to. AT the
	 * start it is the main container. Later on it can be a certain table cell
	 * or a fieldset.
	 */
	private Container curContainer;

	private boolean autoFocus = true; // Determines whether to auto-focus on the
										// first link after page load

	/**
	 * curLine is the basic container to which components are added in the
	 * building process. Components are added to it horizontally and when there
	 * no more space left (or a newline is needed) - it is added to the
	 * curContainer
	 */
	private Container curLine;

	// Layouting - NOTE: In FIXED_WIDTH mode this component assumes the full
	// width of the screen and performs text wrapping due to a bug in the
	// combination of FlowLayout inside BoxLayout. In non fixed width mode the
	// variables x & width are not used.
	...
	private int leftIndent; // Holds the current left margin (can be changed by
							// tags)
	private boolean lastWasEmpty; // true if the last line was empty, used to
									// prevent unneeded multiple row spacing

	// Font
	private HTMLFont font; // Holds the current used font (starts with
							// defaultFont and changes according to various tags
							// and attributes)

	// Links
	private String link; // The current link address (so when a text is
							// processed, we know to attribute it to this link)
	private HTMLLink mainLink; // The "main" link. If a link spans over 2 or
								// more lines, the first segment is considered
								// the "main" link and is responsible for
								// highlighting all segments when focused.
	...
	// Forms
	private HTMLForm curForm; // The current HTMLForm to which input fields
								// should be added
	...
	private boolean optionSelected; // true if the current option is marked as
									// selected (i.e. the default selection)
	private String optionValue; // The current option value
	private Hashtable textfieldsToForms; // A hashtable that maps all input
											// fields to their corresponding
											// forms

	// Lists
	private int ulLevel; // Unordered list level (Used to display the right
							// bullet)
	private int olIndex; // Index of the current item in the list
	private Vector olUpperLevelIndex;// = new Vector(); //Used to save the index
										// when nesting
	private int listType; // The type of the current list
	private int listIndent; // Holds the current list margin

	// Tables
	private Vector tables;// A vector used for nesting, when a table contains a
							// table ,the current one is "pushed" into the
							// vector, to be "popped" later when the nested
	...
	private int quoteTagCount = 0; // A counter used to support QUOTE tag
									// nesting
	...
	private int superscript; // keeps track of current superscript level
								// (negative means subscript)
	private int maxSuperscript; // keeps track of teh maximal superscript value
								// of the current line
	...
	/**
	 * This static segment sets up the INPUT_TYPES vector with values from
	 * INPUT_TYPE_STRINGS. This is used later on for lookup.
	 */
	static {
	 ...
	 * 
	 * * The font family - i.e. Arial, Times New Roman etc. * The font size - in
	 * pixels (i.e. 12, 14 etc.) * The font style - bold, italic or both (no
	 * need to specify plain) * The font tag assignments - if this font should
	 * be used for any HTML specific tags they should be specified - i.e. h1,
	 * kbd etc.
	 * 
	 * The key is just a concatenation of all attributes seperated with a dot.
	 * Examples for valid keys: arial.12 - Describes a plain font from the arial
	 * family in size 12 arial.16.bold - A bold arial font in size 16
	 * courier.italic.bold.20 - A bold and italic courier font in size 20
	 * arial.20.h1 - A plain arial font, size 20, that should be used for
	 * contents of the H1 tag code.kbd.samp - A font that should be used for the
	 * CODE, KBD and SAMP tags
	 * 
	 * Note that the order of the attributes is not important and also that the
	 * case is ignored. This means that arial.12.bold.italic.h3 is equivalent to
	 * itALIc.H3.arial.BOLD.12
	 * 
	 * Also note that while you do not have to provide all the info for the
	 * font, but the info helps the rendering engine to reuse fonts when
	 * suitable. For example, if you have a 16px arial bold font which you want
	 * to use for H2, you can simply add it as "h2", but if you add it as
	 * "arial.16.bold.h2" then if the current font is arial.16 and the renderer
	 * encounters a B tag, it will know it can use the font you added as the
	 * bold counterpart of the current font.
	 * 
	 * When adding system fonts there is no need to describe the font, the usage
	 * of setFont with system fonts is usually just to assign them for tags. The
	 * rendering engine knows to derive bold/italic/bigger/smaller fonts from
	 * other system fonts (default or tag fonts) even if not added.
	 * 
	 * @param fontKey
	 *            The font key in the format described above
	 * @param font
	 *            The actual LWUIT font object
	 */
	...
	/**
	 * Sets a custom HTMLParser for this HTMLComponent By default, a new
	 * HTMLParser instance is created for each HTMLComponent, use this method if
	 * you have a custom parser.
	 * 
	 * @param parser
	 ...
	 * Symbols do not need to include leading & and trailing ; - these will be
	 * trimmed if given as the symbol
	 * 
	 * @param symbol
	 *            The symbol to add
	 * @param code
	...
	/**
	 * Adds support for a special key to be used as an accesskey. The CSS
	 * property -wap-accesskey supports special keys, for example "phone-send"
	 * that may have different key codes per device. This method allows pairing
	 * between such keys to their respective key codes. Note that these keys are
	 * valid only for -wap-aceesskey in CSS files, and not for the XHTML
	 * accesskey attribute.
	 * 
	 * @param specialKeyName
	 *            The name of the special key as denoted in CSS files
	 * @param specialKeyCode
	...
	/**
	 * Adds the given symbols array to the user defined char entities table with
	 * the startcode provided as the code of the first string, startcode+1 for
	 * the second etc. Some strings in the symbols array may be null thus
	 * skipping code numbers.
	 * 
	 * @param symbols
	 *            The symbols to add
	 * @param startcode
	...
	/**
	 * Sets the supported CSS media types to the given strings. Usually the
	 * default media types ("all","handheld") should be suitable, but in case
	 * this runs on a device that matches another profile, the developer can
	 * specify it here.
	 * 
	 * @param supportedMediaTypes
	 *            A string array containing the media types that should be
	 *            supported
	 */
	 ...
	 * 
	 * @param handler
	 *            The HttpRequestHandler to which all requests for links will be
	 *            sent
	 */
	public HTMLComponent(DocumentRequestHandler handler) {
		...
		// The TT (Teletype) tag is not part of XHTML-MP 1.0 but is supported
		// anyway
	 ...
	 * loaded/shown
	 * 
	 * @return the DocumentInfo that currently represents the document
	 *         loaded/shown
	 */
	 ...
	 * 
	 * @return the HTMLCallback that is set on this HTMLComponent or null if
	 *         none
	 */
	 ...
	 * 
	 * @param fontKey
	 *            The font key in the format described in setFont (Can be null
	 *            for default font, but it is recommended to add a descriptive
	 *            key if this is a bitmap font to enable the font engine to use
	...
	/**
	 * Sets whether this HTMLComponent will download and show linked images or
	 * not
	 * 
	 * @param show
	...
	/**
	 * Sets whether this HTMLComponent will ignore all CSS.directives. This
	 * includes external CSS files (which won't be downloaded), embedded CSS
	 * segmentsand style tags and attributes. By default this is false.
	 * 
	 * @param ignore
	 *            true to ignore CSS directives, false otherwise
	 */
	public void setIgnoreCSS(boolean ignore) {
		if (SUPPORT_CSS) { // If not supporting CSS, the loadCSS flag is locked
							// on false
			loadCSS = !ignore;
		}
	}
	 ...
	 * 
	 * @param pixels
	 *            The number of pixels to scroll (positive for forward and
	...
	/**
	 * Scrolls the HTMLComponent several pages forward/backward. TO scroll to
	 * the start or end of the document, one can provide a very big number.
	 * 
	 * @param pages
	 *            The number of pages to scroll (positive for forward and
	 *            negative for backward)
	 * @param animate
	 *            true to animate the scrolling, false otherwise
	 */
	...
	/**
	 * Sets the given string containing HTML code as this HTMLComponent's body.
	 * The string is read using the specified encoding. If the encoding is not
	 * supported it will be read without encoding
	 * 
	 * @param htmlText
	 *            The HTML body to set
	 * @param encoding
	 *            The encoding to use when reading the HTML i.e. UTF8,
	 *            ISO-8859-1 etc.
	...
	/**
	 * Sets the given string containing HTML code either as this HTMLComponent's
	 * body or as the full HTML. The string is read using the specified
	 * encoding. If the encoding is not supported it will be read without
	 * encoding
	 * 
	 * @param htmlText
	 *            The HTML to set
	 ...
	 *            ISO-8859-1 etc.
	 * @param title
	 *            The HTML title, or null if none (Used only when isFullHTML is
	 *            false)
	 * @param isFullHTML
	 *            true if this is a full HTML document (with html/body tags),
	 *            false if this HTML should be used as the HTMLComponent's body
	 * @return true if the encoding succeeded, false otherwise
	 */
	public boolean setHTML(String htmlText, String encoding, String title, boolean isFullHTML) {
		...
		if (isr == null) {
			isr = getStream("Encoding error loading string", null, title, isFullHTML); // TODO
																						// -
																						// dynamic
																						// error
																						// messages
	...
	/**
	 * Adds the specified cssElement which represents an external CSS file to
	 * the external CSS vector
	 * 
	 * @param cssElement
	 *            The element to add
	 */
	...
	/**
	 * Adds the specified cssElement which represents an embedded CSS segment to
	 * the embedded CSS vector
	 * 
	 * @param cssElement
	 *            The element to add
	 */
	...
	/**
	 * Sets the style of the page, allowing for example to set transparency to
	 * the main page. This applies not only to the current page, but rather to
	 * all pages created with this HTMLComponent instance. If both a UIID and a
	 * pageStyle were set, the style overrides the UIID.
	 * 
	 * @param pageStyle
	...
	/**
	 * Sets the UIID of the page (the internal container) This applies not only
	 * to the current page, but rather to all pages created with this
	 * HTMLComponent instance. If both a UIID and a pageStyle were set, the
	 * style overrides the UIID.
	 * 
	 * @param pageUIID
	...
	/**
	 * Applies the user defined page style to the main container. This clones
	 * the page style and sets the clone to the page, as various CSS directives
	 * may change the style of the main container.
	 */
	...
	private void cancelCurrent() {
		...
		if ((pageStatus == HTMLCallback.STATUS_REQUESTED) ||
				...
				(pageStatus == HTMLCallback.STATUS_PARSED)) { // previous page
																// still loading
			cancel();

			// TODO - This mechanism is far from ideal - handle better page life
			// cycle
			int waitTime = 0;
	...
	/*
	 * void clickTimer(String str) { if (str==null) {
	 * startTime=System.currentTimeMillis(); textTime=0; msg=""; } else {
	 * msg+=str+":"+(System.currentTimeMillis()-startTime)+"\n"; } }
	 */

	/**
	 * Sets this HTMLComponent to render the document specified in the
	 * DocumentInfo object
	 * 
	 * @param docInfo
	 *            Containing info about the document (url, encoding etc)
	 */
	void setPage(final DocumentInfo docInfo) {
		cancelCurrent();
		// clickTimer(null);
		this.docInfo = docInfo; // Moved here since when getting CSS imports in
								// embedded style segments, we need to know the
								// relative URL
	...
	public void streamReady(InputStream is, DocumentInfo docInfo) {
		...
		if (isr == null) {
			...
			setPageStatus(HTMLCallback.STATUS_ERROR);
			isr = getStream("Error connecting to stream", null); // TODO -
																	// dynamic
																	// error
																	// messages
		} else {
	...
	/**
	 * Sets this HTMLComponent to render the document in the specified DOM. Note
	 * that relative links if any will be disabled. To allow relative links with
	 * DOM use setDOM(HTMLElement dom,String baseURL)
	 * 
	 * @param dom
	 ...
	 *            An HTMLElement representing the root of the HTML document
	 * @param baseURL
	 *            The base URL for this DOM (Necessary if document references
	 *            relative links)
	 * @throws IllegalArgumentException
	 *             if the HTMLElement supplied is not an 'html' tag
	 */
	...
	private boolean pageLoading() {
		return ((pageStatus == HTMLCallback.STATUS_REQUESTED) ||
				(pageStatus == HTMLCallback.STATUS_CONNECTED) || (pageStatus == HTMLCallback.STATUS_PARSED));
	...
	/**
	 * Refreshes the current DOM so it any changes done after loading will be
	 * rendered.
	 */
	...
	/**
	 * Called internally after both reading and parsing of the HTML document has
	 * completed
	 * 
	 * @param pageURL
	 *            The URL of the page
		...
		if ((!cancelled) || (cancelledCaught)) {
			Display.getInstance().callSerially(new Runnable() {
				...
				public void run() {
					...
					if (pageURL != null) { // pageURL can be null if the page
											// was set using setBodyText and not
											// setPage
						int hash = pageURL.indexOf('#');

						if ((hash != -1) && (pageURL.length() > hash + 1)) { // URL
																				// contains
																				// an
																				// anchor
	...
	/**
	 * Sets whether the active controls in the HTML will trigger events, and
	 * whether the DOM will change dynamically due to user input. If so the
	 * events are dispatched via HTMLCallback methods actionPerformed,
	 * focusGained/Lost, selectionChanged and dataChanged The default is false
	 * in order not to add more overhead if these are not needed.
	 * 
	 * @param enabled
	...
	/**
	 * Determines whether to auto-focus on the first link after page load Note
	 * that focusing will happen only if the link is within a visible range (no
	 * scrolling is performed since this is rarely a wanted behaviour in this
	 * case)
	 * 
	 * @param autoFocus
	...
	void displayPage() {
		...
		// Places the focus on the first link in the page, as long as it is
		// within the visible area of the first page
		if (getComponentForm() != null) {
			getComponentForm().revalidate(); // a revalidate on the component
												// itself does not always result
												// in correct size calculation,
												// thus leaving unreachable
												// elements in the HTML (Note
												// that the first revalidate on
			...
			if (firstFocusable != null) {
				...
				mainContainer.setFocusable(true); // If there are no focused
													// components, the main
													// container will become
													// focusable, thus enabling
													// it to be pixel-scrolled
				getComponentForm().setFocused(mainContainer);
			}
	...
	void setPageStatus(final int status) {
		if ((!pageError) || (status == HTMLCallback.STATUS_REQUESTED)) { // Once
																			// the
																			// page
																			// error
																			// flag
			 ...
			 * +(System.currentTimeMillis()-startTime)); if
			 * (!pageURL.startsWith("file:///")) { clickTimer("Comp");
			 * msg+="Text:"+textTime+"\n"; Dialog.show("Time", msg, "OK",
			 * "Cancel"); } } else if (status==HTMLCallback.STATUS_DISPLAYED) {
			 * System
			 * .out.println("Displayed: "+(System.currentTimeMillis()-startTime
			 * )); clickTimer("Disp"); }
			 */
			if (htmlCallback != null) {
	 ...
	 *            The font size
	 * @param style
	 *            The font style (Font.STYLE_PLAIN or Font.STYLE_ITALIC)
	 * @param weight
	 *            The font weight (Font.STYLE_PLAIN or Font.STYLE_BOLD)
	 * @return the closest font the HTMLComponent could find or null if none
	 *         found.
	 */
	...
	/**
	 * Returns the HTMLFont that holds the given font. This is in fact a
	 * "reverse lookup" for HTMLFonts
	 * 
	 * @param font
	 *            The font to search
	...
	/**
	 * Returns true if there's at least one smallcaps font in the fonts
	 * repository, or false otherwise
	 * 
	 * @return true if there's at least one smallcaps font in the fonts
	 *         repository, or false otherwise
	 */
	...
	/**
	 * A utility method to convert between Element TAG_* constants and HTMLFont
	 * font attribute constants.
	 * 
	 * @param tag
	 *            The tag that describes the font attribute
	 * @param font
	 *            The font for which we want a counterpart font
	 * @return the counterpart font for the given font in the specified tag
	 *         sense.
	 */
	 ...
	 * 
	 * @param attribute
	 *            The requested attribute, one of HTMLFont.BOLD/ITALIC/BIG/SMALL
	 * @param font
	 *            The source font
	...
	HTMLFont getCounterpartFontByAttribute(int attribute, HTMLFont font) {
		...
		while (e.hasMoreElements()) {
			...
			if (hFont.isCounterpart(attribute, font)) {
				if (attribute == HTMLFont.BIG) { // takes the closests font
													// (i.e. big but not
													// biggest)
	...
	/**
	 * Rebuilds the HTMLComponent, this is called usually after a new page was
	 * loaded.
	 */
	private void cleanup() {
		...
		// embeddedCSS=null; // Shouldn't nullify as embedded style tag is
		// collected in the head phase which is before..
		...
		width = Display.getInstance().getDisplayWidth() - getStyle().getMargin(Component.LEFT) - getStyle().getPadding(Component.LEFT) -
				getStyle().getMargin(Component.RIGHT) - getStyle().getPadding(Component.RIGHT) - 10; // The
																										// -10
																										// is
																										// arbitrary
																										// to
	 ...
	 * 
	 * @param linkTag
	 *            AN element containing the LINK tag
	 */
	...
	private void rebuildPage() {
		...
		// Scan for LINK tags directly under the ROOT element, these are in fact
		// converted <?xml-stylesheet ... ?> processing instructions
		// NOTE: Since only single root XML are supported, no converted tags can
		// exist before the HTML tag
		/*
		 * if ((SUPPORT_CSS) && (loadCSS)) { for(int
		 * i=0;i<document.getNumChildren();i++) { HTMLElement
		 * child=(HTMLElement)document.getChildAt(i); if
		 * (child.getId()==HTMLElement.TAG_LINK) { handleLinkTag(child); } } }
		 */

		// Get the HTML root tag and extract the HEAD and BODY (Note that the
		// document tag is ROOT which contains HTML and so on.
		...
		if (html != null) {
			...
			if ((!PROCESS_HTML_MP1_ONLY) && (body == null)) { // frames html
																// does not
																// contain body
																// - but may
																// contain a
		...
		if (head != null) {
			...
			if ((SUPPORT_CSS) && (loadCSS)) { // Scan for LINK tags under the
												// HEAD tag
		...
		if (body != null) {
			...
			if ((SUPPORT_CSS) && (loadCSS)) {
				...
				if (threadQueue.getCSSCount() == -1) { // If there are no
														// pending external CSS,
														// we can already
														// process the CSS
					applyAllCSS(); // Note that this doesn't have to be on EDT
									// as the main container is still not
									// displayed
				}
			}
	...
	/**
	 * Applies the CSS according to the following order (As the spec
	 * determines): - External CSS (CSS files linked with the LINK tag) -
	 * Embedded CSS (STYLE tag) - Inline CSS (STYLE attribute)
	 */
	void applyAllCSS() {
		HTMLElement html = null;
		if (document.getTagId() == HTMLElement.TAG_HTML) {
	...
	/**
	 * Checks if there's a refresh or redirect directive in the META tag and if
	 * so executes accordfingly.
	 * 
	 * @param head
				...
				if ((httpequiv != null) && (httpequiv.equalsIgnoreCase("refresh"))) {
					...
					if (content != null) {
						...
						if (seperator != -1) {
							...
							if (redirectURL.startsWith("url=")) { // i.e.
																	// 10;url=http://....
	...
	/**
	 * Cancels redirects if exists. This is useful when the page has a meta tag
	 * with redirection/refresh in X seconds, and in that time the user clicked
	 * a link
	 */
	...
	/**
	 * Adds the container representing the current line to the current container
	 * and creates a new one
	 */
	private void newLine(int align) {
		if (curLine.getComponentCount() == 0) { // If no components are present,
												// create a vertical spacing in
												// the size of the font height
			curLine.setPreferredH(font.getHeight());
		} else if (maxSuperscript != 0) {
			// The following handles superscript. Bottom margin is added in
			// addString to superscript-ed texts
			// but it doesn't create the effect of superscript, still the
			// components are still drawn at the top.
			// Here we basically transfer the padding to the top, so that
			// superscripted text will have less
			// top margin than non-superscripted text, which will render it as
			// it should.
			// NOTE: handling this is the line level is not an ideal
			// implementation, but other approaches will make it too complicated
	...
	/**
	 * Shows a text that has been marked with the html PRE tag which means the
	 * text will be displayed as is without truncation of white spaces and new
	 * lines when reaching the end of the screen.
	 * 
	 * @param text
	 *            The text to display
	 * @param align
	 *            The current horizontal alignment
	 * @return a vector containing components each representing a sentence
	 *         fragment
	 */
	...
	Vector getWords(String text, int align, boolean returnComps) {
		...
		for (int c = 0; c < text.length(); c++) {
			char ch = text.charAt(c);
			if ((CJK_SUPPORT) &&
					(((ch >= 0x3400) && (ch <= 0x9fff)) || // 4E00-9FFF: CJK
															// Unified
															// Ideographs
															// (Common),
															// 3400-4DFF: CJK
					...
					// Since char can get a max value of 0xffff, the following
					// are not applicable
					// ((ch>=0x20000) && (ch<=0x2a6df)) || //CJK Unified
					// Ideographs Extension B (Rare, historic)
					// ((ch>=0x2f800) && (ch<=0x2fa1f)) //CJK Compatibility
					// Ideographs Supplement (Unifiable variants)
				...
				if ((ch == ' ') || (ch == 10) || (ch == 13) || (ch == '\t') || (ch == '\n')) {
					if (word.length() != 0) {
						...
						word = "";
					} else if ((words.isEmpty()) && (text.length() > 1)) { // The
																			// first
																			// word
																			// can
																			// have
				...
				} else if ((!returnComps) && (font.stringWidth(word + ch) > width - leftIndent)) { // break
																									// words
																									// that
																									// are
																									// longer
	...
	/**
	 * Shows the given text. This method breaks lines as necessary and adds the
	 * text either as regular labels or links.
	 * 
	 * @param text
	 *            The text to display
	 * @param align
	...
	private Vector showTextFixedWidth(String text, int align) {
		...
		if (words.size() > 0) {
			int w = 0;
			String wordStr = "";
			if ((CSSParser.isWhiteSpace(text.charAt(0))) && (curLine.getComponentCount() != 0)) { // leading
																									// space
																									// is
																									// trimmed
																									// if
	...
	/**
	 * Adds the given text to the container as a label or a link. The string
	 * given here does not need line breaking as this was calculated before in
	 * the calling method.
	 * 
	 * @param str
	 *            The text to display
	...
	private Label addString(String str, int align) {
		Label lbl = null;
		int color = textColor;
		if ((curLine.getComponentCount() == 0) && (str.startsWith(" "))) { // First
																			// word
																			// in
																			// paragraph
																			// ignores
		...
		if (link != null) {
			...
			if (accesskey != '\0') {
				addAccessKey(accesskey, lbl, false);// accessKeys.put(new
													// Integer(accesskey), lbl);
				accesskey = '\0'; // To prevent the access key from adding again
									// to all words of the link
			}
			...
			if (labelForID != null) {
				...
				if (accesskey != '\0') {
					addAccessKey(accesskey, lbl, false);// accessKeys.put(new
														// Integer(accesskey),
														// lbl);
					accesskey = '\0'; // To prevent the access key from adding
										// again to all words of the link
				}
				labelForID = null;
			} else {
				lbl = new Label(str);
		...
		if (superscript > 0) {
			...
			if (margin > maxSuperscript) {
				maxSuperscript = margin; // The height superscript margin is
											// saved for a later adjustment at
											// newLine
			}
		...
		// lbl.setVerticalAlignment(Component.BOTTOM); //TODO - This still
		// doesn't align as label alignment in LWUIT refers to the text
		// alignment in relation to its icon (if exists)
		...
		if (align != JUSTIFY) { // Regular LWUIT labels do not support justify.
								// We acheive justification in fixed width mode
								// below
	 ...
	 * 
	 * @param accessKey
	 *            The accesskey key code
	 * @param cmp
	 *            The component that should be focused when the access key is
	 *            pressed
	 * @param override
	 *            If true, cancel any previous accesskey associated with this
	 *            component (Relevant for CSS -wap-accesskey)
	 */
	...
	/**
	 * Overrides initComponent to add the key listeners to the access keys when
	 * the component is first added to the form/displayed This is useful when
	 * the component is added only after the page was read
	 */
	...
	/**
	 * If the component is taken off for any reason, makes sure access keys are
	 * not active
	 */
	protected void deinitialize() {
		...
		// TODO - clean DOM's UI references on deinit? Then what happens on
		// init???
	}

	/**
	 * Handles the IMG tag. This includes calculating its size (if available),
	 * applying any links/accesskeys and adding it to the download queue
	 * 
	 * @param imgElement
	 *            the IMG element
	 * @param align
	 *            th current alignment
	 * @param cmd
	 *            The submit command of a form, used only for INPUT type="image"
	 */
	private void handleImage(HTMLElement imgElement, int align, Command cmd) {
		...
		if (imageUrl != null) {
			...
			if (link != null) { // This image is inside an A tag with HREF
								// attribute
				...
				if (accesskey != '\0') {
					addAccessKey(accesskey, imgLabel, false);// accessKeys.put(new
																// Integer(accesskey),
																// imgLabel);
				}
				...
				if (imageMap.startsWith("#")) { // Image map are denoted by #
												// and then the map name (But we
												// also tolerate if map is
												// specified without #)
			...
			if ((iWidth != 0) || (iHeight != 0)) { // reserve space while
													// loading image if either
													// width or height are
			...
			} else { // If no space is reserved, make a minimal text, otherwise
						// LWUIT won't calculate the size right after the image
						// loads
			...
			// It is important that the padding of the image component itself
			// will be all 0
			// This is because when the image is loaded, its preferred size is
			// checked to see if its width/height were preset by the
					...
					if ((coordsStr != null) && (hrefStr != null)) {
						...
						try {
							...
							if (shape.equalsIgnoreCase("rect")) {
								...
								// coords[2] is the radius, and 0,1 are the
								// center x,y
								curImageMap.addRectArea(new Rectangle(coords[0] - coords[2], coords[1] - coords[2], coords[2] * 2 + 1, coords[2] * 2 + 1), hrefStr);
								// Note: The 'circle' SHAPE in AREA tag is
								// implemented as a rectangle to avoid
								// complication of circle pixel collision
								error = false;
							}
						} catch (Exception e) { // Can be number format
												// exception or index out of
												// bounds
	...
	private void handleInput(HTMLElement element, int align) {
		...
		case INPUT_RESET:
			...
			if (resetCmd == null) {
				resetCmd = new Command(HTMLForm.getDefaultResetText()); // dummy
																		// command
																		// - no
																		// form
																		// so it
		...
		case INPUT_SUBMIT:
			...
			if (submitCmd == null) {
				submitCmd = new Command(value.equals("") ? HTMLForm.getDefaultSubmitText() : value); // dummy
																										// command
																										// -
																										// no
																										// form
			...
			cmp = submitButton;
			break;
		case INPUT_IMAGE: // Image submit is not officially supported in
							// XHTML-MP 1.0 but was added anyway, but pixel data
							// submission is not supported (i.e.
							// name.x=xx&name.y=yy)
			submitCmd = null;
			if (curForm != null) {
				submitCmd = curForm.createSubmitCommand(name, value);
		...
		if (cmp != null) {
			...
			if ((aKey != null) && (aKey.length() == 1)) {
				addAccessKey(aKey.charAt(0), cmp, false);// accessKeys.put(new
															// Integer(aKey.charAt(0)),
															// cmp);
			}
			if (eventsListener != null) {
				eventsListener.registerComponent(cmp, element);
			}
			element.setAssociatedComponents(cmp); // Even if CSS is off, we need
													// to associate it for
													// HTMLElement.getCurentValue
			if ((curForm != null) && (curForm.action == null)) { // Form that
																	// submits
																	// to a
																	// forbidden
																	// link
	...
	/**
	 * Sets input format validation for a TextArea or TextField This is called
	 * from the CSSEngine, and since it is done after the TextArea has been
	 * added it is a bit more complicated
	 * 
	 * @param inputField
	 *            The TextArea to place the input format validation on
	 * @param inputFormat
	 *            The string representing the input format
		...
		if (SUPPORT_INPUT_FORMAT) {
			...
			if (form != null) {
				...
				if (format != null) {
					...
					// Replace operation must be done on the EDT if the form is
					// visible
					if (Display.getInstance().getCurrent() != inputField.getComponentForm()) { // ((inputField.getComponentForm()==null)
																								// ||
						inputField.getParent().replace(inputField, newInputField, null); // Applying
																							// the
																							// constraints
																							// may
																							// return
	 ...
	 * 
	 * @param inputField
	 *            The TextArea to place the input required restriction on
	 * @param inputRequired
	 *            true if input is required (i.e. emptyok=false), false if input
	 *            is not required (i.e. emptyok=true)
	 */
	void setInputRequired(TextArea inputField, boolean inputRequired) {
		if (SUPPORT_INPUT_FORMAT) {
			HTMLForm form = (HTMLForm) textfieldsToForms.get(inputField);
			if (form != null) {
				if (inputRequired) { // Note that input-required is the reverse
										// from emptyok...
	...
	/**
	 * Adds the given component to the curLine container after performing size
	 * checks
	 * 
	 * @param cmp
	 ...
	 * 
	 * @param delta
	 *            The pixels to increment (positive value) or decrement
	 *            (negative value) from the current margin
	 */
	...
	private void handleTableCell(HTMLElement tdTag, int align) {
		...
		while ((trTag != null) && (trTag.getTagId() != HTMLElement.TAG_TR)) { // Though
																				// in
																				// strict
																				// XHTML
																				// TR
		...
		/*
		 * if (trTag!=null) { // Null checks to prevent exceptions for a TD tag
		 * without table etc. HTMLElement
		 * tableTag=(HTMLElement)trTag.getParent(); while ((tableTag!=null) &&
		 * (tableTag.getTagId()!=HTMLElement.TAG_TABLE)) { // Though in strict
		 * XHTML TABLE can only contain TR - in some HTMLs it might be different
		 * tableTag=(HTMLElement)tableTag.getParent(); }
		 * 
		 * if (tableTag!=null) {
		 * border=getInt(tableTag.getAttributeById(HTMLElement.ATTR_BORDER)); }
		 * } cell.getUnselectedStyle().setPadding(border, border, border,
		 * border); cell.getSelectedStyle().setPadding(border, border, border,
		 * border);
		 */
		...
		if (trTag != null) {
			...
			if ((tagId == HTMLElement.TAG_TBODY) || (tagId == HTMLElement.TAG_THEAD) || (tagId == HTMLElement.TAG_TFOOT)) {
				halign = getHorizAlign(tGroupTag.getAttributeById(HTMLElement.ATTR_ALIGN), halign, false); // Get
																											// the
																											// default
																											// TR
																											// alignment
				valign = getVertAlign(tGroupTag.getAttributeById(HTMLElement.ATTR_VALIGN), valign); // Get
																									// the
																									// default
																									// TR
																									// valignment
			}
			halign = getHorizAlign(trTag.getAttributeById(HTMLElement.ATTR_ALIGN), halign, false); // Get
																									// the
																									// default
																									// TR
																									// alignment
			valign = getVertAlign(trTag.getAttributeById(HTMLElement.ATTR_VALIGN), valign); // Get
																							// the
																							// default
																							// TR
																							// valignment
		...
		if ((pW > 0) && (pW < 100)) {
			// constraint.setWidthPercentage(pW); //TODO - Setting a width
			// constraint currently makes the field width 0 - needs to be fixed
			// in TableLayout
		} else {
			pW = getInt(cWidth);
			if (pW != 0) {
		...
		if ((pH > 0) && (pH < 100)) {
			// constraint.setHeightPercentage(pH); //TODO - Setting a height
			// constraint currently makes the field height 0 - needs to be fixed
			// in TableLayout
		} else {
			pH = getInt(cHeight);
			if (pH != 0) {
	...
	/**
	 * Processes the given tag. This is the main processing method that calls
	 * all others and uses itself in a recursive manner.
	 * 
	 * @param element
	...
	private void processTag(HTMLElement element, int align) {
		...
		for (int i = 0; i < element.getNumChildren(); i++) {
			...
			case HTMLElement.TAG_TEXT:
				...
				if ((curComboBox != null) && (optionTag)) { // Text is inside an
															// OPTION tag, i.e.
															// belongs to a
															// ComboBox
				...
				} else if (curTextArea != null) { // Text is inside of a
													// TEXTAREA tag
					curTextArea.setText(text);
					if (curForm != null) {
						curForm.setDefaultValue(curTextArea, text);
					}
				} else if (element.getTagId() == HTMLElement.TAG_LEGEND) { // Note:
																			// this
																			// is
																			// element,
																			// i.e.
				...
				} else if ((curTable != null) && (element.getTagId() == HTMLElement.TAG_CAPTION)) { // Note:
																									// this
																									// is
																									// element,
																									// i.e.
					...
					// long startTextTime=System.currentTimeMillis(); //debug
					...
					// textTime+=(System.currentTimeMillis()-startTextTime);
					// //debug code for performance measurement
				}
				break;
			...
			case HTMLElement.TAG_CENTER: // CENTER is practically DIV
											// align=CENTER
			...
			case HTMLElement.TAG_FIELDSET:
				...
				if (fieldsets.size() == 0) { // First fieldset shouldn't have
												// margin
			...
			case HTMLElement.TAG_MENU:
				...
				if ((ulLevel == 1) && (olIndex == Integer.MIN_VALUE)) { // newline
																		// only
																		// if
																		// it's
																		// the
					...
					listIndent += INDENT_UL; // extra indentation for level 2
												// and beyond
				}
				pushContainer(child);
				break;
			case HTMLElement.TAG_OL:
				...
				if ((olUpperLevelIndex.size() == 0) && (ulLevel == 0)) { // newline
					...
					listIndent += INDENT_OL; // add indent only for second level
												// - first one already gets it
												// from the numbers alignment to
												// a 4-digit number
				}
				pushContainer(child);
				break;
			case HTMLElement.TAG_LI:
				...
				if (((HTMLElement) child.getParent()).getTagId() == HTMLElement.TAG_OL) {
					...
					// following aligns short and long numbers (assuming a
					// 4-digit number as the maximum, as other browsers do)
			...
			case HTMLElement.TAG_SELECT:
				...
				// If a select tag has size specified, it will be shown as an
				// open list, and not as LWUIT combo, even if there's no
				// multiple seection allowed
				...
				child.setAssociatedComponents(curComboBox); // Even if CSS is
															// off, we need to
															// associate it for
															// HTMLElement.getCurentValue
				if (eventsListener != null) {
			...
			case HTMLElement.TAG_TEXTAREA:
				...
				child.setAssociatedComponents(curTextArea); // Even if CSS is
															// off, we need to
															// associate it for
															// HTMLElement.getCurentValue
				String aKey = element.getAttributeById(HTMLElement.ATTR_ACCESSKEY);
				if ((aKey != null) && (aKey.length() == 1)) {
					addAccessKey(aKey.charAt(0), curTextArea, false);// accessKeys.put(new
																		// Integer(aKey.charAt(0)),
																		// curTextArea);
				}
			...
			case HTMLElement.TAG_TABLE:
				...
				width = width / 2; // In fixed width mode we arbitrarily divide
									// the size by a factor knowing that
									// probably there are several cells (If we
									// don't do it, labels inside the cell will
									// be built up to the full width size,
			...
			case HTMLElement.TAG_FONT:
				textColor = HTMLElement.getColor(child.getAttributeById(HTMLElement.ATTR_COLOR), textColor); // TODO
																												// -
																												// This
																												// will
																												// not
			...
			case HTMLElement.TAG_INS: // INS (Inserted text) is rendered exactly
										// like U (underline) in most browsers
				if (underlineCount == 0) {
			...
			case HTMLElement.TAG_DEL:// DEL (Deleted text) is rendered exactly
										// like S (strikethru) in most browsers
			...
			case HTMLElement.TAG_MENU:
				...
				if ((ulLevel == 0) && (olIndex == Integer.MIN_VALUE)) {
					...
					listIndent -= INDENT_UL; // level 2 and beyond got extra
												// indentation, so we remove it
												// here
				}
				listIndent -= INDENT_UL;
				popContainer();
			...
			case HTMLElement.TAG_OL:
				if (olUpperLevelIndex.size() != 0) {
					...
					listIndent -= INDENT_OL; // First level of ordered list
												// doesn't get indentation, so
												// we substract only if it's
												// nested
				} else {
					olIndex = Integer.MIN_VALUE;
				}
				if ((olIndex == Integer.MIN_VALUE) && (ulLevel == 0)) {
					newLine(curAlign); // new line only if it is the last nested
										// list
				}
			...
			case HTMLElement.TAG_LI:
				...
				newLineIfNotEmpty(curAlign);
				// We can't use popContainer, since with LI the container is
				// pushed even when CSS is ignored, to provide the spacing
				// between the list item bullet/number and the text (in a nested
				// way if needed)
			...
			case HTMLElement.TAG_TT:
				...
				break;
			case HTMLElement.TAG_FORM:
				if ((curForm != null) && (!curForm.hasSubmitButton) && (curForm.getNumFields() > 0)) { // This
																										// is
																										// a
																										// fix
																										// for
			...
			case HTMLElement.TAG_TEXTAREA:
				...
				if (curForm != null) { // This was moved to the end tag to
										// enable auto complete support (i.e. if
										// there's an autocomplete it overrides
										// the default value)
			...
			case HTMLElement.TAG_TABLE:
				newLineIfNotEmpty(curAlign);
				curTable.commitRowIfNotEmpty(); // For a case that TR was not
												// closed properly
				if (curTable.getRowCount() != 0) { // Don't add an empty table
													// (Creates an exception in
													// TableLayout and useless)
					/*
					 * if (TABLES_LOCK_SIZE) { for(int
					 * r=0;r<curTable.getRowCount();r++) { for(int
					 * c=0;c<curTable.getColumnCount();c++) { Component
					 * cmp=(Component)curTable.getValueAt(r, c); if (cmp!=null)
						...
						if (padding != -1) {
							for (int r = 0; r < curTable.getRowCount(); r++) {
								for (int c = 0; c < curTable.getColumnCount(); c++) {
									...
									if (cmp != null) { // Can be null for cells
														// that are
														// "spanned over"
				...
				width = width * 2; // In fixed width mode we arbitrarily divide
									// the size by a factor knowing that
									// probably there are several cells - here
									// we restore the size back
	...
	/**
	 * The following replaces the layout of the curLine container with the
	 * correct alignment, in non fixed width alignment is done by flowlayout's
	 * orientation. In case the line is not empty, a new line will be opened by
	 * newLineIfNotEmpty below
	 * 
	 * @param align
	 *            The general alignment of the element
	 * @param curAlign
	 *            The alignment of the specific component we want to
	 */
	...
	/**
	 * Figures out what is the appropriate list type (i.e. which bullet types)
	 * for the unordered list in question
	 * 
	 * @param element
	 *            The element containing the UL tag
	 * @param defaultType
	...
	/**
	 * Figures out the appropriate list type for the given Ordered List element
	 * according to its attributes This calls the
	 * getOrderedListType(Element,int) with LIST_NUMERIC as the default type
	 * 
	 * @param element
	 *            The OL element
	...
	private void pushContainer(HTMLElement element) {
		if (loadCSS) {
			...
			// contCount=(contCount+1)%Element.COLOR_VALS.length; // debug for
			// CSS
	 ...
	 * 
	 * @param quoteElement
	 *            The quote element (TAG_Q)
	 * @param curAlign
	 *            The current horizontal alignment
	 */
	private void addQuote(HTMLElement quoteElement, int curAlign, boolean isStartTag) {
		String quote = null;
		int quoteNum = isStartTag ? 0 : 1; // 0 is the start tag of primary tag
											// and 1 its closing tag. 2 is the
											// start of secondary tag and 3 the
											// closing tag (Used for CSS_QUOTES)
		if (quoteTagCount == 0) {
			quote = "\"";
		} else {
			quote = "'";
	...
	/**
	 * Converts a textual horizontal alignment description to a LWUIT alignment
	 * constant
	 * 
	 * @param alignment
	 *            The string describing the alignment
	 * @param defaultAlign
	 *            The default alignment if the string cannot be converted
	 * @param allowJustify
	 *            true to allow justify alignment, false to return center if
	 *            justify is mentioned
	 * @return Component.LEFT, RIGHT or CENTER or the defaultAlign in case no
	 *         match was found.
	 */
	...
	/**
	 * Converts a textual vertical alignment description to a LWUIT alignment
	 * constant
	 * 
	 * @param alignment
	 *            The string describing the alignment
	 * @param defaultAlign
	 *            The default alignment if the string cannot be converted
	 * @return Component.TOP, BOTTOM or CENTER or the defaultAlign in case no
	 *         match was found.
	 */
	...
	/**
	 * A convenience method to convert a string to an int. This is mainly
	 * intended to save the try-catch for NumericFormatExceptions
	 * 
	 * @param intStr
	 *            The string describing the integer
	 * @param defaultValue
	 *            The value to return if the string is not numeric
	 * @return the integer value of the string, or defaultValue if the string is
	 *         not numeric
	 */
	 ...
	 * @param intStr
	 *            The string describing the integer
	 * @return the integer value of the string, or 0 if the string is not
	 *         numeric
	 */
	...
	/**
	 * A convenience method to convert a string describing a percentage to an
	 * int.
	 * 
	 * @param percent
	 *            The string representing the percentage
	 * @return The percentage integer value (i.e. 80% will return 80) or 0 if
	...
	/**
	 * Calculates width or height of an element according to its original size,
	 * requested size and default size
	 * 
	 * @param origDim
	 *            The original width/height
	 * @param requestedDim
	 *            The string describing the requested width/height either in
	 *            pixels or in percentage
	 * @param defaultDim
	 *            The default width/height to return in case the calculation
	 *            fails
	...
	static int calcSize(int origDim, String requestedDim, int defaultDim, boolean negativeAllowed) {
		...
		} else if (requestedDim.endsWith("px")) { // Pixels can be described
													// either simply as '20' or
													// as '20px'
	...
	/**
	 * Returns the input fields of this HTMLComponent, used by the FOR label
	 * mecahnism.
	 * 
	...
	/**
	 * Focuses on the given component and if it's a checkbox/radiobutton selects
	 * it This is used both for ForLabels and for access keys
	 * 
	 * @param cmp
	 *            The component to focus and select
	 */
	 ...
	 * 
	 * @param url
	 *            The url to convert (Can be relative)
	 * @return The absolute URL representing the given URL in relation to the
	 *         current one.
	 */
	String convertURL(String url) {
		if (docInfo != null) {
			...
			return url; // No conversion is possible if we don't have a
						// DocumentInfo object (in cases of setBody from a
						// string and not from a document address), absolute
						// URLs should have no problem, and as for relative the
						// RequestHandler will have to handle these cases
		}
	}
	...
	public void layoutContainer() {
		if ((FIXED_WIDTH) && (displayWidth != 0) && (Display.getInstance().getDisplayWidth() != displayWidth)) {
			new Thread() {
				public void run() {
					cleanup();
					rebuildPage(); // screen form factor changed -
									// landscape/portrait
				}
			}.start();
		}
	...
	/**
	 * Increments or resets the specified counter (This is CSS related, but
	 * since CSSEngine has no class members for storage, we keep control of it
	 * here)
	 * 
	 * @param counterStr
	 *            The counter-increment/reset property value which is the string
	 *            counter name followed optionally by the factor by which to
	 *            increment or the value to reset to
	 * @param reset
	 *            true to reset, false to increment
	 */
	void incCounter(String counterStr, boolean reset) {
		counterStr = counterStr.trim();
		int value = reset ? 0 : 1; // 0 is the default value to reset to, while
									// 1 is the default increment step
		Vector v = getWords(counterStr, Component.LEFT, false); // align is LEFT
																// but just as a
																// place holder,
																// it doesn't
																// really have
	...
	/**
	 * A label that when clicked focuses (and toggels when applicable) a certain
	 * input field. This implements the LABEL html tag
	 * 
	 * @author Ofir Leitner
	 */
	...
	class HTMLListIndex extends HTMLListItem {
		...
		/**
		 * Converts a number to a roman numeral (Up to 99, should suffice for
		 * HTML lists...)
		 * 
		 * @param index
		 *            The list index to convert
	...
	class HTMLBullet extends HTMLListItem {
		...
		public void setImage(String imageUrl) {
			if (imageUrl != null) {
				// setText(" "); // Due to a LWUIT bug Labels with an icon only
				// and no text can be cut
	...
	/**
	 * HTMLComboBox overrides ComboBox to allow usage of MultiComboBox as its
	 * list. This is done for OPTGROUP labels support (Note that multiple
	 * features of MultiComboBox will be switched off)
	 * 
	 * @author Ofir Leitner
	 */

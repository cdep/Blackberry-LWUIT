...
class CSSEngine {
	...
	private static int DEFAULT_3D_BORDER_COLOR = 0x9a9a9a; // default color for
															// outset/inset/ridge/groove

	// int count; //for debugging
	private static CSSEngine instance; // The instance of this singleton class
	private static Hashtable specialKeys; // A hashtable containing all
											// recognized special key strings
											// and their keycodes
	private Hashtable matchingFonts = new Hashtable(); // A hashtable used as a
														// cache for quick find
														// of matching fonts

	/**
	 * A list of the attributes that can contain a URL, in order to scan them
	 * and update relative URLs to an absolute one
	 */
	private static final int[] URL_ATTRIBUTES = { CSSElement.CSS_BACKGROUND_IMAGE, CSSElement.CSS_LIST_STYLE_IMAGE };

	/**
	 * Denotes that the selector should be applied to the unselected style of
	 * the component
	 */
	final static int STYLE_UNSELECTED = 1;

	/**
	 * Denotes that the selector should be applied to the selected style of the
	 * component
	 */
	final static int STYLE_SELECTED = 2;

	/**
	 * Denotes that the selector should be applied to the pressed style of the
	 * component
	 */
	final static int STYLE_PRESSED = 4;

	/**
	 * The indentation applied on a list when its 'list-style-position' is
	 * 'inside' vs. 'outside'
	 */
	...
	private static final int VISIBILITY_COLLAPSE = 2; // collapse behaves the
														// same as hidden in
														// most browsers.
	 ...
	 * 
	 * @param css
	 *            A css vector holding CSSElements, where each element holds CSS
	 *            selectors as its children
	...
	private CSSElement[] sortSelectorsBySpecificity(CSSElement[] css) {
		...
		for (int s = 0; s < css.length; s++) {
			...
			for (int iter = 0; iter < cssRoot.getNumChildren(); iter++) {
				...
				if (cssPageURL != null) { // Since with external CSS pages, the
											// base URL is that of the CSS file
											// and not of the HTML document, we
											// have to convert relative image
				...
				int i = 0;
				int specificity = currentSelector.getSelectorSpecificity(); // Note
																			// that
																			// it
																			// is
																			// important
	...
	/**
	 * Applies all CSS directives to the given document and HTMLComponent,
	 * including external CSS files, embedded CSS segments and inline CSS (Style
	 * attribute) This is called by HTMLComponent after the document was fully
	 * parsed and all external CSS have been retrieved. This method actually
	 * initializes a sorted CSS array to be used by the recursive private
	 * applyCSS method.
	 * 
	 * @param document
	 *            The HTML document to apply the CSS on
	 * @param htmlC
	 *            The HTMLComponent to apply the CSS on
	 * @param externalCSS
	 *            A vector containing CSSElelemnts each being the root of
	 *            external CSS file (1 per file)
	 * @param embeddedCSS
	 *            A vector containing CSSElelemnts each being the root of
	 *            embedded CSS segments (1 per segment)
	 */
	...
	/**
	 * Applies the style attribute in the specified element and all of its
	 * descendants (where exists) This method is used when no external and
	 * embedded CSS segments exists, to speed up the process of CSS application
	 * 
	 * @param element
	...
	/**
	 * A recursive method that tries to match all CSS selectors with the
	 * specified element
	 * 
	 * @param element
	 ...
	 *            The HTMLComponent to apply the CSS on
	 * @param css
	 *            An array containing selectors sorted by specificity from all
	 *            the external CSS files and then the embedded CSS segments
	 * @param nestedSelectors
	 *            A vector containing nested selectors, or null if none
	 */
	private Vector applyCSS(HTMLElement element, HTMLComponent htmlC, CSSElement[] css, Vector nestedSelectors, Vector siblingSelectors) { // Vector
																																			// styleAttributes
		...
		if (!HTMLComponent.PROCESS_HTML_MP1_ONLY) { // sibling selectors are not
													// supported in HTML-MP1
		...
		for (int i = 0; i < element.getNumChildren(); i++) {
			...
			if (!child.isTextElement()) { // Sibling selectors skip text
											// elements
	...
	/**
	 * Checks if the given selector matches the given element either by its tag
	 * name, class name or id. If there's a match but the selector has children,
	 * it means that it is a nested selector, and thus its only child is added
	 * to the nested selectors vector to be checked against the children of this
	 * element in the next recursion of applyCSS
	 * 
	 * @param currentSelector
	 *            The current CSS selector to check
	 * @param element
	 *            The element to check
	 * @param htmlC
	 *            The HTMLComponent
	 * @param className
	 *            The element's class name (Can be derived from element but
	 *            since this method is called a lot it is extracted before and
	 *            sent as a parameter)
	 * @param id
	 *            The element's id (Same comment as in className)
	 * @param nextNestedSelectors
	 *            A vector containing the nested selectors
	 */
		...
		if (((currentSelector.getSelectorTag() == null) || ((!element.isTextElement()) && (currentSelector.getSelectorTag().equalsIgnoreCase(element.getTagName())))) &&
			...
			if (currentSelector.getNumChildren() == 0) {
				if ((element.getTagId() != HTMLElement.TAG_A) ||
						((currentSelector.getSelectorPseudoClass() & (CSSElement.PC_LINK + CSSElement.PC_VISITED)) == 0) || // not
																															// link/visited
																															// (but
																															// can
																															// be
				...
				if (child.siblingSelector) {
					if (!HTMLComponent.PROCESS_HTML_MP1_ONLY) { // sibling
																// selectors are
																// not supported
																// in HTML-MP1
					...
					// Check if this is a Descendant selector (i.e. div b -
					// which means match any b that is the descendant of div
					// If so then we pass not only the child selector (i.e. the
					// b) but also the "* b" to allow matching later decendants
	...
	/**
	 * Checks if the specified class is contained in the specified text This is
	 * used for elements that have several classes i.e. class="class1 class2"
	 * Note: A simple indexOf could not be used since we need to find whole
	 * words and not frgaments of words
	 * 
	 * @param selectorClass
	 *            The text
	 * @param elementClass
	...
	private boolean containsClass(String elementClass, String selectorClass) {
		...
		// The spaces addition is to make sure we get a whole word and not a
		// fragment of a word
		elementClass = " " + elementClass + " ";

		// Selector can require multiple classes, i.e. class.1class2 (which
		// needs to match to "class 1 class2" and "class2 class1" and also
		// "class1 otherclasses class2"
	 ...
	 *            The element to apply the style to
	 * @param selector
	 *            The selector containing the style directives
	 * @param htmlC
	 *            The HTMLComponent
	 */
	...
	/**
	 * Returns a mask of the STYLE_* constants of which LWUIT styles this
	 * selector should be applied to
	 * 
	 * @param cmp
	 *            The component in question
	 * @param selector
	 *            The selector
	 * @return a mask of the STYLE_* constants of which LWUIT styles this
	 *         selector should be applied to
	 */
	private int getApplicableStyles(Component cmp, CSSElement selector) {
		...
		if (cmp instanceof HTMLLink) {
			...
			boolean done = false;
			if ((pseudoClass & CSSElement.PC_FOCUS) != 0) { // Focused (i.e. CSS
															// focus/hover)
				result |= STYLE_SELECTED;
				done = true;
			}
			if ((pseudoClass & CSSElement.PC_ACTIVE) != 0) { // active in CSS
																// means pressed
																// in LWUIT
	...
	/**
	 * Sets the specified color as the foreground color of the component and all
	 * its children
	 * 
	 * @param cmp
	 ...
	 *            The color to set
	 * @param selector
	 *            The selector with the color directive
	 */
	private void setColorRecursive(Component cmp, int color, CSSElement selector) {
		...
		if (cmp instanceof Container) {
			...
			for (int i = 0; i < cont.getComponentCount(); i++) {
				if (!(cont.getComponentAt(i) instanceof HTMLLink)) { // A link
																		// color
																		// is a
																		// special
																		// case,
	...
	/**
	 * Sets the font of the component and all its children to the closest font
	 * that can be found according to the specified properties
	 * 
	 * @param htmlC
	 *            The HTMLComponent this component belongs to (For the available
	 *            bitmap fonts table)
	 * @param cmp
	 *            The component to work on
	 * @param fontFamily
	 ...
	 * @param fontStyle
	 *            The font style - either Font.STYLE_PLAIN or Font.STYLE_ITALIC
	 * @param fontWeight
	 *            The font weight - either Font.STYLE_PLAIN ot Font.STYLE_BOLD
	 * @param selector
	 *            The selector with the font directive
	 */
	...
	/**
	 * Usually we don't have to set visibility in a recursive manner, i.e.
	 * suffices to set a top level container as invisible and all its contents
	 * are invisible. However, in CSS it is possible that a top level element
	 * has visibility:hidden and some child of his has visibility:visible, and
	 * then what we do is use the setVisibleParents to make sure all containers
	 * containing this child are visible. But since other child components still
	 * need to be invsibile - we make sure that all are invisible with this
	 * method.
	 * 
	 * @param cmp
	 *            The component to set visibility on
	 * @param visible
	 *            true to set visible and enabled, false otherwise
	 */
	...
	// TODO - This is a problematic implementation since if a text has been
	// converted to UPPERCASE and then due to a child's style attribute it has
	// to change back to none/capitalize - there's no way to restore the
	// original text.
	// Also it has a problem with FIXED_WIDTH mode since when uppercasing for
	// example, labels will grow in size which will take some of them out of the
	// screen, The correct way is working on the elements and not the text, and
	 ...
	 *            The component to work on
	 * @param transformType
	 *            The text transform type, one of the TEXT_TRANSFORM_* constants
	 */
			...
			case TEXT_TRANSFORM_CAPITALIZE:
				...
				for (int i = 0; i < text.length(); i++) {
					...
					if (CSSParser.isWhiteSpace(c)) {
						...
						if ((c >= 'a') && (c <= 'z')) {
							c -= 32; // 'A' is ASCII 65, and 'a' is ASCII 97,
										// difference: 32
						}
						capNextLetter = false;
					}
	...
	/**
	 * Sets the alignment of the component and all its children according to the
	 * given alignment
	 * 
	 * @param cmp
	 *            The component to set the alignment on
	 * @param align
	 *            The alignment - one of left,center,right
	 */
	private void setTextAlignmentRecursive(Component cmp, int align) {
		...
		} else if ((HTMLComponent.FIXED_WIDTH) && (cmp instanceof Label)) { // In
																			// FIXED_WIDTH
																			// mode
																			// labels
																			// are
	 ...
	 * Note: This doesn't really work well with HTMLComponent.FIXED_WIDTH mode
	 * since labels there are not single words but rather the whole line, so
	 * they get pushed out of the screen
	 * 
	 * @param cmp
	 *            The component to set the indentation on
	 * @param indent
	...
	private void setTextIndentationRecursive(Component cmp, int indent) {
		if (cmp instanceof Container) {
			...
			if ((cont.getLayout() instanceof FlowLayout) && (cont.getComponentCount() > 0)) {
				// Note that we don't need to consider the "applicable" styles,
				// as this is a container and will always return
				// selected+unselected
	...
	/**
	 * Replaces an unwrapped text with a wrapped version, while copying the
	 * style of the original text.
	 * 
	 * @param label
	 *            The current label that contains the unwrapped text
	 * @param words
	 *            A vector containing one word of the text (without white
	 *            spaces) in each element
	 * @param element
	 *            The text element
	 */
	...
	/**
	 * Sets this element and all children to have wrapped text. In cases where
	 * text is already wrapped no change will be made. This will work only in
	 * FIXED_WIDTH mode (Checked before called) Technically all this logic can
	 * be found in HTMLComponent.showText, but since we don't want to get into
	 * the context of this element (i.e. what was the indentation, alignment
	 * etc.), we use this algorithm.
	 * 
	 * @param element
	...
	private void setWrapRecursive(HTMLElement element, HTMLComponent htmlC) {
		if (element.isTextElement()) {
			...
			if ((text != null) && (ui != null) && (ui.size() == 1)) { // If it's
																		// already
																		// wrapped,
																		// no
																		// need
	...
	/**
	 * Replaces a wrapped text with an unwrapped version. This in fact removes
	 * all the labels that contains a single word each, and replaces them with
	 * one label that contains the whole text. This way the label is not
	 * wrapped.
	 * 
	 * @param label
	 *            The first label of this text element (can be derived from ui
	 *            but already fetched before the call)
	 * @param ui
	 *            The vector consisting all components (labels) that contain the
	 *            text's words
	 * @param newText
	 *            The new text that should replace current components (unwrapped
	 *            text without extra white spaces)
	 * @param element
	 *            The text element
	 */
	private void setNowrapText(Label label, Vector ui, String newText, HTMLElement element) {
		...
		if (label instanceof HTMLLink) {
			((HTMLLink) label).childLinks = new Vector(); // Reset all
															// associated link
															// fragments so we
															// don't have
															// unneeded
	...
	/**
	 * Sets this element and all children to have unwrapped text. In cases where
	 * text is already unwrapped no change will be made. This will work only in
	 * FIXED_WIDTH mode (Checked before called) Technically a lot of this logic
	 * can be found in HTMLComponent, but since we don't want to get into the
	 * context of this element (i.e. what was the indentation, alignment etc.),
	...
	private void setNowrapRecursive(final HTMLElement element) {
		...
		element.recalcUi(); // If children elements' UI was changed, we need to
							// recalc the UI of the parent

	}

	// //////
	// CSS2 additions - the following are not in the WCSS spec, but rather in
	// the CSS2 spec
	// /////
	 ...
	 * 
	 * @param cmp
	 *            The component to set the spacing on
	 * @param spacing
	...
	private void setWordSpacingRecursive(Component cmp, int spacing) {
		if (cmp instanceof Container) {
			...
			for (int i = 0; i < cont.getComponentCount(); i++) {
				...
				(cmp.getParent().getComponentIndex(cmp) < cmp.getParent().getComponentCount() - 1)) { // don't
																										// apply
																										// to
																										// the
																										// last
	 ...
	 *            The component to set the spacing on
	 * @param halfHeight
	 *            Half of the line height in pixels (will be added to top and
	 *            bottom margins to make for a full height)
	 */
	 ...
	 * 
	 * @param style
	 *            The style to add the decoration to
	 * @param decoration
	...
	/**
	 * Sets the specified decoration to the specified components and all of its
	 * Label descendants
	 * 
	 * @param cmp
	 *            The component to apply the decoration to
	 * @param decoration
	 *            The deocration (One of Style.TEXT_DECORATION_* constants)
	 * @param selector
	 *            The selector with the text direction directive
	 */
	...
	/**
	 * Removes all text decorations from the specified components and its Label
	 * descendants This will be used for {text-decoration: none}
	 * 
	 * @param cmp
	 *            The component to remove decorations from
	 * @param selector
	 *            The selector with the text direction directive
	 */
	 ...
	 * 
	 * @param cmp
	 *            The component to change the quotes in
	 * @param quotes
	 *            an array with 4 strings representing how quotes should look
	 *            like (primary start,primary end,secondary start,secondary end)
	 */
	 ...
	 * 
	 * @param ui
	 *            The component representing (part of) the element that the
	 *            style should be applied to
	 * @param selector
	 *            The style attributes relating to this element
	 * @param element
	 *            The element the style should be applied to
	 * @param htmlC
	 *            The HTMLComponent to which this element belongs to
	 * @param focus
	 *            true if the style should be applied only to the selected state
	 *            iof the ui (a result of pseudo-class selector a:focus etc.)
	 */
	private void applyStyleToUIElement(Component ui, CSSElement selector, HTMLElement element, HTMLComponent htmlC) {
		// count++;
		int styles = getApplicableStyles(ui, selector); // This is relevant only
														// for non recursive
														// types - otherwise we
														// need to recheck
														// everytime since it
		...
		// White spaces
		if (HTMLComponent.FIXED_WIDTH) { // This works well only in fixed width
											// mode (Since we cannot "force" a
											// newline in FlowLayout)
		...
		if ((v != null) && ((element.getTagId() == HTMLElement.TAG_TEXTAREA) || (element.getTagId() == HTMLElement.TAG_INPUT)) && (ui instanceof TextArea)) {
			...
			ui = htmlC.setInputFormat((TextArea) ui, v); // This may return a
															// new instance of
															// TextField taht
															// has to be updated
															// in the tree. This
															...
															// first thing
															// checked - see
															// HTMLInputFormat.applyConstraints
			element.setAssociatedComponents(ui);
		}
		...
		case DISPLAY_MARQUEE: // Animate component (ticker-like)
			...
			break;
		// TODO - support also: block, inline and list-item (All mandatory in
		// WCSS)
		}
		...
		if (visibility != -1) {
			...
			if (!visible) {
				return; // Don't waste time on processing hidden elements,
						// though technically the size of the element is still
						// reserved and should be according to style
			} else {
				setParentsVisible(ui); // Need to turn on visibility of all
										// component's parents, in case they
										// were declared hidden
			}
		}
		...
		// TODO - debug: Width and Height don't always work - for simple
		// components they usually do, but for containers either they don't have
		// any effect or some inner components (with size restrictions)
		// disappear
		// We use the entire display width and height as reference since htmlC
		// still doesn't have a preferred size or actual size
		// Width

		// TODO - Width/Height is disabled currently, since it causes a lot of
		// side effects, making some components disappear
		/*
		 * int width=selector.getAttrLengthVal(CSSElement.CSS_WIDTH,ui,Display.
		 * getInstance().getDisplayWidth());
		 * 
		 * // Height int
		 * height=selector.getAttrLengthVal(CSSElement.CSS_HEIGHT,ui
		 * ,Display.getInstance().getDisplayHeight());
		 * 
		 * if (!HTMLComponent.PROCESS_HTML_MP1_ONLY) { int
		 * minWidth=selector.getAttrLengthVal
		 * (CSSElement.CSS_MIN_WIDTH,ui,Display
		 * .getInstance().getDisplayWidth()); int
		 * maxWidth=selector.getAttrLengthVal
		 * (CSSElement.CSS_MAX_WIDTH,ui,Display
		 * .getInstance().getDisplayWidth()); int
		 * minHeight=selector.getAttrLengthVal
		 * (CSSElement.CSS_MIN_HEIGHT,ui,Display
		 * .getInstance().getDisplayHeight()); int
		 * maxHeight=selector.getAttrLengthVal
		 * (CSSElement.CSS_MAX_HEIGHT,ui,Display
		 ...
		 * width=minWidth; } if ((maxWidth!=-1) &&
		 * (maxWidth<ui.getPreferredW())) { width=maxWidth; } } if (height==-1)
		 * { // process min/max only if exact was not specified if
		 * ((minHeight!=-1) && (minHeight>ui.getPreferredH())) {
		 * height=minHeight; } if ((maxHeight!=-1) &&
		 * (maxHeight<ui.getPreferredH())) { height=maxHeight; } } }
		 * 
		 * if ((width!=-1) || (height!=-1)) { if (width==-1) {
		 * width=ui.getPreferredW(); } if (height==-1) {
		 * height=ui.getPreferredH(); } ui.setPreferredSize(new
		...
		if (v != null) {
			...
			if (url != null) {
				...
				// Setting an alternative bgPainter that can support CSS
				...
				// Note that we don't set transparency to 255, since the image
				// may have its own transparency/opaque areas - we don't want to
				// block the entire component/container entirely
				if ((styles & STYLE_SELECTED) != 0) {
				...
				// Background attachment: Either 'fixed' (i.e. the bg image is
				// fixed in its position even when scrolling)
				// or 'scroll' (default) which means the it moves with scrolling
				// (Like usually in LWUIT backgrounds)
		...
		if (ui instanceof Label) { // If this is a Label/HTMLLink we do not
									// apply the margin individually to each
									// word, but rather to the whole block
		...
		for (int i = CSSElement.CSS_MARGIN_TOP; i <= CSSElement.CSS_MARGIN_RIGHT; i++) {
			...
			if ((i == CSSElement.CSS_MARGIN_TOP) || (i == CSSElement.CSS_MARGIN_BOTTOM)) {
				marginPixels = selector.getAttrLengthVal(i, ui, htmlC.getHeight()); // Here
																					// the
																					// used
																					// component
																					// is
			...
			if (marginPixels >= 0 && marginComp != null) { // Only positive or 0
				...
				if ((styles & STYLE_SELECTED) != 0) {
					...
					// If this is a link and the selector applies only to
					// Selected, it means this is an 'a:focus'
					// Since the marginComp is not focusable (as it is the
					// container holding the link), HTMLLink takes care of
					// focusing the
				...
				// Since we don't apply the margin/padding on the component but
				// rather on its parent
				// There is no point in setting the PRESSED style since we don't
				// have a pressed event from Button, nor do we have a
				// pressedStyle for containers
				// That's why we can't do the same trick as in selected style,
				// and the benefit of this rather "edge" case (That is anyway
				// not implemented in all browsers) seems rather small
				// if ((styles & STYLE_PRESSED)!=0) {
				// ((HTMLLink)ui).getPressedStyle().setMargin(i-CSSElement.CSS_MARGIN_TOP,
				// marginPixels);
				// }
			}
		}
		...
		for (int i = CSSElement.CSS_PADDING_TOP; i <= CSSElement.CSS_PADDING_RIGHT; i++) {
			...
			if (padPixels >= 0) { // Only positive or 0
				if ((styles & STYLE_SELECTED) != 0) {
					...
					if ((ui instanceof HTMLLink) && (styles == STYLE_SELECTED)) { // See
																					// comment
																					// on
																					// margins
						((HTMLLink) ui).setParentChangesOnFocus();
				...
				// if ((styles & STYLE_PRESSED)!=0) {
				// ((HTMLLink)padComp).getPressedStyle().setPadding(i-CSSElement.CSS_PADDING_TOP,
				// padPixels);
				// }
			}
		}
		...
		if (align != -1) {
			...
			case HTMLElement.TAG_TABLE:
				...
				break;
			default:
				setTextAlignmentRecursive(ui, align); // TODO - this sometimes
		...
		if (valign != -1) {
			...
			case HTMLElement.TAG_TR:
				...
				break;
			// case Element.TAG_TABLE: // vertical alignment denoted in the
			// table tag doesn't affect it in most browsers
		...
		if (indent >= 0) { // Only positive (0 also as it may cancel previous
							// margins)
		...
		if (fontFamily != null) {
			...
			if (index != -1) { // Currently we ignore font families fall back
								// (i.e. Arial,Helvetica,Sans-serif) since even
								// finding a match for one font is quite
		...
		// Since LWUIT/J2ME doesn't support small-caps fonts, when a small-caps
		// font varinat is requested
		// the font-family is changed to "smallcaps" which should be loaded to
		// HTMLComponent and the theme as a bitmap font
		// If no smallcaps font is found at all, then the family stays the same,
		// but if even only one is found - the best match will be used.
		...
		// Process font only if once of the font CSS properties was mentioned
		// and valid
		...
		if ((element.getTagId() == HTMLElement.TAG_LI) || (element.getTagId() == HTMLElement.TAG_UL) || (element.getTagId() == HTMLElement.TAG_OL) || (element.getTagId() == HTMLElement.TAG_DIR) || (element.getTagId() == HTMLElement.TAG_MENU)) {
			...
			if (listPos == LIST_STYLE_POSITION_INSIDE) {
				// Padding and not margin since background color should affect
				// also the indented space
		...
		boolean leftBorder = false; // Used to prevent drawing a border in the
									// middle of two words in the same segment
		boolean rightBorder = false; // Used to prevent drawing a border in the
										// middle of two words in the same
										// segment
		...
		if (hasBorder) {
			...
			// In case this element was assigned a top border for instance, and
			// then by belonging to another tag/class/id it has also a bottom
			// border - this merges the two (and gives priority to the new one)
			if ((curBorder != null) && (curBorder.getCompoundBorders() != null)) { // TODO
																					// -
																					// This
																					// doesn't
																					// cover
		...
		if ((v != null) && (v.length() >= 1) &&
				((element.getTagId() == HTMLElement.TAG_INPUT) || // These are
																	// the only
																	// tags that
																	// can
																	// accpet an
				...
				((element.getTagId() == HTMLElement.TAG_A) && (ui instanceof HTMLLink) && ((HTMLLink) ui).parentLink == null)) // For
																																// A
																																// tags
																																// this
																																// is
			...
			// The accesskey string may consist fallback assignments (comma
			// seperated) and multiple assignments (space seperated) and any
			// combination of those
			// For example: "send *, #" (meaning: assign both the send and *
			// keys, and if failed to assign one of those assign the # key
			// instead)
			int index = v.indexOf(',');
			...
			while (index != -1) { // Handle fallback access keys
				...
				if (assigned) {
					break; // comma denotes fallback, and once we succeeded
							// assigning the accesskey, the others are
							// irrelevant
				}
				index = v.indexOf(',');
			}
		...
		if (!HTMLComponent.PROCESS_HTML_MP1_ONLY) {
			...
			// Text decoration (In HTML-MP1 the only mandatory decoration is
			// 'none')
			...
			if (!HTMLComponent.FIXED_WIDTH) {
				int wordSpace = selector.getAttrLengthVal(CSSElement.CSS_WORD_SPACING, ui, 0); // The
																								// relative
																								// dimension
																								// is
																								// 0,
			...
			// Technically the font height should be queried when actually
			// resizing the line (since it may differ for a big block) - but
			// since this would be ery time consuming and also major browsers
			// don't take it into account - we'll do the same
			// int
			// lineHeight=selector.getAttrLengthVal(CSSElement.CSS_LINE_HEIGHT,
			// ui, ui.getStyle().getFont().getHeight());
			int lineHeight = selector.getAttrLengthVal(CSSElement.CSS_LINE_HEIGHT, ui, ui.getStyle().getFont().getHeight());

			if (lineHeight != -1) {
				lineHeight = Math.max(0, lineHeight - ui.getStyle().getFont().getHeight()); // 100%
																							// means
																							// normal
																							// line
																							// height
			...
			if (quotesStr != null) {
				...
				if ((size == 2) || (size == 4)) {
					...
					if (size == 2) { // If only 2 quotes are specified they are
			...
			if (ui instanceof HTMLTable) {
				...
				tableProp = selector.getAttrVal(CSSElement.CSS_CAPTION_SIDE); // bottom
																				// =
																				// 0
																				// ,
																				// top
				...
				if (tableProp != -1) {
					...
					int tablePos = tableParentCont.getComponentIndex(ui); // should
																			// result
																			// in
																			// 0
																			// when
		...
		// Note that we carefully check the structure of the LI/UL/OL element
		// with instanceof and checking component count
		// This is since in some cases other elements can come between a OL/UL
		// and its LI items (Though illegal in HTML, it can occur)
	 ...
	 * 
	 * @param ui
	 *            The component representing the table (a HTMLTable)
	 * @param align
	 *            The alignment
	 * @param isHorizontal
	 *            true for horizontal alignment, false for vertical alignment
	 */
	...
	/**
	 * Sets the table cell 'ui' to the requested alignment Note that when called
	 * directly (on TAG_TR) this is actually called multiple times, each with a
	 * different cell of the row as 'ui'. This happens since TR elements contain
	 * all their cells as their UI and as such, applyStyle will call
	 * applyToUIElement each time with another cell
	 * 
	 * @param trTag
	 *            The element representing the table row (TR tag) who is the
	 *            parent of the cell we want to modify
	 * @param ui
	 *            The component representing the table (a HTMLTable)
	 * @param align
	 *            The alignment
	...
	private void setTableCellAlignmentTR(HTMLElement trTag, Component ui, int align, boolean isHorizontal) {
		if ((trTag != null) && (trTag.getTagId() == HTMLElement.TAG_TR)) {
			...
			while ((tableTag != null) && (tableTag.getTagId() != HTMLElement.TAG_TABLE)) { // Though
																							// in
																							// strict
																							// XHTML
																							// TABLE
			...
			if ((tableTag != null) && (tableTag.getTagId() == HTMLElement.TAG_TABLE)) {
				...
				table.setModel(model); // Setting the same model again causes
										// re-evaluation of the constraints
	...
	/**
	 * Tries to assign the given key string as an access key to the specified
	 * component The key string given here may consist of a multiple key
	 * assignment, i.e. several keys seperated with space
	 * 
	 * @param keyStr
	 *            The string representing the key (either a character, a unicode
	 *            escape sequence or a special key name
	 * @param htmlC
	 *            The HTMLComponent
	 * @param ui
	...
	private boolean processAccessKeys(String keyStr, HTMLComponent htmlC, Component ui) {
		...
		boolean isFirstKey = true; // Keeps track of whether this is the first
									// key we are adding (In order to override
									// XHTML accesskey or failed multiple
									// assignments)
		while (index != -1) { // Handle multiple/fallback access keys
			...
			if (!processAccessKey(key, htmlC, ui, isFirstKey)) {
				return false; // If failing to set one of the keys - we return a
								// failure
			}
			isFirstKey = false;
	...
	/**
	 * Tries to assign the given key string as an access key to the specified
	 * component The key string given here is a single key
	 * 
	 * @param keyStr
	 *            The string representing the key (either a character, a unicode
	 *            escape sequence or a special key name
	 ...
	 *            The component to set the access key on
	 * @param override
	 *            If true overrides other keys assigned previously for this
	 *            component
	 * @return true if successful, false otherwise
	 */
	private boolean processAccessKey(String keyStr, HTMLComponent htmlC, Component ui, boolean override) {
		if (keyStr.startsWith("\\")) { // Unicode escape sequence, may be used
										// to denote * and # which technically
										// are illegal as values
			try {
	 ...
	 *            The style attributes element containing the border directives
	 * @param ui
	 *            The component we want to set the border on
	 * @param location
	 *            One of Component.TOP/BOTTOM/LEFT/RIGHT
	 * @return
	 */
	...
	static String omitQuotesIfExist(String str) {
		...
		if (((str.charAt(0) == '\"') || (str.charAt(0) == '\'')) && (str.length() >= 2)) {
			str = str.substring(1, str.length() - 1); // omit quotes from both
														// sides
		}
		return str;
	}
	...
	/**
	 * Sets the font of the component to the closest font that can be found
	 * according to the specified properties Note that system fonts will be
	 * matched only with system fonts and same goes for bitmap fonts
	 * 
	 * @param htmlC
	 *            The HTMLComponent this component belongs to (For the available
	 *            bitmap fonts table)
	...
	private void setMatchingFont(HTMLComponent htmlC, Component cmp, String fontFamily, int fontSize, int fontStyle, int fontWeight, CSSElement selector) {
		...
		if (curFont.getCharset() == null) { // system font
			// The family string in system fonts is just used to index the font
			// in the matchingFonts cache hashtable
			...
			curSize = curFont.getHeight() - 2; // Font height is roughly 2-3
												// pixels above the font size,
												// and is the best indicator we
												// have to what the system font
												// size is
		...
		if (((fontFamily != null) && (curFamily != null) && (!fontFamily.equalsIgnoreCase(curFamily))) ||
				...
				((isItalic) != (fontWeight == Font.STYLE_ITALIC))) { // This
																		// checks
																		// if
																		// there's
																		// a
			...
			// Set the unspecified attributes of the requested font to match
	...
	static String getCSSUrl(String cssURL) {
		if ((cssURL != null) && (cssURL.toLowerCase().startsWith("url("))) {
			...
			if (index != -1) {
				...
				cssURL = cssURL.trim(); // According to the CSS spec, a URL can
										// have white space before/after the
										// quotes
	 ...
	 * 
	 * @param open
	 *            true if this is an opening quote, false otherwise
	 * @return The quote label
	 */
	private Label getQuote(boolean open) {
		Label quoteLabel = new Label("\"");
		quoteLabel.putClientProperty(HTMLComponent.CLIENT_PROPERTY_QUOTE, new Integer(open ? 0 : 1)); // 0
																										// is
																										// open
																										// quote,
																										// 1
																										...
																										// -
																										// see
																										// HTMLComponent.addQuote)
		return quoteLabel;

	}

	/**
	 * Evaluates a CSS content property expression and returns the matching
	 * label component
	 * 
	 * @param htmlC
	 *            The HTMLComponent
	 ...
	 * @param element
	 *            The element this content property
	 * @param selector
	 *            The CSS selector that includes the content property (mainly
	 *            for error messages)
	 * @return A label representing the evaluated expression or null if not
	 *         found
	 */
	 ...
	 *            The element this content property
	 * @param selector
	 *            The CSS selector that includes the content property
	 * @param htmlC
	 *            The HTMLComponent
	 */
		...
		if (content != null) { // if there's no content, we don't add anything
			...
			if (cmp instanceof Container) {
				...
				while ((parent.getComponentCount() > 0) && (parent.getComponentAt(after ? parent.getComponentCount() - 1 : 0) instanceof Container)) {
					parent = (Container) parent.getComponentAt(after ? parent.getComponentCount() - 1 : 0); // find
																											// the
																											// actual
																											// content
				}
			...
			String str = "";
			content = content + " "; // to make sure the last expression is
										// evaluated, note that this will not
										// print an extra space in any case,
										// since it is out of the quotes if any
			...
			for (int i = 0; i < content.length(); i++) {
				...
				if (c == '"') {
					...
					if (segment) {
						...
						if (lbl == null) { // if we didn't find a match we
											// search for the following
											// expressions which are used to
											// remove added content
							int removeQuoteType = -1;
							boolean removeAll = false;
							if ((str.equals("none")) || (str.equals("normal"))) { // normal/none
																					// means
																					// remove
																					// all
																					// content
								...
								removeQuoteType = 0; // 0 is the quote type for
														// open quote, 1 for
														// closed one
							...
							if ((removeAll) || (removeQuoteType != -1)) {
								...
								return; // stop processing after removal clauses
										// such as none/normal
							}
						}

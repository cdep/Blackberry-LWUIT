...
/**
 * The HTMLElement class defines a single HTML element with its attributes and
 * children. Due to its hierarchial nature, this class can be used for a single
 * "leaf" Element, for more complex elements (with child elements), and up to
 * describing the entire document.
 * 
 * @author Ofir Leitner
 */
public class HTMLElement extends Element {
	...
	// HTML Tag ID codes:
	public static final int TAG_CSS_ILLEGAL_SELECTOR = -3; // If this is the tag
															// ID it means this
															// selector was
															// malformed and as
															// such will never
	...
	public static final int TAG_SPAN = 25; // While this is not parsed, it will
											// be kept in the DOM and as such
											// CSS will affect its class/ID
	...
	public static final int TAG_OPTGROUP = 53; // Currently still not supported
												// as LWUIT's ComboBox can't
												// display option groups, but
												// will display as a regular
												// ComboBox
	...
	// HTML 4 tags - the following tags are not part of the XHTML MP 1.0
	// standard:
	...
	public static final int TAG_NOSCRIPT = 76; // Since we don't support scripts
												// this tag will always be
												// evaluated
	public static final int TAG_NOFRAMES = 77; // Since we don't support frames
												// this tag will always be
												// evaluated
	...
	// Text nodes (not an actual tag - text segments are added by the parser as
	// the 'text' tag
	public static final int TAG_TEXT = 81;

	private static int LAST_TAG_INDEX = HTMLComponent.PROCESS_HTML_MP1_ONLY ? TAG_FIELDSET : TAG_TFOOT; // In
																										// any
																										// case
																										// we
																										// exclude
	...
	/**
	 * Defines the tag names, these are specified according to the tag constants
	 * numbering.
	 */
	...
	public static final int ATTR_DIR = 59; // Currently only supported in the
											// html tag
	...
	/**
	 * Defines the allowed attribute names, these are specified according to the
	 * ATTR_* constants numbering.
	 */
	...
	/**
	 * This array defines the allowed attributes for each tag, according to the
	 * XHTML-MP 1.0 spec
	 */
	private static final int[][] TAG_ATTRIBUTES = {
			// Structure Module
			{
					ATTR_BGCOLOR, // #rrggbb | colors // Deprecated but
									// supported
	...
	{}, // HEAD = 1;
	{
			// ATTR_VERSION, // = //WAPFORUM//DTD XHTML Mobile 1.0//EN // We
			// don't use the version attribute
			ATTR_DIR
	...
	{}, // TITLE = 3;
			...
			{
			// ATTR_CITE // URL // The cite attribute is not supported by any of
			// the major browsers.
			}, // Q = 23;
			{}, // SAMP = 24;
			{}, // SPAN = 25;
			...
			// Hypertext Module -
			{
					...
					// ATTR_CHARSET, // cdata // The charset attribute is not
					// supported in any of the major browsers.
					ATTR_HREF, // URL
					// ATTR_HREFLANG, // ((?i)[A-Z]{1,8}(-[A-Z]{1,8})*)? // The
					// hreflang attribute is not supported in any of the major
					// browsers.
					// ATTR_REL, // nmtokens // Not used by browsers
					// ATTR_REV, // nmtokens // Not used by browsers
					// ATTR_TABINDEX, // number
					// ATTR_TYPE, // cdata // Should specify the MIME type of
					// the document, but we don't use it anyway
					ATTR_NAME // Note: Name on the a tag (anchor) is not
								// supported on XHTML-MP 1.0, but we support it
			}, // A = 28;
			...
			}, // FORM = 35;
			{
					...
					ATTR_EMPTYOK, // true | false // This attribute was said to
									// be supported on XHTML-MP1 on various
									// sources, but verified as not
					ATTR_FORMAT, // cdata // Deprecated but still supported
					// ATTR_ISTYLE, // cdata // This attribute was said to be
					// supported on XHTML-MP1 on various sources, but verified
					// as not
					// ATTR_LOCALSRC, // cdata // This attribute was said to be
					// supported on XHTML-MP1 on various sources, but verified
					// as not
					...
					ATTR_SRC, // URL
					ATTR_TABINDEX, // number
					ATTR_TYPE, // text | password | checkbox | radio | submit |
								// reset | hidden
					ATTR_VALUE, // cdata
					ATTR_DISABLED,
					ATTR_READONLY
			...
			{
					// ATTR_SUMMARY, // cdata // The summary attribute makes no
					// visual difference in ordinary web browsers.
					ATTR_BORDER,
					ATTR_CELLSPACING,
					ATTR_CELLPADDING,
					ATTR_FRAME,
			...
			{
					// ATTR_ABBR, // cdata // The abbr attribute makes no visual
					// difference in ordinary web browsers.
					ATTR_ALIGN, // left | center | right
					// ATTR_AXIS, // cdata // The axis attribute is not
					// supported by any of the major browsers
					ATTR_COLSPAN, // number
					// ATTR_HEADERS, // IDREFS // The headers attribute makes no
					// visual difference in ordinary web browsers.
					ATTR_ROWSPAN, // number
					// ATTR_SCOPE, // row | col // // The scope attribute makes
					// no visual difference in ordinary web browsers.
			...
			// Image Module -
			{
					...
					// supported on XHTML-MP1 on various sources, but verified
					// as not
					// ATTR_LONGDESC, // URL //The longdesc attribute is not
					// supported by any of the major browsers.
					ATTR_SRC, // URL
					ATTR_VSPACE, // number
					ATTR_WIDTH, // number[%]
			...
			{
			/*
			 * ATTR_ARCHIVE, // &URLs ATTR_CLASSID, // URL ATTR_CODEBASE, // URL
			 * ATTR_CODETYPE, // cdata ATTR_DATA, // URL ATTR_DECLARE, //
			 * declare ATTR_HEIGHT, // number[%] ATTR_NAME, // cdata
			 * ATTR_STANDBY, // cdata ATTR_TABINDEX, // number ATTR_TYPE, //
			 * cdata ATTR_WIDTH, // number[%]
			 */
			}, // OBJECT = 47;
			{
			/*
			 * ATTR_NAME, // cdata ATTR_TYPE, // cdata ATTR_VALUE, // cdata
			...
			// Metainformation Module
			{
					...
					ATTR_NAME, // nmtoken // We do not make any use of this
								// attribute
			...
			// Link Module -
			{
					...
					ATTR_MEDIA, // cdata
					ATTR_REL, // nmtokens
					// ATTR_REV, // nmtokens // The rev attribute is not
					// supported in any of the major browsers.
					ATTR_TYPE, // cdata
			}, // LINK = 50;
	...
	// These are the possible types for an attribute. Types define what values
	// are acceptable to the attribute.
	static final int TYPE_NUMBER = 0;
	static final int TYPE_PIXELS_OR_PERCENTAGE = 1;
	static final int TYPE_COLOR = 2;
	static final int TYPE_ALIGN = 3; // note: there are different types of align
										// - TD allows only left,center,right
										// DIV allows also justify, CAPTION
										// allows top,bottom,left,right -
										// however we don't get to that
										// resolution
	...
	static final int TYPE_CSS_LENGTH = 24; // values with CSS suffixes (px/em/ex
											// etc.)
	static final int TYPE_CSS_LENGTH_OR_PERCENTAGE = 25; // values with CSS
															// suffixes
															// (px/em/ex etc.)
															// or percentages
	static final int TYPE_CSS_URL = 26;
	static final int TYPE_CSS_LENGTH_OR_PERCENTAGE_OR_MULTIPLIER = 27; // values
																		// with
																		// CSS
																		// suffixes
																		// (px/em/ex
	...
	/**
	 * Some types accept only a specific set of strings. For these this array
	 * defines the allowed strings. If the value is null it means that the type
	 * has another rule set (for example numbers only). This is checked against
	 * in the DOM building process.
	 */
	private static String[][] ALLOWED_STRINGS = {
			null, // TYPE_NUMBER = 0;
			null, // TYPE_PIXELS_OR_PERCENTAGE = 1;
			null, // TYPE_COLOR = 2;
			{ "left", "right", "top", "bottom", "center", "middle", "justify" }, // TYPE_ALIGN
																					// =
																					// 3;
			null, // TYPE_CHAR = 4;
			null, // TYPE_URL = 5;
			...
			{ "top", "bottom", "middle" }, // TYPE_VALIGN = 21; //
											// top/middle/bottom
	...
	/**
	 * The allowed values for the 'frame' attribute in the 'table' tag (This was
	 * not placed in the ALLOWED_STRINGS in order not to add more data types,
	 * since it is very specific for this attribute)
	 */
	...
	/**
	 * The allowed values for the 'rules' attribute in the 'table' tag (This was
	 * not placed in the ALLOWED_STRINGS in order not to add more data types,
	 * since it is very specific for this attribute)
	 */
	...
	public static final int COLOR_ORANGE = 0xffa500; // Orange is added in CSS
														// 2.1

	/**
	 * Defines the allowed color string that are acceptable as a value to color
	 * attributes in HTML-MP1
	 */
	...
	/**
	 * Defines additional allowed color string that are acceptable as a value to
	 * color attributes in HTML4
	 */
	...
	/**
	 * Assigns a color constant to each of the colors defined in
	 * MORE_COLOR_STRINGS
	 */
	...
	/**
	 * Converts a color string into an int value. This method supports color
	 * denoted in hex (with a leading #), named colors (from the standard HTML
	 * colors) and rgb(x,y,z)
	 * 
	 * @param colorStr
	 *            The string representing the color
	 * @param defaultColor
	 *            Default color if color parsing can't be done
	...
	static int getColor(String colorStr, int defaultColor) {
		...
		if (colorStr.length() == 3) { // shortened format rgb - translated to
										// rrggbb
			String newColStr = "";
			for (int i = 0; i < 3; i++) {
	...
	/**
	 * The tag ID. Upon construction of tag with a name, a lookup is performed
	 * to assign it an ID accordign to the TAG_NAMES array. THe ID is used to
	 * find what are the allowed attributes, and also prevents the need for
	 * further string parsing later on.
	 */
	...
	private void init(String tagName) {
		...
		while ((tagId == -1) && (i <= LAST_TAG_INDEX)) { // -1 to exclude
															// TAG_TEXT, which
															// is given only on
															// text element
															// creation
	...
	/**
	 * Sets the given component or Vector of components to be associated with
	 * this element. This is used internally to apply CSS styling.
	 * 
	 * @param obj
	 *            The component (or vector of components) representing this
	 *            HTMLElement
	 */
	...
	/**
	 * Adds the given component to be associated with this element. This is used
	 * internally to apply CSS styling.
	 * 
	 * @param cmp
	...
	/**
	 * Returns whether this element supports the common core attributes. These
	 * are attributes most HTML tags support, with a few exceptions that are
	 * checked here. Note that to be exact the common atributes are divided to 2
	 * groups: core attributes (class,id,title,style) and language attributes
	 * (xmlns,xml:lang) The tags checked here all don't support the core
	 * attributes but in fact may support the language attributes. Since the
	 * language attributes are not implemented anyway, this is not critical. For
	 * reference, tags that do not support the language attributes in XHTML-MP1
	 * are: param, hr, base, br
	 * 
	...
	public int setAttribute(String attribute, String value) {
		if (id == TAG_UNSUPPORTED) {
			return -1; // No error code for this case since tag not supported
						// error is already notified before
		}
	...
	/**
	 * Returns a list of supported attributes for this tag. Note that the list
	 * does not include the core attributes that are supported on almost all
	 * tags
	 * 
	...
	/**
	 * Verifies that the specified value conforms with the attribute's type
	 * restrictions. This basically checks the attribute type and according to
	...
	private boolean isValid(int type, String value) {
		...
		case TYPE_COLOR:
			...
			if (value.charAt(0) != '#') {
				...
				return verify(value.substring(1), HEX, null, 3, 6); // Color can
																	// also be
																	// #rgb
																	// which
																	// means
	 ...
	 * @param allowedMask
	 *            DIGITS or HEX or ABC or a combination of those
	 * @param allowedChars
	 *            Characters that are allowed even if they don't conform to the
	 *            mask
	...
	/**
	 * Verifies that the specified string conforms with the specified
	 * restrictions.
	 * 
	 * @param value
	 *            The string to be checked
	 ...
	 * 
	 * @param tagId
	 *            The tag ID to set, one of the TAG_* constants (Not to be
	 ...
	 * 
	 * @param tagId
	 *            The child's tag ID, one of the TAG_* constants (Not to be
	 *            confused with the id attribute)
	 ...
	 * 
	 * @param attrKey
	 *            The attribute key, which is typically an Integer object made
	 *            of its int attrId
	 * @return the attribute name of the requested attribute
	 */
	public String getAttributeName(Integer attrKey) { // This method is not
														// static since it needs
														// to be overriden in
														// CSSElement
	...
	/**
	 * A recursive method for creating a printout of a full tag with its entire
	 * hierarchy. This is used by the public method toString().
	 * 
	 * @param spacing
	 *            Increased by one in each recursion phase to provide with
	 *            indentation
	 ...
	 */
	Vector getUi() {
		if (comps == null) { // If no UI exists this may be a tag with children
								// that do have UI, such as TAG_A
			comps = new Vector();
			Vector children = getChildren();
			if (children != null) {
	...
	/**
	 * Causes a recalculation of the UI, if the UI of this element was deduced
	 ...
	 * {@inheritDoc}
	 */
	public void removeChildAt(int index) { // Overiding is done to clear the
											// associated components vector of
											// the child
	 ...
	 * 
	 * @param tagId
	 *            The tag ID to look for, one of the TAG_* constants (Not to be
	 *            confused with the id attribute)
	 * @param depth
	 *            The search depth (1 - children, 2 - grandchildren ....
	...
	boolean isFirstChild() {
		if ((HTMLComponent.PROCESS_HTML_MP1_ONLY) || (isTextElement())) { // :first-child
																			// is
																			// not
																			// supported
																			// in

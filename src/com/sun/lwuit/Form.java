...
public class Form extends Container {
	...
	/**
	 * Title area manipulation might break with future changes to LWUIT and
	 * might damage themeing/functionality of the LWUIT application in some
	 * platforms
	 * 
	...
	/**
	 * This listener is invoked when device orientation changes on devices that
	 ...
	 * hidden this method isn't called for form based events and is generally
	 ...
	 * @see com.sun.lwuit.painter.PainterChain#installGlassPane(Form,
	...
	/**
	 * Allows modifying the title attributes beyond style (e.g. setting
	 * icon/alignment etc.)
	 * 
	...
	/**
	 * Allows replacing the title with a different title component, thus
	 * allowing developers to create more elaborate title objects.
	 * 
	 * @param title
	 *            new title component
	 */
	...
	/**
	 * Allows replacing the title with a different title component, thus
	 * allowing developers to create more elaborate title objects. This version
	 * of the method allows special effects for title replacement such as
	 * transitions for title entering
	 * 
	 * @param title
	 *            new title component
	 * @param t
	...
	/**
	 * Add a game key listener to the given gamekey for a callback when the key
	 * is released
	 * 
	 * @param keyCode
	 *            code on which to send the event
	 ...
	 * animate its appearance. This method would not register a compnent
	...
	/**
	 * Identical to the none-internal version, the difference between the
	 * internal/none-internal is that it references a different vector that is
	 * unaffected by the user actions. That is why we can dynamically
	 * register/deregister without interfearing with user interaction.
	 */
			...
			if (c.animate()) {
				if (c instanceof Component) {
					...
					if (rect != null) {
						...
						// this probably can't happen but we got a really weird
						// partial stack trace to this
	...
	/**
	 * Dispatches a command via the standard form mechanism of firing a command
	 * event
	 * 
	 * @param cmd
	 ...
	 * useful for "back" navigation since it reverses the direction of the
	 * transition.
	 */
	...
	/**
	 * Allows subclasses to bind functionality that occurs when a specific form
	 * or dialog is "really" showing hence when the transition is totally
	 * complete (unlike onShow which is called on intent). The necessity for
	 * this is for special cases like media that might cause artifacts if played
	 * during a transition.
	 */
	...
	public void setFocused(Component focused) {
		...
		if (oldFocus != null) {
			...
			// if we need to revalidate no need to repaint the Component, it
			// will
	 ...
	 * 
	 * @param cmp
	 *            the Component to change the focus state
	 * @param gained
	 *            if true this Component needs to gain focus if false it needs
	 *            to lose focus
	...
	public void keyRepeated(int keyCode) {
		if (focused != null) {
			...
			// Fix for issue 433: the focus might be changed by the key repeated
			// method in a way that can turn it to null
	 ...
	 * 
	 * @param cyclicFocus
	 *            marks whether focus should cycle
	 */

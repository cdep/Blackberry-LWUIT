...
/**
 * A spinner allows us to select a numeric, date or time value using the arrow
 * keys in a similar way to a list or a combo box.
 * 
 * @author Shai Almog
 */
public class Spinner extends List {
	...
	/**
	 * The image appearing on the side of the spinner widget to indicate its
	 * "spinnability"
	 */
	...
	/**
	 * Creates a new time spinner instance, time is an integer represented in
	 * seconds since mindnight
	 * 
	 * @param min
	 *            lowest value allowed in seconds since midnight
	 * @param max
	 *            maximum value allowed in seconds since midnight
	 * @param currentValue
	 *            the starting value in seconds since midnight
	 * @param step
	 *            increments in the spinner (in seconds)
	 ...
	 * @param separatorChar
	 *            character to separate the entries during rendering
	 * @param format
	...
	/**
	 * Set the value of the spinner to a number or a date based on the spinner
	 * type
	 * 
	 * @param o
	 *            a number or a date
	...
	/**
	 * Returns the value of the spinner to a number or a date based on the
	 * spinner type
	 * 
			...
			try {
				...
				if (getRenderer() instanceof DateTimeRenderer) {
					...
					if (getModel() instanceof SpinnerNumberModel) {
						...
						// update the spinner positioning if we have two
						// characters
				...
				// easier to ignore exceptions than build "proper" error
				// handling
	...
	public boolean isRTL() {
		// Since spinner is numeric it shouldn't be affected by RTL and should
		// naturally be right aligned
		return false;
	}
	...
	/**
	 * Indicates the time after which the skip input area for entering spinner
	 * values manually will disappear
	 * 

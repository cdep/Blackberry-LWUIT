 ...
 * {@link com.sun.lwuit.list.ListCellRenderer} and are extracted via the
 * {@link com.sun.lwuit.list.ListModel}.
 * <p>
 ...
 * MVC. Specifically a list component is relatively simple, it invokes the model
 * in order to extract the displayed/selected information and shows it to the
 * user by invoking the cell renderer.
 * <p>
 * The list class itself is completely decoupled from everything, thus it allows
 * us to extract its content from any source (e.g. network, storage etc.) and
 * display the information in any form (e.g. checkboxed elemenents, icons etc.).
 * 
...
public class List extends Component {
	...
	/**
	 * Indicates the list selection will only reach the edge when there are no
	...
	/**
	 * Indicates the list selection is fixed into place at the bottom of the
	 * list or at the right of the list
	 */
	public static final int FIXED_TRAIL = 11;
	/**
	 * Indicates the list selection is fixed into place at the center of the
	 * list
	 */
	...
	/**
	 * Indicates whether the list should not paint the focus component if the
	 * list itself has no focus.
	 * 
	 ...
	 * list itself has no focus.
	 * 
	 * @param aDefaultIgnoreFocusComponentWhenUnfocused
	 *            the defaultIgnoreFocusComponentWhenUnfocused to set
	 */
	 ...
	 * Default value for the fire on click behavior
	 * 
	 * @param aDefaultFireOnClick
	 *            the defaultFireOnClick to set
	 */
	...
	/**
	 * Indicates the number of elements the list should check to determine the
	 * element sizes. This is ignored when a rendering prototype is present.
	 */
	private int listSizeCalculationSampleCount = 5;

	/**
	 * Minimum number of elements shown in a list, this member is used to
	 * calculate the list preferred size. If the number of elements in the model
	 * is smaller than this then this value is used in the calculations.
	 */
	...
	/**
	 * Indicates that the background of a cell renderer might mutate between one
	 * entry and the next, it is recommended that this flag remains false for
	 * performance reasons.
	 */
	 ...
	 * 
	 * @param items
	 *            set of items placed into the list model
	 */
	...
	void initComponentImpl() {
		...
		if (index >= 0) {
			...
			// scroll to the element, this can happen on some devices where
			// laidOut is invoked before
	...
	void deinitializeImpl() {
		...
		// cleanup to allow garbage collection even if the user keeps the model
		// in
	...
	public int getSideGap() {
		// isScrollableY() in the base method is very expensive since it
		// triggers getScrollDimension before the layout is complete!
	 ...
	 * implementation will scroll the list to the selection if the selection is
	 ...
	 *            indicates whether scrolling to selection should occur if the
	 *            selection is outside of view
	 */
	...
	public void setShouldCalcPreferredSize(boolean shouldCalcPreferredSize) {
		...
		// we should try passing the should calcPreferredSize to the renderer so
		// it can revalidate too
	 ...
	 * 
	 * @param model
	 *            the new model underlying the list
	 */
	public void setModel(ListModel model) {
		if (this.model != null) {
			...
			// when replacing a model on a scrolled list reset the scrolling if
			// necessary
	 ...
	 * Indicate whether pressing the number keys should trigger an action
	 * 
	 * @param numericKeyActions
	 *            true to trigger an action on number keys
	 */
	 ...
	 * user "clicks" a command from the list its action performed method is
	 * invoked.
	 * 
	 ...
	 * 
	 * @param commandList
	 *            true for the list to be treated as a command list
	 */
	 ...
	 * list itself has no focus.
	 * 
	 * @param ignoreFocusComponentWhenUnfocused
	 *            true to ignore the focus component false otherwise
	 */
	 ...
	 * performance reasons.
	 * 
	 * @param mutableRendererBackgrounds
	 ...
	 * element sizes. This is ignored when a rendering prototype is present.
	 * 
	 * @param listSizeCalculationSampleCount
	 *            the listSizeCalculationSampleCount to set
	 */
	 ...
	 * @param renderer
	 *            cell renderer instance
	 * @deprecated use setRenderer instead, this method was deprecated to
	 *             confirm better to JavaBean convention of having the
	 ...
	 * 
	 * @param rect
	 *            the rectangle area to scroll to
	 */
	...
	public void keyReleased(int keyCode) {
		// other events are in keyReleased to prevent the next event from
		// reaching the next form
	...
	/**
	 * Calculates the desired bounds for the component and returns them within
	 * the given rectangle.
	 */
	private void calculateComponentPosition(int index, int defaultWidth, Rectangle rect, Dimension rendererSize, Dimension selectedSize, boolean beforeSelected) {
		...
		// the algorithm illustrated here is very simple despite the "mess" of
		// code...
		// The idea is that if we have a "fixed" element we just add up the
		// amount of pixels
		// to get it into its place in the screen (nothing for top obviously).
		// In order to cause the list to be cyclic we just subtract the list
		// size
	...
	/**
	 * Allows us to recalculate the bounds of a coordinate to make it "loop"
	 * back into view
	 * 
	 * @param offset
	 *            either x or y coordinate
	 * @param totalSize
	 *            the total width or height of the list with all the elements
	 *            (including scroll)
	 * @param viewSize
	 *            the size visible to the user
	 * @param componentSize
	...
	public void paint(Graphics g) {
		...
		if (fixedSelection < FIXED_NONE_BOUNDRY) {
			...
			if (isRTL()) {
				// In RTL the start of the list is not in the left side of the
				// viewport, but rather the right side
				startX += getWidth();
			}
		...
		if (mutableRendererBackgrounds) {
			...
			if (selectedIndex > -1 && selectedIndex < numOfcomponents) {
				// this is essential otherwise we constantly ticker based on the
	 ...
	 * 
	 * @param inputOnFocus
	 *            true is a list can start handling input implicitly upon
	 *            gaining focus
	 */
	 ...
	 * List and is recommended for performance reasons. You should invoke it
	 * with an object representing a theoretical value in the list which will be
	 * used to calculate the size required for each element in the list.
	 * <p>
	 * This allows list size calculations to work across look and feels and
	 * allows developers to predetermin size for list elements.
	 * <p>
	 * e.g. For a list of Strings which you would like to always be 5 characters
	 * wide you can use a prototype "XXXXX" which would use the preferred size
	 * of the XXXXX String to determine the size of the list element. E.g. for a
	 * list of dates you can use new Date(30, 12, 00) etc..
	 * 
	 * @param renderingPrototype
	 *            a value that can be passed to the renderer to indicate the
	 *            preferred size of a list component.
	 */
	...
	Dimension getElementSize(boolean selected, boolean addMargin) {
		if (selected) {
			if (selectedElemSize == null) {
				// don't keep element size if there are no elements and no
				// prototype...
				if (renderingPrototype == null) {
					if (model.getSize() == 0) {
						// put a sensible value as default when there are no
						// elements or rendering prototype
						if (addMargin) {
	...
	/**
	 * Calculates the size of an element based on a forumla or on rendering
	 * prototype
	 */
	...
	public void pointerPressed(int x, int y) {
		...
		// prevent a hover event from activating the drag in case of a click
		// screen,
			...
			if (orientation != HORIZONTAL) {
				...
				if (isRTL()) {
					if (x > pos.getX() + selectedSize.getWidth()) {
						...
						// should have been -1-delta, but works better like
						// this.
	...
	/**
	 * This method determines if the List fires the action event when the
	 * pointer was clicked on one of the items, or only if the item was the
	 * selected item By default the value is true, this setting is only relevant
	 * for none fixed Lists
	 * 
	 * @param fireOnClick
	 */
	...
	/**
	 * Allows adding an element to a list if the underlying model supports this,
	 * notice that it is an optional operation and if the model does not support
	 * it (default list model does) then this operation may failed.
	 * 
	 * @param item
	 *            the item to be added to a list model
	 */
	 ...
	 * 
	 * @return one of: FIXED_NONE, FIXED_TRAIL, FIXED_LEAD, FIXED_CENTER,
	 *         FIXED_NONE_CYCLIC
	 */
	...
	public boolean animate() {
		// parent is performing the animation we shouldn't do anything in this
		// case
	...
	protected boolean isTactileTouch(int x, int y) {
		// provide touch feedback only when pressing an entry in the list and
	 ...
	 *            if true the List scrolls to the selected element when It's
	 *            been initalized.
	 */
	...
	/**
	 * Sets the TextArea hint icon, the hint is displayed on the TextArea When

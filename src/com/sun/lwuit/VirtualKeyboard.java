...
public class VirtualKeyboard extends Dialog implements VirtualKeyboardInterface {
	...
	static class InputField extends TextField {
		...
		public void setCursorPosition(int i) {
			...
			// this can happen since this method is invoked from a constructor
			// of the base class...
	...
	/**
	 * Invoked internally by the implementation to indicate the text field that
	 * will be edited by the virtual keyboard
	 * 
	 * @param field
	 ...
	 * 
	 * @param mode
	 *            the String that represents the mode(QWERTY_MODE, SYMBOLS_MODE,
	 *            ...)
	 */
			...
			for (int j = 0; j < currentKeyboardChars[i].length; j++) {
				...
				if (txt.startsWith("$") && txt.endsWith("$") && txt.length() > 1) {
					...
					if (prefW != 0) {
						...
						// if we can't determind the size at this stage, wait
						// until
						// the loops ends and give the remains size to the
						// special
						// button
	 ...
	 * @param inputChars
	 *            2 dimensional String array that contains buttons String and
	 *            special buttons (a special button is identified with $...$
	 *            marks e.g: "$Space$")
	 */
	 ...
	 *            a string that represents the identifier of the mode
	 * @param inputChars
	 *            2 dimentional String array that contains buttons String and
	 *            special buttons (a special button is identified with $...$
	 *            marks e.g: "$Space$")
	 */
	 ...
	 * 
	 * @param key
	 *            the string identifier from within the relevant input mode
	 * @param cmd
	 *            the Command to invoke when this button is invoked.
	 */
	 ...
	 *            the Command to invoke when this button is invoked.
	 * @param space
	 *            how much space in percentage from the overall row the special
	...
	/**
	 * Indicates whether the VKB should commit changes to the text field when
	 * the VKB is closed not via the OK button. This might be useful for some
	 * situations such as searches
	 * 
	 * @param tf
	 *            the text field to mark as commit on dispose
	 * @param b
	 *            the value of commit on dispose, true to always commit changes
	 */
	...
	/**
	 * This method is used to bind a specific instance of a virtual keyboard to
	 * a specific TextField. For example if a specific TextField requires only
	 * numeric input consider using this method as follows:
	 * 
	 * TextField tf = new TextField(); tf.setConstraint(TextField.NUMERIC);
	 * tf.setInputModeOrder(new String[]{"123"}); VirtualKeyboard vkb = new
	 * VirtualKeyboard(); vkb.setInputModeOrder(new
	 * String[]{VirtualKeyboard.NUMBERS_MODE});
	 * VirtualKeyboard.bindVirtualKeyboard(tf, vkb);
	 * 
	 * @param t
	 *            the TextField to bind a VirualKeyboard to.
	 * @param vkb
	 *            the binded VirualKeyboard.
	 */
	 ...
	 * 
	 * @param t
	 *            a TextField.that might have an associated VirtualKeyboard
	 *            instance
	 ...
	 * @param tf
	 *            the text field
	 * @return true if the text field should save the data despite the fact that
	 *         it was disposed in an irregular way
	 */
	...
	/**
	 * Sets the tint color for the virtual keyboard when shown on top of this
	 * text field see the form tint methods for more information
	 * 
	 * @param tf
	...
	/**
	 * Indicates whether tooltips should be shown when the keys in the VKB are
	 * pressed
	 * 
	 ...
	 * The transition out for the VKB
	 * 
	 * @param aTransitionOut
	 *            the transitionOut to set
	 */
	 ...
	 * com.sun.lwuit.VirtualKeyboard type This class is used as the default
	 * virtual keyboard class if the current platform VirtualKeyboard is
	 * com.sun.lwuit.VirtualKeyboard. Platform VirtualKeyboard is defined here:
	 * Display.getIntance().setDefaultVirtualKeyboard(VirtualKeyboardInterface
	 * vkb)
	 * 
	 * @param vkbClazz
	 *            this class must extend VirtualKeyboard.
	 */

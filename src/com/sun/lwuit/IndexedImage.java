...
/**
 * An indexed image is an image "compressed" in memory to occupy as little
 * memory as possible in this sense it is slower to draw and only a single
 * indexed image can be drawn at any given time. However, this allows images
 * with low color counts to use as little as one byte per pixel which can save
 * up to 4 times of the memory overhead.
 * 
 * @deprecated This class should no longer be referenced directly. Use
 *             Image.createIndexed instead
 * @author Shai Almog
 */
public class IndexedImage extends Image {
	...
	/**
	 * Converts an image to a package image after which the original image can
	 * be GC'd
	 */
	...
	public static IndexedImage pack(int[] rgb, int width, int height) {
		...
		// using a Vector is slower for a small scale device and this is mission
		// critical code
	 ...
	 * 
	 * @param sourceImage
	 *            the image which would be converted to a packed image if
	 *            possible
	 * @return the source image if packing failed or a newly packed image if it
	 *         succeeded
	 */
	...
	/**
	 * This method allows us to store a package image into a persistent stream

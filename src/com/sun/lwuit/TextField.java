 ...
 * the external native text box. The main drawback in this approach is that
 * editing can't support features such as T9 and might not have the same
 * keymapping or behavior of the native text input.
 * <p>
 * Notice that due to limitations of text area and text field input modes in
 * text area aren't properly supported since they won't work properly across
 * devices. To limit input modes please use the setInputModeOrder method. All
 * constants declated in TextArea are ignored with the exception of PASSWORD.
 * 
 * @author Shai Almog
 */
public class TextField extends TextArea {
	...
	/**
	 * Indicates that native text input should be used in text field when in
	 * place editing is supported by the platform
	 * 
	...
	/**
	 * Indicates whether the left/right keys will trigger editing, this is true
	 * by default. Left and right key edit trigger might be disabled for cases
	 * such as text field positioned horizontally one next to the other.
	 */
	...
	class CommandHandler extends Command {
		...
		public void actionPerformed(ActionEvent ev) {
			...
			case 1:
				...
				// called triggering a long click, this code disables the long
				// click
	 ...
	 * @param columns
	 *            - the number of columns
	 * @return a text field if native in place editing is unsupported and a text
	 *         area if it is
	 */
	 ...
	 * This is used in a numeric keypad to allow the user to repeatedly press a
	 * number to change values.
	 */
	 ...
	 *            is a String containing the characters to toggle between for
	 *            the given keycode
	 * @param firstUpcase
	 *            indicates if this input mode in an input mode used for the
	 ...
	 * @return the char mapping to this key or 0 if no appropriate char was
	 *         found (navigation, input mode change etc...).
	 */
	protected char getCharPerKeyCode(int pressCount, int keyCode, boolean longClick) {
		initInputModes();
		String input = inputMode;

		// if this is a first letter uppercase input mode then we need to pick
		// either
	...
	/**
	 * Blocks the text area from opening the native text box editing on
	 * touchscreen click
	 */
	void onClick() {
	 ...
	 * contain a clear key and even in those that contain it this might be an
	 * issue
	 * 
	 * @param keyCode
	...
	/**
	 * Returns true if the given input mode should commit immediately or wait
	 ...
	 * the focal point for all input. It can be overriden to prevent a
	 * particular char from insertion or provide a different behavior for char
	 * insertion. It is the responsibility of this method to shift the cursor
	 * and invoke setText...
	 * <p>
	 * This method accepts a string for the more elaborate cases such as
	 * multi-char input and paste.
	 * 
	 * @param c
	...
	/**
	 * Returns true if the cursor should cycle to the beginning of the text when
	 * the user navigates beyond the edge of the text and visa versa.
	 * 
	...
	/**
	 * Returns true if this keycode is the one mapping to the symbol dialog
	 * popup
	 * 
	 * @param keyCode
	...
	protected void deinitialize() {
		...
		// if the text field is removed without restoring the commands we need
		// to restore them
		if (handlesInput()) {
			if (useSoftkeys) {
	 ...
	 * @param t9
	 *            the t9 command
	 * @return clear command already installed in the form if applicable, none
	 *         if no clear command was installed before or not applicable.
	 */
	 ...
	 *            the t9 command
	 * @param originalClear
	 *            the command originally assigned as the clear command (or null
	 *            if no command was assigned before)
	 */
	...
	void focusLostInternal() {
		...
		Form f = getComponentForm();
		if (f != null) {
			// prevent the VKB folding in case we are moving from one text
	 ...
	 * editing, by default fire or any numeric key should trigger editing
	 * implicitly. This method is only called when handles input is false.
	 * 
	 * @param keyCode
	...
	public void pointerReleased(int x, int y) {
		...
		// unlike text area the text field supports shifting the cursor with the
		// touch screen
	...
	/**
	 * The default value for the qwerty flag so it doesn't need setting for
	 * every text field individually.
	 * 
	 * @param v
	 ...
	 * such as text field positioned horizontally one next to the other.
	 * 
	 * @param leftAndRightEditingTrigger
	 ...
	 * such as text field positioned horizontally one next to the other.
	 * 
	 * @return leftAndRightEditingTrigger Indicates whether the left/right keys
	 *         will trigger editing
	 */

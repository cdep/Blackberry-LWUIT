...
public class Component implements Animation, StyleListener {
	...
	/**
	 * This property is useful for blocking in z-order touch events, sometimes
	 * we might want to grab touch events in a specific component without making
	 * it focusable.
	 */
	private boolean grabsPointerEvents;

	/**
	 * Indicates whether tensile drag (dragging beyond the boundry of the
	 * component and snapping back) is enabled for this component.
	 */
	private boolean tensileDragEnabled;

	/**
	 * Indicates whether tensile highlight (drawing a highlight effect when
	 * reaching the edge) is enabled for this component.
	 */
	...
	/**
	 * gets the Component dirty region, this method is for internal use only and
	 * SHOULD NOT be invoked by user code. Use repaint(int,int,int,int)
	 * 
	 * @return returns the region that needs repainting or null for the whole
	 *         component
	 */
	...
	/**
	 * sets the Component dirty region, this method is for internal use only and
	 * SHOULD NOT be invoked by user code. Use repaint(int,int,int,int)
	 * 
	 * @param dirty
	 *            the region that needs repainting or null for the whole
	 *            component
	 */
	...
	/**
	 * Allows determining the text for the select command used in the 3rd
	 * softbutton mode.
	 * 
	 * @param selectText
	...
	/**
	 * Allows us to indicate the label associated with this component thus
	 * providing visual feedback related for this component e.g. starting the
	 * ticker when the component receives focus.
	 * 
	...
	protected void paintScrollbarX(Graphics g) {
		...
		if (getScrollX() + getWidth() == scrollW) {
			// normalize the offset to avoid rounding errors to the bottom of
			// the screen
			offset = 1 - block;
		} else {
	...
	/**
	 * This method is used internally by the look and feel to implement the
	 * fading scrollbar behavior.
	 * 
	 ...
	 * components that include an internal selection behavior and are not
	 * containers (currently only List) this method allows returning the
	 * position of the selection itself which is useful for things such as the
	 * popup dialog and similar UI's that need to reference the position of the
	...
	/**
	 * This method can be overriden by a component to draw on top of itself or
	 * its children after the component or the children finished drawing in a
	 * similar way to the glass pane but more refined per component
	 * 
	 * @param g
	...
	/**
	 * Returns the Component Form or null if this Component is not added yet to
	 * a form
	 * 
	 ...
	 * 
	 * @param x
	 *            boundry of the region to repaint
	 * @param y
	 *            boundry of the region to repaint
	 * @param w
	 *            boundry of the region to repaint
	 ...
	 * the display. This is true for PC mouse pointer as well as some devices
	 * such as the BB storm.
	 * 
	 * @param x
	 ...
	 * will be overlayed on top of the form during a drag and drop operation
	 * 
	 * @return an image
	 */
	...
	/**
	 * This method allows a component to indicate if it is a drop target for the
	 * given component at the given x/y location (in component coordiate space).
	 * This method can also update the drop tagets appearance to indicate the
	 * drop location.
	 * 
	 * @param dragged
	 ...
	 * indication visuaization in draggingOver this visualization should be
	 * cleared..
	 * 
	 * @param dragged
	 *            the component being dragged
	...
	/**
	 * Performs a drop operation of the component at the given X/Y location in
	 * coordinate space, this method should be overriden by subclasses to
	 * perform all of the logic related to moving a component, by default this
	 * method does nothing and so dragging a component and dropping it has no
	 * effect
	 * 
	 * @param dragged
	 ...
	 * @return a component drop target or null if no drop target is available at
	 *         that coordinate
	 */
	...
	public void pointerDragged(int x, int y) {
		if (dragAndDropInitialized) {
			...
			// we repaint twice to create an intersection of the old and new
			// position
	...
	private void initScrollMotion() {
		...
		// as smaller than the screen and grew (e.g. by adding components to the
		// container
		// once it is visible).
		Form f = getComponentForm();
		if (f != null) {
	...
	/**
	 * This method allows a developer to define only a specific portion of a
	 * component as draggable by default it returns true if the component is
	 * defined as "draggable"
	 * 
	 * @param x
	 *            the x coordinate relative to the component
	 * @param y
	 *            the y coordinate relative to the component
	 * @return true if a press in this point might indicate the desire to begin
	 *         a drag operation
	 */
	...
	void startTensile(int offset, int dest) {
		...
		// just to be sure, there are some cases where this doesn't work as
		// expected
		Form p = getComponentForm();
		if (p != null) {
			p.registerAnimatedInternal(this);
	...
	/**
	 * Returns the Component Style for the pressed state allowing us to
	 * manipulate the look of the component when it is pressed
	 * 
	 * @return the component Style object
	 */
	...
	/**
	 * Returns the Component Style for the unselected mode allowing us to
	 * manipulate the look of the component
	 * 
	 * @return the component Style object
	 */
	...
	/**
	 * Returns the Component Style for the selected state allowing us to
	 * manipulate the look of the component when it owns focus
	 * 
	 * @return the component Style object
	 */
	...
	/**
	 * Returns the Component Style for the disabled state allowing us to
	 * manipulate the look of the component when its disabled
	 * 
	 * @return the component Style object
	 */
	...
	/**
	 * Changes the Component selected Style by replacing the Component Style
	...
	/**
	 * Changes the Component disalbed Style by replacing the Component Style
	 * with the given Style
	 * 
	 * @param style
	 *            the component Style object
	...
	/**
	 * Allows subclasses to create their own custom style types and install the
	 * background painter into them
	 * 
	 * @param s
	 *            the custom style
	 */
	...
	/**
	 * Indicates whether we are in the middle of a drag operation, this method
	 * allows developers overriding the pointer released events to know when
	 * this is a drag operaton.
	 * 
	...
	/**
	 * This method should be implemented correctly by subclasses to make snap to
	 * grid functionality work as expected. Returns the ideal grid Y position
	 * closest to the current Y position.
	 * 
	...
	/**
	 * This method should be implemented correctly by subclasses to make snap to
	 * grid functionality work as expected. Returns the ideal grid X position
	...
	public boolean animate() {
		...
		if (draggedMotion != null) {
			...
			// this can't be a part of the parent if since we need the last
			// value to arrive
	...
	/**
	 * Removes the internal animation. This method may be overriden by
	 * sublcasses to block automatic removal
	 */
	...
	/**
	 * A component name allows us to easily identify the component within a
	 * dynamic UI.
	 * 
	...
	/**
	 * Allows components to create a style of their own, this method binds the
	 * listener to the style and installs a bg painter
	 * 
	 * @param s
	 *            style to initialize
	 */
	...
	/**
	 * Elaborate components might not provide tactile feedback for all their
	 * areas (e.g. Lists) this method defaults to returning the value of
	 * isTactileTouch
	 * 
	 * @param x
	 *            the x position
	 ...
	 * failed and null if successful. Notice that some builtin properties such
	 * as "$designMode" might be sent to components to indicate application
	 * state.
	 * 
	 * @param name
	 *            the name of the property
	 ...
	 * transitions that might be quite expensive on the device. A lock should be
	 * released using paintLockRelease(), it is implicitly released when a
	 * component is deinitialized although a component doesn't need to be
	 * initialized to be locked!<br>
	 ...
	 * 
	 * @param hardLock
	 *            indicates whether the lock uses a hard or a soft reference to
	 *            the image
	...
	/**
	 * Indicates whether scrolling this component should jump to a specific
	 * location in a grid
	 * 
	...
	/**
	 * Makes the component effectively opaque by blending the backgrounds into
	 * an image in memory so the layer of underlying components is only drawn
	 * once when this component is repainted. This does have a significant
	 * memory overhead.
	 * 
	...
	/**
	 * Sets the hint text and Icon, the hint text and icon are displayed on the
	 * component when it is empty
	 * 
	 * @param hint
	 *            the hint text to display
	 * @param icon
	 *            the hint icon to display
	 */
	 ...
	 * essentially takes a component whose preferred size has changed and
	 * creates a "growing" effect that lasts for the duration. Notice that some
	 * components (such as text areas) don't report proper preferred size untill
	 * they are laid out once. Hence the first time around a text area (or
	 * container containing a text area) will not produce the expected effect.
	 * This can be solved by invoking revalidate before the call to this method
	...
	public void growShrink(int duration) {
		...
		// we are using bgpainter just to save the cost of creating another
		// class
	...
	/**
	 * Enable the tensile drag to work even when a component doesn't have a
	 * scroll showable (scrollable flag still needs to be set to true)
	 * 
	...
	/**
	 * Indicates whether this component can receive dropped components into it,
	 * notice that when dropping on a component or container the parents will be
	 * checked recursively to find a valid drop target
	 * 
	...
	/**
	 * Indicates that this component and all its children should be hidden when
	 * the device is switched to portrait mode
	 * 

 ...
 * container with a border layout, use one of these five constants.
 * 
 * @author Nir Shabi
 */
public class BorderLayout extends Layout {
	...
	/**
	 * Defines the behavior of the component placed in the center position of
	 * the layout, places the component in the center of the space availble to
	...
	private void positionLeftRight(Component c, int targetWidth, int bottom, int top) {
		...
		c.setHeight(bottom - top - c.getStyle().getMargin(false, Component.TOP) - c.getStyle().getMargin(false, Component.BOTTOM)); // verify
																																	// I
																																	// want
																																	// to
																																	// use
	...
	private void positionTopBottom(Component target, Component c, int right, int left, int targetHeight) {
		...
		c.setHeight(Math.min(targetHeight, c.getPreferredH())); // verify I want
																// to use tge
																// prefered size
		c.setX(left + c.getStyle().getMargin(target.isRTL(), Component.LEFT));
	}
	 ...
	 *            should always be used when adding a component to the layout).
	 *            One of NORTH/SOUTH/EAST/WEST/CENTER.
	 * @param landscapePosition

...
public class LayoutStyle {
	...
	private int getCBRBPadding(Component c, int position) {
		/*
		 * if (c.getUIClassID() == "CheckBoxUI" || c.getUIClassID() ==
		 * "RadioButtonUI") { Border border = c.getBorder(); if (border
		 * instanceof UIResource) { return getInset(c, position); } }
		 */
		return 0;
	}
	...
	/*
	 * private boolean isLeftAligned(Label button, int position) { if (position
	 * == GroupLayout.WEST) { boolean ltr =
	 * button.getComponentOrientation().isLeftToRight(); int hAlign =
	 * button.getHorizontalAlignment(); return ((ltr && (hAlign ==
	 * SwingConstants.LEFT || hAlign == SwingConstants.LEADING)) || (!ltr &&
	 * (hAlign == SwingConstants.TRAILING))); } return false; }
	 * 
	 * private boolean isRightAligned(AbstractButton button, int position) { if
	 * (position == SwingConstants.EAST) { boolean ltr =
	 * button.getComponentOrientation().isLeftToRight(); int hAlign =
	 * button.getHorizontalAlignment(); return ((ltr && (hAlign ==
	 * SwingConstants.RIGHT || hAlign == SwingConstants.TRAILING)) || (!ltr &&
	 * (hAlign == SwingConstants.LEADING))); } return false; }
	 */

	/*
	 * private Image getIcon(Label button) { Icon icon = button.getIcon(); if
	 * (icon != null) { return icon; } String key = null; if (button instanceof
	 * JCheckBox) { key = "CheckBox.icon"; } else if (button instanceof
	 * JRadioButton) { key = "RadioButton.icon"; } if (key != null) { Object
	 * oIcon = UIManager.get(key); if (oIcon instanceof Icon) { return
	 * (Icon)oIcon; } } return null; }
	 */
	...
	int getButtonChildIndent(Component c, int position) {
		 ...
		 * AbstractButton button = (AbstractButton)c; Insets insets =
		 * c.getInsets(); Icon icon = getIcon(button); int gap =
		 * button.getIconTextGap(); if (isLeftAligned(button, position)) {
		 * return insets.left + icon.getIconWidth() + gap; } else if
		 * (isRightAligned(button, position)) { return insets.right +
		 * icon.getIconWidth() + gap; } }
		 */
		return 0;
	}
}

...
public class GroupLayout extends Layout {
	 ...
	 *            the component
	 * @param honorsVisibility
	 *            whether <code>component</code>'s visibility should be
	 ...
	 * invoke <code>removeComponent</code> on the parent <code>Container</code>.
	 * 
	 * @param component
	 ...
	 * checkParent(parent); prepare(MIN_SIZE); return
	 * adjustSize(horizontalGroup.getMinimumSize(HORIZONTAL),
	 ...
	 * checkParent(parent); prepare(MAX_SIZE); return
	 * adjustSize(horizontalGroup.getMaximumSize(HORIZONTAL),
	 ...
	 */
	/*
	 * public float getLayoutAlignmentX(Container parent) { checkParent(parent);
	 * return .5f; }*
	 * 
	 ...
	 * along the origin, 1 is aligned the furthest away from the origin, 0.5 is
	 * centered, etc.
	 * 
	 * @param parent Container hosting this LayoutManager
	 * 
	 * @throws IllegalArgumentException if <code>parent</code> is not the same
	 * <code>Container</code> that this was created with
	 * 
	 * @return alignment
	 */
	/*
	 * public float getLayoutAlignmentY(Container parent) { checkParent(parent);
	 * return .5f; }
	 */
	 ...
	 */
	/*
	 * public void invalidateLayout(Container parent) { checkParent(parent); //
	 * invalidateLayout is called from Container.invalidate, which // does NOT
	 * grab the treelock. All other methods do. To make sure // there aren't any
	 * possible threading problems we grab the tree lock // here.
	 * //synchronized(parent.getTreeLock()) { isValid = false; //} }
	 */
	...
	private void prepare(int sizeType) {
		...
		if (!isValid) {
			...
			for (Enumeration cis = componentInfos.elements(); cis.hasMoreElements();) {
	...
	private class BaselineGroup extends ParallelGroup {
		...
		int getBaseline() {
			if (springs.size() > 1) {
				...
				this.getPreferredSize(VERTICAL);
				return prefAscent;
			} else if (springs.size() == 1) {
	...
	private final class ComponentSpring extends Spring {
		...
		int getBaseline() {
			if (baseline == -1) {
				...
				int height = this.getPreferredSize(VERTICAL);
				if (width > 0 && height > 0) {
	...
	private class AutopaddingSpring extends Spring {
		...
		int calculateMaximumSize(int axis) {
			if (max >= 0) {
				return Math.max(this.getPreferredSize(axis), max);
			}
			return size;
		}

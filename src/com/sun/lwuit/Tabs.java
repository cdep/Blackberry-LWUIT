 ...
 * count minus 1.
 * <p>
 * The <code>Tabs</code> uses a <code>SingleSelectionModel</code> to represent
 * the set of tab indices and the currently selected index. If the tab count is
 * greater than 0, then there will always be a selected index, which by default
 * will be initialized to the first tab. If the tab count is 0, then the
...
public class Tabs extends Container {
	 ...
	 * <code>icon</code>. Cover method for <code>insertTab</code>.
	 * 
	 * @param title
	 *            the title to be displayed in this tab
	 * @param component
	...
	/**
	 * Adds a <code>component</code> represented by a <code>button</code>. Cover
	 * method for <code>insertTab</code>. The Button styling will be associated
	 * with "Tab" UIID.
	 * 
	 * @param tab
	...
	/**
	 * Inserts a <code>component</code>, at <code>index</code>, represented by a
	 * <code>title</code> and/or <code>icon</code>, either of which may be
	 * <code>null</code>. Uses java.util.Vector internally, see
	 * <code>insertElementAt</code> for details of insertion conventions.
	 * 
	 * @param title
	 *            the title to be displayed in this tab
	 * @param icon
	...
	/**
	 * Inserts a <code>component</code>, at <code>index</code>, represented by a
	 * <code>button</code> Uses java.util.Vector internally, see
	 * <code>insertElementAt</code> for details of insertion conventions. The
	 * Button styling will be associated with "Tab" UIID.
	 * 
	 * @param tab
	 *            represents the tab on top
	 * @param component
	...
	/**
	 * Indicates that a tab should change when the focus changes without the
	 * user physically pressing a button
	 * 
	...
	/**
	 * Indicates that the tabs container should have its style changed to the
	 * selected style when one of the tabs has focus this allows incorporating
	 * it into the theme of the application
	 * 
	 ...
	 * it into the theme of the application
	 * 
	 * @param changeTabContainerStyleOnFocus
						...
						while (testCmp != null && testCmp != contentPane) {
							...
							if (testCmp.isScrollable()) {
								if (testCmp.isScrollableX()) {
									// we need to block swipe since the user is
									// trying to scroll a component
									lastX = -1;
									initialX = -1;
									blockSwipe = true;
									return;
								...
								// scrollable Y component, we want to make side
								// scrolling
								// slightly harder so it doesn't bother the
								// vertical swipe
								riskySwipe = true;
								break;
							}
			...
			case DRAG: {
				...
				if (!dragStarted) {
					if (riskySwipe) {
						...
						// start drag not imediately, giving components some
						// sort
						// of weight.

...
/**
 * An optionally multi-line editable region that can display text and allow a
 * user to edit it. Depending on the platform editing might occur in a new
 * screen. Notice that when creating a text area with one row it will act as a
 * text field and never grow beyond that, however when assigning a greater
 * number of rows the text area becomes multi-line with a minimum number of
 * visible rows, the text area will grow based on its content.
 * 
...
public class TextArea extends Component {
	...
	/**
	 * Indicates the defaut vertical alignment for a text field, only applies to
	 ...
	 * than string width. However, with some fonts and especially some languages
	 * (such as Arabic, Korean etc.) the width of the string drawn might not
	 * equal the summary of the width of the chars. Hence for portability to
	 * those languages/fonts this flag must be set to true.
	 */
	...
	/**
	 * Input constraint which should be one of ANY, NUMERIC, PHONENUMBER, URL or
	 * EMAIL
	 */
	...
	private int maxSize = defaultMaxSize; // maximum size (number of characters)
											// that can be stored in this
											// TextField.
	...
	// problematic maxSize = 20; //maximum size (number of characters) that can
	// be stored in this TextField.
	...
	/**
	 * Indicates the widest character in the alphabet, this is useful for
	 * detecting linebreaks internally. In CJK languages the widest char is
	 * different than W hence this functionality is exposed to developers.
	 */
	private static char widestChar = 'W';

	/**
	 * Indicates whether this is a single line text area, in which case
	 * "growing" won't work as expected.
	 */
	...
	public TextArea(String text, int rows, int columns) {
		this(text, defaultMaxSize, rows, columns, ANY); // String , maxSize,
														// constraints= 0 (ANY)
	}
	...
	/**
	 * Creates an area with the given text, this constructor will create a
	...
	/**
	 * Creates an empty text area, this constructor will create a single line
	 * text area similar to a text field!
	 */
	public TextArea() {
		this("");
	}
	...
	public void keyPressed(int keyCode) {
		...
		// this works around a bug where fire is also a softkey on devices such
		// as newer Nokia
		// series 40's (e.g. the Nokia emulator). It closes its native text box
		// on fire then
		// as a result of a Nokia bug we get the key released of that closing
		// and assume the
		// users wants to edit the text... When means the only way to exit the
		// native text box
	...
	public void keyReleased(int keyCode) {
		int action = com.sun.lwuit.Display.getInstance().getGameAction(keyCode);
		if (isEditable()) {
			// this works around a bug where fire is also a softkey on devices
			// such as newer Nokia
			// series 40's
	...
	/**
	 * Returns the number of actual rows in the text area taking into
	 * consideration growsByContent
	 * 
	...
	/**
	 * Override this to modify the text for rendering in cases of invalid
	 * characters for display, this method allows the developer to replace such
	 * characters e.g.: replace "\\t" with 4 spaces
	 * 
	 * @param text
	 *            the text to process
	...
	private synchronized void initRowString() {
		...
		/*
		 * if(textAreaWidth <= 0) { if(columns < 1) { textAreaWidth =
		 * Math.min(Display.getInstance().getDisplayWidth() - tPadding,
		 * getText().length()) * charWidth; } else { textAreaWidth =
		 * Math.min(Display.getInstance().getDisplayWidth() - tPadding, columns)
		 * * charWidth; } }
		 */
		...
		while (to < textLength) {
			...
			if (useStringWidth) {
				// fix for an infinite loop issue:
				// http://forums.java.net/jive/thread.jspa?messageID=482802
				// currentRowWidth = 0;
				String currentRow = "";

				// search for "space" character at close as possible to the end
				// of the row
				for (i = to; i < textLength && fastCharWidthCheck(text, from, i - from + 1, textAreaWidth, charWidth, font); i++) {
					char c = text[i];
					/*
					 * if(updateRowWidth(c, font) >= textAreaWidth) { break; }
					 */
					currentRow += c;
					...
					if (c == ' ' || c == '\n') {
						spaceIndex = i;
						// newline has been found. We can end the loop here as
						// the line cannot grow more
						if (c == '\n')
							break;
					}
					maxLength++;
	 ...
	 * 
	 * @param value
	 *            new value for autoDegradeMaxSize
	 */
	 ...
	 *            the unsupported character string
	 */
	 ...
	 * "growing" won't work as expected.
	 * 
	 * @param singleLineTextArea
	 *            set to true to force a single line text
	 */
	...
	/**
	 * Returns the absolute alignment of the TextArea In RTL LEFT alignment is
	 * actually RIGHT, but this method returns the actual alignment
	 * 
	 * @return the alignment of the TextArea one of: CENTER, LEFT, RIGHT
	 * @see #CENTER
	 * @see #LEFT
	...
	/**
	 * True is this is a qwerty device or a device that is currently in qwerty
	 * mode.
	 * 
	...
	/**
	 * Indicates whether text field input should scroll to the right side when
	 * no more room for the input is present.
	 * 
	 ...
	 * operation should only be performed once and it solves cases where a
	 * devices language might have a char bigger than 'W' that isn't
	 * consistently bigger. Notice that this method will use the TextArea style
	 * font which might differ when switching themes etc.
	 * 
	 * @param s
	 *            string to search using charWidth
	 */
	 ...
	 * those languages/fonts this flag must be set to true.
	 * 
	 * @param aUseStringWidth
	 *            the new value for useStringWidth
	 */
	...
	/**
	 * Returns the vertical alignment of the text field, this only applies to
	 * single line text field
	 * 
	 * 
	 * @return the vertical alignment of the TextField one of: CENTER, TOP,
	 *         BOTTOM
	 * @see #CENTER
	 * @see #TOP
	 * @see #BOTTOM

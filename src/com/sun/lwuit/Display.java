...
/**
 * Central class for the API that manages rendering/events and is used to place
 * top level components ({@link Form}) on the "display". Before any Form is
 * shown the Developer must invoke Display.init(Object m) in order to register
 * the current MIDlet.
 * <p>
 * This class handles the main thread for the toolkit referenced here on as the
 * EDT (Event Dispatch Thread) similar to the Swing EDT. This thread
 * encapsulates the platform specific event delivery and painting semantics and
 * enables threading features such as animations etc...
 * <p>
 * The EDT should not be blocked since paint operations and events would also be
 * blocked in much the same way as they would be in other platforms. In order to
 * serialize calls back into the EDT use the methods
 * {@link Display#callSerially} &amp; {@link Display#callSeriallyAndWait}.
 * <p>
 * Notice that all LWUIT calls occur on the EDT (events, painting, animations
 * etc...), LWUIT should normally be manipulated on the EDT as well (hence the
 * {@link Display#callSerially} &amp; {@link Display#callSeriallyAndWait}
 * methods). Theoretically it should be possible to manipulate some LWUIT
 * features from other threads but this can't be guaranteed to work for all use
 * cases.
 * 
...
public final class Display {
	...
	/**
	 * Normally LWUIT folds the VKB when switching forms this field allows us to
	 * block that behavior.
	 */
	 ...
	 * Notice that when no change is occurring on the screen no frame is drawn
	...
	/**
	 * This is the instance of the EDT used internally to indicate whether we
	 * are executing on the EDT or some arbitrary thread
	 */
	private Thread edt;

	/**
	 * Contains animations that must be played in full by the EDT before
	 * anything further may be processed. This is useful for transitions/intro's
	 * etc... that animate without user interaction.
	 */
	...
	/**
	 * Ignore all calls to show occurring during edit, they are discarded
	 * immediately
	 */
	public static final int SHOW_DURING_EDIT_IGNORE = 1;

	/**
	 * If show is called while editing text in the native text box an exception
	 * is thrown
	 */
	...
	/**
	 * Internally track display initialization time as a fixed point to allow
	 * tagging of pointer events with an integer timestamp
	 * (System.currentTimeMillis() - displayInitTime) and not a long value.
	 */
	private long displayInitTime = 0;

	/**
	 * Allows a LWUIT application to minimize without forcing it to the front
	 * whenever a new dialog is poped up
	 */
	private boolean allowMinimizing;

	/**
	 * Indicates that the LWUIT implementation should decide internally the
	 * command behavior most appropriate for this platform.
	 */
	public static final int COMMAND_BEHAVIOR_DEFAULT = 1;

	/**
	 * Indicates the classic LWUIT command behavior where the commands are
	 * placed in a list within a dialog. This is the most customizable approach
	 * for none touch devices.
	 */
	public static final int COMMAND_BEHAVIOR_SOFTKEY = 2;

	/**
	 * Indicates the touch menu dialog rendered by LWUIT where commands are
	...
	/**
	 * Identical to the bar behavior, places the back command within the title
	 * bar of the form/dialg
	 */
	public static final int COMMAND_BEHAVIOR_BUTTON_BAR_TITLE_BACK = 5;

	/**
	 * Places all commands on the right side of the title bar with a uniform
	 * size grid layout
	 */
	...
	/**
	 * This is the Display initialization method. This method must be called
	 * before any Form is shown
	 * 
	 * @param m
	 *            the main running MIDlet
	 */
	public static void init(Object m) {
		if (!INSTANCE.lwuitRunning) {
			...
			// this can happen on some cases where an application was restarted
			// etc...
			// generally its probably a bug but we can let it slide...
			if (INSTANCE.edt == null) {
				INSTANCE.touchScreen = INSTANCE.impl.isTouchDevice();

				// initialize the LWUIT EDT which from now on will take all
				// responsibility
	...
	/**
	 * Closes down the EDT and LWUIT, under normal conditions this method is
	 * completely unnecessary since exiting the application will shut down
	 * LWUIT. However, if the application is minimized and the user wishes to
	 * free all resources without exiting the application then this method can
	 * be used. Once this method is used LWUIT will no longer work and
	 * Display.init(Object) should be invoked again for any further LWUIT call!
	 * Notice that minimize (being a LWUIT method) MUST be invoked before
	 ...
	 * default this is set to 10. The advantage of limiting framerate is to
	 * allow the CPU to perform other tasks besides drawing. Notice that when no
	 * change is occurring on the screen no frame is drawn and so a high/low FPS
	 ...
	 * text in the native text box can have several behaviors:
	 * SHOW_DURING_EDIT_IGNORE, SHOW_DURING_EDIT_EXCEPTION,
	 * SHOW_DURING_EDIT_ALLOW_DISCARD, SHOW_DURING_EDIT_ALLOW_SAVE,
	 * SHOW_DURING_EDIT_SET_AS_NEXT
	 * 
	 * @param showDuringEdit
	...
	/**
	 * Returns true if we are currently in the event dispatch thread. This is
	 * useful for generic code that can be used both with the EDT and outside of
	 * it.
	 * 
	 * @return true if we are currently in the event dispatch thread; otherwise
	 *         false
	 */
	...
	/**
	 * Causes the runnable to be invoked on the event dispatch thread. This
	 * method returns immediately and will not wait for the serial call to occur
	 * 
	 * @param r
	 *            runnable (NOT A THREAD!) that will be invoked on the EDT
	 *            serial to the paint and key handling events
	 */
	...
	/**
	 * Identical to callSerially with the added benefit of waiting for the
	 * Runnable method to complete.
	 * 
	 * @param r
	 *            runnable (NOT A THREAD!) that will be invoked on the EDT
	 *            serial to the paint and key handling events
	 * @throws IllegalStateException
	 *             if this method is invoked on the event dispatch thread (e.g.
	 *             during paint or event handling).
	 */
	...
	/**
	 * Allows us to "flush" the edt to allow any pending transitions and input
	 * to go by before continuing with our other tasks.
	 */
	void flushEdt() {
		if (!isEdt()) {
			return;
	...
	/**
	 * Called by the underlying implementation to indicate that editing in the
	 * native system has completed and changes should propogate into LWUIT
	 * 
	 * @param c
	 *            edited component
	 * @param text
	...
	void processSerialCalls() {
		...
		if (size > 0) {
			...
			synchronized (lock) {
				...
				// copy all elements to an array and remove them otherwise
				// invokeAndBlock from
			...
			// after finishing an event cycle there might be serial calls
			// waiting
			// to return.
			synchronized (lock) {
				lock.notify();
	...
	/**
	 * Invokes runnable and blocks the current thread, if the current thread is
	 * the edt it will still be blocked however a separate thread would be
	 * launched to perform the duties of the EDT while it is blocked. Once
	 * blocking is finished the EDT would be restored to its original position.
	 * This is very similar to the "foxtrot" Swing toolkit and allows coding
	 * "simpler" logic that requires blocking code in the middle of event
	 * sensitive areas.
	 * 
	 * @param r
	 *            runnable (NOT A THREAD!) that will be invoked synchroniously
	 *            by this method
	 * @param dropEvents
	 *            indicates if the display should drop all events while this
	 *            runnable is running
	 */
	public void invokeAndBlock(Runnable r, boolean dropEvents) {
		this.dropEvents = dropEvents;
		if (isEdt()) {
			// this class allows a runtime exception to propogate correctly out
			// of the
			// internal thread
			...
			synchronized (lock) {
				try {
					// yeald the CPU for a very short time to let the invoke
					// thread
					// get started
					lock.wait(2);
				} catch (InterruptedException ex) {
	 ...
	 * defaults to true if the device has pen events but can be overriden by the
	 * developer.
	 * 
	...
	/**
	 * Calling this method with noSleep=true will cause the edt to run without
	 * sleeping.
	 * 
	 * @param noSleep
	...
	void setCurrent(final Form newForm, boolean reverse) {
		...
		synchronized (lock) {
			...
			if (current != null) {
				// make sure the fold menu occurs as expected then set the
				// current
				...
				if (newForm != current) {
					if ((current != null && current.getTransitionOutAnimator() != null) || newForm.getTransitionInAnimator() != null) {
						...
						// prevent form transitions from breaking our dialog
						// based
	...
	public void keyPressed(final int keyCode) {
		...
		// this solves a Sony Ericsson bug where on slider open/close someone
		// "brilliant" chose
		// to send a keyPress with a -43/-44 keycode... Without ever sending a
		// key release!
	...
	public void keyReleased(final int keyCode) {
		...
		// this can happen when traversing from the native form to the current
		// form
		// caused by a keypress
		// We need the previous key press for lwuit issue 108 which can occur
		// when typing into
		// text field rapidly and pressing two buttons at once. Originally I had
		// a patch
		// here specifically to the native edit but that patch doesn't work
		// properly for
		// all native phone bugs (e.g. incoming phone call rejected and the key
		// release is
	...
	/**
	 * Pushes a pointer hover release event with the given coordinates into
	 * LWUIT
	 * 
	 * @param x
	 *            the x position of the pointer
	...
	/**
	 * Notifies LWUIT of display size changes, this method is invoked by the
	...
	/**
	 * Broadcasts hide notify into LWUIT, this method is invoked by the LWUIT
	 * implementation to notify LWUIT of hideNotify events
	 */
	...
	/**
	 * Broadcasts show notify into LWUIT, this method is invoked by the LWUIT
	 * implementation to notify LWUIT of showNotify events
	 */
	 ...
	 * pointerReleased callback. This method will indicate if a drag occured
	 * since the pointer press event, notice that this method will not behave as
	 * expected for multi-touch events.
	 * 
	 ...
	 * transitions and returning the last current in the transition (the
	 * upcoming value for current)
	 * 
	...
	private Form getCurrentUpcomingForm(boolean includeMenus) {
		...
		// we are in the middle of a transition so we should extract the next
		// form
		if (animationQueue != null) {
	...
	public Form getCurrent() {
		...
		if (current != null && current instanceof Dialog) {
			if (((Dialog) current).isMenu() || ((Dialog) current).isDisposed()) {
				...
				// we are in the middle of a transition so we should extract the
				// next form
				if (animationQueue != null) {
					int size = animationQueue.size();
					for (int iter = 0; iter < size; iter++) {
	 ...
	 * 
	 * @param cmp
	 *            the given component to repaint
	 */
	...
	/**
	 * Returns the keycode matching the given game action constant (the opposite
	 * of getGameAction). On some devices getKeyCode returns numeric keypad
	 * values for game actions, this breaks the code since we filter these
	 * values (to prevent navigation on '2'). We pick unused negative values for
	 * game keys and assign them to game keys for getKeyCode so they will work
	 * with getGameAction.
	 * 
	 * @param gameAction
	 *            game action constant from this class
	 * @return keycode matching this constant
	 * @deprecated this method doesn't work properly across device and is mocked
	 *             up here mostly for the case of unit testing. Do not use it
	 *             for anything other than that! Do not rely on
	 *             getKeyCode(GAME_*) == keyCodeFromKeyEvent, this will never
	 *             actually happen!
	 */
	...
	/**
	 * Get the default virtual keyboard or null if the VirtualKeyboard is
	 * disabled
	 * 
	...
	/**
	 * An error handler will receive an action event with the source exception
	 * from the EDT once an error handler is installed the default LWUIT error
	...
	/**
	 * This is an internal state flag relevant only for pureTouch mode
	 * (otherwise it will always be true). A pureTouch mode is stopped if a user
	 * switches to using the trackball/navigation pad and this flag essentially
	 * toggles between those two modes.
	 * 
	 ...
	 * 
	 * @param c
	 *            the component to test against, this prevents a touch outside
	 *            of the component that triggers a repaint from painting the
	 *            component selection
	 * @return the shouldRenderSelection
	 */
	...
	/**
	 * Indicates the way commands should be added to a form as one of the
	 * ocmmand constants defined in this class
	 * 
	...
	/**
	 * Sets a local property to the application, this method has no effect on
	 * the implementation code and only allows the user to override the logic of
	 * getProperty for internal application purposes.
	 * 
	 * @param key
	 ...
	 * Notice that an audio is "auto destroyed" on completion and cannot be
	 * played twice!
	 * 
	 * @param uri
	...
	/**
	 * Create the sound in the given stream Notice that an audio is
	 * "auto destroyed" on completion and cannot be played twice!
	 * 
	 * @param stream
